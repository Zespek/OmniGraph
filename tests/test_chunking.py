"""Tests for token-aware chunking and parallel chunk execution in omnigraph.llm."""
import time
...
def test_pack_chunks_packs_small_files_together(tmp_path):
    ...
    from omnigraph.llm import _pack_chunks_by_tokens
...
def test_estimate_file_tokens_uses_tiktoken_when_available(tmp_path):
    ...
    # so the hardening call (disallowed_special=()) reaches the mock.
...
def test_corpus_parallel_runs_chunks_concurrently(tmp_path):
    ...
    from omnigraph.llm import extract_corpus_parallel
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=slow_extract):
        t0 = time.time()
...
def test_corpus_parallel_sequential_when_max_concurrency_is_one(tmp_path):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=record):
...
def test_corpus_parallel_merge_order_is_submission_order_not_completion(tmp_path):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=latency_skewed):
        result = extract_corpus_parallel(
...
def test_corpus_parallel_continues_after_chunk_failure(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=maybe_fail):
        result = extract_corpus_parallel(
            files, backend="kimi", token_budget=None, chunk_size=1, max_concurrency=1
        )
...
def test_checkpoint_scopes_cache_writes_to_chunk_files(tmp_path):
    ...
    from omnigraph.llm import extract_corpus_parallel
    from omnigraph.cache import save_semantic_cache, load_cached
    ...
    # a stray node to B.py — the mis-attribution.
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=stray):
        extract_corpus_parallel(
    ...
    # entries with the prompt that produced them, so read that namespace.
    from omnigraph.llm import _extraction_system
...
def test_truncated_chunk_is_cached_partial_and_missed_on_reload(tmp_path):
    ...
    from omnigraph.llm import extract_corpus_parallel, _extraction_system
    from omnigraph.cache import load_cached
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=truncated):
        extract_corpus_parallel(
...
def test_checkpoint_writes_deep_namespace_in_deep_mode(tmp_path):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=ok):
        extract_corpus_parallel(
            [doc], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
            deep_mode=True,
    ...
    # — a deep run's prompt carries the deep suffix.
...
def test_omitted_documents_are_reconciled_and_warned(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=omit_odd):
        result = extract_corpus_parallel(
...
def test_out_of_scope_nodes_are_dropped_from_merged_result(tmp_path, capsys):
    ...
    # B.py exists on disk but is NOT dispatched — the out-of-scope case.
    b = tmp_path / "B.py"; b.write_text("def b(): pass\n")
...
def test_out_of_scope_drop_count_is_zero_when_all_in_scope(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=clean):
        result = extract_corpus_parallel(
            [a], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
        )
...
def test_checkpoint_caches_sliced_document_chunks(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=sliced):
        extract_corpus_parallel(
            [doc], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
        )
    ...
    # The checkpoint stamps entries with the prompt that produced them.
...
def test_adaptive_retry_returns_directly_when_not_truncated(tmp_path):
    ...
    from omnigraph.llm import _extract_with_adaptive_retry
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=stub):
...
def test_adaptive_retry_caps_at_max_depth(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=always_truncate):
...
# ---- special-token strings in docs must not crash token estimation ----
