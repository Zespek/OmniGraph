...
import omnigraph.llm as llmmod
...
# ── 2. Integration / CLI Tests for Behavior ─────────────────────────────
...
def test_3203_e2e_repro_protected_and_manifest_unstamped(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(mainmod.sys, "argv", ["omnigraph", "extract", str(corpus), "--backend", "claude"])
    code1 = _run_cli()
    assert code1 == 0
...
def test_3203_allow_partial_override_permits_intentional_reduction(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--backend", "claude", "--allow-partial"],
    )
