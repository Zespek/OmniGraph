...
from omnigraph.affected import affected_nodes
from omnigraph.extract import extract, extract_python
...
def test_cross_file_indirect_survives_id_relativization(tmp_path):
    ...
    root, as the `omnigraph extract` CLI passes), the id-remap rewrites node ids
    ...
    # the internal callable marker now persists to graph.json, so an
...
def test_cross_file_affected_includes_importing_dispatcher(tmp_path):
    ...
    # editing on_event now flags the cross-module dispatcher — the gap
...
# ── Dispatch tables (slice 1): functions referenced as collection values ─
