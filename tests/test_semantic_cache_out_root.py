...
#1990 — `omnigraph extract --out` saves recovery checkpoints in the wrong directory.
...
def _semantic_dir(root: Path, mode: str | None = None) -> Path:
    ...
    return root / "omnigraph-out" / "cache" / kind
...
# ---------------------------------------------------------------------------
# — checkpoint writes to cache_root, not corpus root
# ---------------------------------------------------------------------------
...
def test_save_semantic_cache_no_corpus_omnigraph_out_created(tmp_path):
    """With cache_root set, no omnigraph-out/ dir should be created inside corpus."""
    corpus = tmp_path / "corpus"
    out = tmp_path / "out"
    corpus.mkdir()
    out.mkdir()
    ...
    assert not (corpus / "omnigraph-out").exists(), (
        "omnigraph-out/ must not be created inside the corpus when cache_root is set"
    )


# ---------------------------------------------------------------------------
# — checkpoint written with cache_root is found on recovery read
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# — final save resolves source_file against corpus root, not out_root
# ---------------------------------------------------------------------------
