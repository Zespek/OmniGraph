...
def test_score_nodes_multiword_exact_label_outranks_superset():
    ...
    Regression for the `omnigraph path` "No path found" bug: every node sharing
...
def test_find_node_resolves_when_label_and_norm_label_diverge():
    # hardening: the tokenized-label tier only rescues the match by
...
def test_find_node_matches_punctuated_node_id_exactly():
    # the id is only ever compared against `term`, which tokenizes on \w+
...
def test_find_node_matches_merge_graphs_namespaced_node_id():
    # `prefix_graph_for_global` namespaces every id with "<repo>::", so on a
...
def _force_full_scan(monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.serve._trigram_candidates", lambda *a, **k: None)
...
def test_node_search_text_appends_folded_non_ascii_node_id():
    # for a Hangul id the raw and folded forms differ — precomposed syllables
...
def test_find_node_matches_non_ascii_node_id_through_prefilter():
    # `_node_search_text` indexed the id raw while every query folds through
...
def test_query_terms_filters_only_short_english_terms(monkeypatch):
    import omnigraph.serve as serve_mod
...
def test_load_graph_missing_file(tmp_path):
    omnigraph_dir = tmp_path / "omnigraph-out"
    omnigraph_dir.mkdir()
    with pytest.raises(SystemExit):
        _load_graph(str(omnigraph_dir / "nonexistent.json"))
...
def test_load_graph_corrupted_json_prints_recovery_message(tmp_path, capsys):
    ...
    assert "Re-run /omnigraph to rebuild" in err
...
def test_load_graph_rejects_oversized_file(monkeypatch, tmp_path, capsys):
    ...
    p = tmp_path / "graph.json"
    p.write_text(json.dumps(data))
    monkeypatch.setattr("omnigraph.security._MAX_GRAPH_FILE_BYTES", 16)
    with pytest.raises(SystemExit):
        _load_graph(str(p))
    err = capsys.readouterr().err
...
def test_load_graph_accepts_under_cap(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.security._MAX_GRAPH_FILE_BYTES", 10 * 1024 * 1024)
    G2 = _load_graph(str(p))
    assert G2.number_of_nodes() == G.number_of_nodes()
...
# --- integration: identifier + noise query seeds from identifier ---
...
# --- relational-intent verbs must not seat decoy seeds ---
...
def test_query_graph_text_context_filter_aliases_resolve():
    import networkx as nx
    from omnigraph.serve import _normalize_context_filters
...
def test_serve_import_is_clean_under_syntax_warnings(tmp_path):
    ...
    result = subprocess.run(
        [
            ...
            "-c", "import omnigraph.serve as serve; assert serve._jieba is not None",
        ],
        capture_output=True,
        text=True,
    )
...
# --- get_community header: show the community name, no placeholder doubling ---
...
def test_pick_seeds_with_optimized_best_seed_matches_legacy_semantics(terms):
    ...
    # the same normalized label already claimed the slot (label dedup).
...
def test_score_query_matches_legacy_under_full_scan_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "omnigraph.serve._trigram_candidates", lambda G, needles: None
    )
...
def test_query_graph_text_makes_exactly_one_score_query_call(monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.serve._score_query", counting_sq)
    monkeypatch.setattr("omnigraph.serve._score_nodes", counting_sn)
...
# --- token budget on get_neighbors / get_community line lists ----------
...
def test_shortest_path_tool_directed_backwards_is_no_path():
    # Directed is the default: walking the chain backwards must report
...
def test_snake_case_identifier_still_matches_itself():
    ...
    G = nx.Graph()
    G.add_node("n1", label="_query_terms", source_file="omnigraph/serve.py",
...
def test_resolve_single_node_shared_by_get_node_and_get_neighbors():
    ...
    from omnigraph.serve import _resolve_single_node
...
def test_score_nodes_rationale_does_not_count_toward_term_coverage():
    ...
    from omnigraph.serve import _EXACT_MATCH_BONUS
    G = nx.Graph()
...
def test_node_search_text_includes_rationale_so_trigram_prefilter_stays_complete():
    ...
    # No rationale: field layout unchanged (the positions still hold).
