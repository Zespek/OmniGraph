...
import pytest
from omnigraph.extract import extract_js, extract_go, extract_rust, extract, extract_sql
...
def test_go_method_declaration_emits_refs_only_when_name_present():
    ...
    import ast
    import inspect
    from omnigraph.extract import extract_go
...
def test_rust_trait_method_declarations_are_captured():
    # signature-only trait method declarations (and default-bodied ones)
...
def test_rust_trait_decl_method_is_distinct_from_impl_method():
    # the trait-declared `Processor.run` must be its own node, not collide
...
def test_mask_sql_comments_literal_and_comment_handling():
    ...
    from omnigraph.extractors.sql import _mask_sql_comments as mask
...
# from omnigraph: the point is that future edits to the live mask are
