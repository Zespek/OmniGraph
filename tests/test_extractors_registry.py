...
own ``omnigraph/extractors/<lang>.py`` module, kept a verbatim re-export in
``omnigraph.extract`` (the facade every existing importer uses), and seeded a
``omnigraph.extractors.LANGUAGE_EXTRACTORS`` registry. Three things must stay
true for that split to be behavior-preserving:

- the function is importable from its new per-language module,
- ``omnigraph.extract`` still re-exports the SAME function object (facade
...
- ``LANGUAGE_EXTRACTORS`` maps to that same object (registry identity).
 ...
 as a per-language check; generalized
...
import omnigraph.extract as facade
from omnigraph.extractors import LANGUAGE_EXTRACTORS
...
def test_every_registry_extractor_is_reexported_from_facade():
    ...
    assert not missing, f"registry extractors not re-exported from omnigraph.extract: {missing}"
    assert not diverged, f"facade object diverges from registry: {diverged}"


def test_terraform_migrated():
    # The concrete anchor from extract_terraform lives in its own module,
