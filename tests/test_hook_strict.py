...
import omnigraph.cli as cli


def _fixture(tmp_path, *, indexed=True, fresh=True):
    """A project with omnigraph-out/graph.json + manifest and one source file.
...
def test_strict_first_read_denies_then_nudges(tmp_path, monkeypatch):
    ...
    assert "omnigraph query" in json.loads(out1)["hookSpecificOutput"]["permissionDecisionReason"]
    # marker created
    assert (tmp_path / "omnigraph-out" / "cache" / "hook_sessions" / "s1.denied").exists()
...
def test_fresh_query_stamp_suppresses_deny(tmp_path, monkeypatch):
    f = _fixture(tmp_path)
    stamp = tmp_path / "omnigraph-out" / "cache" / "last_query_stamp"
...
def test_expired_query_stamp_still_denies(tmp_path, monkeypatch):
    ...
    out = _invoke("read", _read(f), tmp_path, monkeypatch, strict=True, env={"OMNIGRAPH_HOOK_STRICT_TTL": "1800"})
    assert _is_deny(out)
...
def test_env_forces_strict_on(tmp_path, monkeypatch):
    f = _fixture(tmp_path)
    out = _invoke("read", _read(f), tmp_path, monkeypatch, strict=False, env={"OMNIGRAPH_HOOK_STRICT": "1"})
    assert _is_deny(out)


def test_env_kills_strict(tmp_path, monkeypatch):
    f = _fixture(tmp_path)
    out = _invoke("read", _read(f), tmp_path, monkeypatch, strict=True, env={"OMNIGRAPH_HOOK_STRICT": "0"})
...
def test_needs_update_flag_softens(tmp_path, monkeypatch):
    f = _fixture(tmp_path)
    (tmp_path / "omnigraph-out" / "needs_update").write_text("1", encoding="utf-8")
    out = _invoke("read", _read(f), tmp_path, monkeypatch, strict=True)
...
def test_strict_enabled_env_precedence():
    import os as _os
    saved = _os.environ.get("OMNIGRAPH_HOOK_STRICT")
    try:
        _os.environ["OMNIGRAPH_HOOK_STRICT"] = "1"
        assert cli._hook_strict_enabled(False) is True
        _os.environ["OMNIGRAPH_HOOK_STRICT"] = "0"
        assert cli._hook_strict_enabled(True) is False
        _os.environ.pop("OMNIGRAPH_HOOK_STRICT", None)
        ...
        if saved is None:
            ...
            _os.environ["OMNIGRAPH_HOOK_STRICT"] = saved


def test_install_hook_carries_strict_flag():
    from omnigraph.install import _claude_pretooluse_hooks
