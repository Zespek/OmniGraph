...
from omnigraph.analyze import god_nodes, surprising_connections, _is_concept_node, graph_diff, _surprise_score, _file_category, _is_json_key_node, find_import_cycles, suggest_questions
...
def _make_file_node(path: str) -> tuple[str, dict]:
    """Create a graph node resembling real omnigraph schema."""
    nid = _make_id(path)
