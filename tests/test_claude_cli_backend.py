...
def test_nonzero_exit_surfaces_envelope_error_when_stderr_empty():
    ...
    # Without reading it the user gets a bare "exited 1: " and no cause.
    completed = MagicMock(
...
def test_raises_on_error_envelope_with_zero_exit():
    ...
    # bisects the chunk while every request is still being refused.
    completed = MagicMock(
...
def test_raises_on_array_shaped_error_envelope():
    ...
    # object last; the is_error flag must be honoured in that shape too.
    streamed = [
...
def test_call_llm_raises_on_error_envelope():
    ...
    # that string into the graph as a label.
    completed = MagicMock(
        returncode=0, stdout=json.dumps(_ERROR_ENVELOPE), stderr="",
    )
    with patch("shutil.which", return_value="/fake/bin/claude"), \
...
def test_extraction_instructions_ride_in_user_turn(fake_claude):
    ...
    assert "omnigraph semantic extraction agent" in sent
...
# `--json-schema`, omnigraph constrains the output shape structurally so the
...
def test_json_schema_flag_added_when_cli_supports_it(monkeypatch, fake_claude):
    ...
    pins the top-level {nodes, edges} shape omnigraph parses."""
...
# ---------- OMNIGRAPH_API_TIMEOUT honoured by all backends ----------
