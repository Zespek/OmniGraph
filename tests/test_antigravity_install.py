...
the SKILL.md but never wrote `.agents/rules/omnigraph.md` or
`.agents/workflows/omnigraph.md` - even though the uninstall path removes them.
"""
import omnigraph.__main__ as m


def test_antigravity_project_install_writes_rules_and_workflows(tmp_path):
    m._project_install("antigravity", tmp_path)
    skill = tmp_path / ".agents" / "skills" / "omnigraph" / "SKILL.md"
    rules = tmp_path / ".agents" / "rules" / "omnigraph.md"
    workflow = tmp_path / ".agents" / "workflows" / "omnigraph.md"
...
def test_antigravity_workflow_names_no_skill_path(tmp_path):
    ...
    point at ~/.gemini/config/skills/omnigraph/SKILL.md, which it never writes.
    """
    m._project_install("antigravity", tmp_path)
    body = (tmp_path / ".agents" / "workflows" / "omnigraph.md").read_text(encoding="utf-8")
    ...
    assert "omnigraph skill" in body, "workflow should still point at the skill by name"
...
def test_antigravity_global_install_workflow_names_no_skill_path(tmp_path, monkeypatch):
    ...
    assert (
        tmp_path / "home" / ".gemini" / "config" / "skills" / "omnigraph" / "SKILL.md"
    ).exists()
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".agents" / "rules" / "omnigraph.md").exists()
    assert not (tmp_path / ".agents" / "workflows" / "omnigraph.md").exists()
    assert not (tmp_path / ".agents" / "skills" / "omnigraph" / "SKILL.md").exists()
