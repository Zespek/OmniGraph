"""Tests for `omnigraph reflect` and the work-memory reflection layer.

`omnigraph reflect` reads the outcome-tagged Q&A docs that `omnigraph save-result`
files into omnigraph-out/memory/ and writes a deterministic lessons artifact
(omnigraph-out/reflections/LESSONS.md) an agent can load next session: preferred
...
from omnigraph.ingest import save_query_result
from omnigraph.reflect import (
    aggregate_lessons,
    lessons_fresh,
    load_memory_docs,
    parse_memory_doc,
...
def _write_raw_doc(mem: Path, filename: str, date: str, *, outcome="dead_end",
    ...
    lines = ["---", 'type: "query"', f'date: "{date}"', f'question: "{question}"',
             'contributor: "omnigraph"', f'outcome: "{outcome}"']
    if nodes:
...
def test_cli_reflect_end_to_end(tmp_path):
    ...
    lessons = cwd / "omnigraph-out" / "reflections" / "LESSONS.md"
...
def test_cli_save_result_reads_answer_from_file(tmp_path):
    ...
    docs = list((tmp_path / "omnigraph-out" / "memory").glob("*.md"))
...
def test_cli_reflect_cold_start_writes_empty_lessons(tmp_path):
    """First run with no omnigraph-out/memory/ still succeeds and writes a valid doc."""
    r = _run(["reflect"], tmp_path)
    assert r.returncode == 0, r.stderr
    assert "Reflected 0 memories" in r.stdout
    lessons = tmp_path / "omnigraph-out" / "reflections" / "LESSONS.md"
    assert lessons.exists()
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels in tmp_path/omnigraph-out/.
    ...
    (out / ".omnigraph_analysis.json").write_text(json.dumps({
...
@pytest.mark.parametrize("sidecar_name", [".omnigraph_analysis.json", ".omnigraph_labels.json"])
def test_lessons_fresh_false_when_graph_sidecar_newer(tmp_path, sidecar_name):
    ...
    out = tmp_path / "LESSONS.md"; out.write_text("y", encoding="utf-8")
    graph = tmp_path / "graph.json"; graph.write_text("{}", encoding="utf-8")
    analysis = tmp_path / ".omnigraph_analysis.json"; analysis.write_text("{}", encoding="utf-8")
    labels = tmp_path / ".omnigraph_labels.json"; labels.write_text("{}", encoding="utf-8")
...
# --- work-memory overlay sidecar (.omnigraph_learning.json) --------------------
#
...
from omnigraph.reflect import (  # noqa: E402
...
def test_sidecar_write_classifies_and_keys_by_canonical_id(tmp_path):
    """reflect with a graph writes .omnigraph_learning.json next to graph.json with
...
def test_relative_source_file_not_spuriously_stale_in_omnigraph_out_layout(tmp_path):
    """Regression: with a RELATIVE source_file and graph.json under omnigraph-out/,
    ...
    (omnigraph-out/) — otherwise every node looked unfindable and was marked stale.
    The edit case must still flip stale=True."""
    out = tmp_path / "omnigraph-out"          # graph.json lives here
...
def test_relative_source_file_resolved_via_omnigraph_root_marker(tmp_path):
    """When a committed .omnigraph_root marker records the project root (e.g. a
    OMNIGRAPH_OUT override pointing the output dir elsewhere), the fingerprint
    ...
    (out / ".omnigraph_root").write_text(str(proj), encoding="utf-8")  # the marker
    mem = out / "memory"
    _write_raw_doc(mem, "a.md", "2026-05-01", outcome="useful", nodes=["login()"])
    _write_raw_doc(mem, "b.md", "2026-05-10", outcome="useful", nodes=["login()"])
    reflect(mem, out / "reflections" / "LESSONS.md",
...
def test_flat_layout_does_not_match_same_named_file_one_dir_up(tmp_path):
    ...
    # Flat layout: graph.json sits directly in proj/ (not a omnigraph-out subdir).
