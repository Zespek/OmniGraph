...
from omnigraph.extract import extract_python, extract, collect_files, _make_id, extract_bash, extract_json, _DISPATCH
...
def test_go_imported_type_stubs_do_not_collide_across_source_files(tmp_path):
    ...
    #1462 kept unresolved bare stubs distinct per source file because OmniGraph
...
def test_collect_files_from_dir():
    from omnigraph.extract import _DISPATCH
...
def test_extract_generic_surfaces_tree_sitter_version_mismatch_hint(monkeypatch):
    ...
    import sys
    import types
    from omnigraph.extract import _extract_generic, LanguageConfig
...
def test_extract_js_destructured_require_imports_from():
    ...
    from omnigraph.extract import extract_js
...
def test_extract_js_destructured_require_named_symbols():
    ...
    from omnigraph.extract import extract_js, _make_id, _file_stem
    result = extract_js(FIXTURES / "cjs_require.js")
...
def test_out_of_tree_cache_root_keeps_source_file_relative_to_scan_root(tmp_path):
    ...
    # must not regress: no absolute path / username leak.
    for n in result["nodes"]:
...
def test_extract_tsx_uses_tsx_grammar():
    ...
    from omnigraph.extract import _TSX_CONFIG, _TS_CONFIG
...
def test_extract_falls_back_to_sequential_when_parallel_returns_false(tmp_path, monkeypatch):
    ...
    from omnigraph import extract as extract_mod
...
def test_extract_parallel_skips_pool_when_max_workers_is_one(tmp_path, monkeypatch):
    ...
    The Windows post-commit hook exports OMNIGRAPH_MAX_WORKERS=1, so before this the
    ...
    monkeypatch.setenv("OMNIGRAPH_MAX_WORKERS", "1")
...
def test_extract_parallel_still_spawns_pool_for_multiple_workers(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNIGRAPH_MAX_WORKERS", "4")
...
def test_extract_falls_back_when_worker_future_breaks_pool(
    ...
    # A 1-CPU runner resolves max_workers to 1 and never enters the pool.
    monkeypatch.setenv("OMNIGRAPH_MAX_WORKERS", "2")
...
def test_extract_bash_emits_script_invocation_calls(tmp_path, command):
    ...
    assert invocation == [{
        ...
        # Transient canonicalization hint; popped before persist.
...
# ---------------------------------------------------------------------------
# PR regression tests — bash extractor Copilot review findings
# ---------------------------------------------------------------------------
...
def test_extract_json_dependency_edges_are_not_label_self_loops(tmp_path):
    ...
    # The dependency structure restored is still present.
...
# ---------------------------------------------------------------------------
# Data JSON must not explode into orphan key-nodes
# ---------------------------------------------------------------------------
...
def test_semantic_reference_edges_carry_context_and_source():
    from omnigraph.extract import _semantic_reference_edge
...
def test_dart_child_node_ids_are_stem_based(tmp_path):
    ...
    from omnigraph.extract import extract_dart, _file_stem, _make_id
...
def test_extract_warns_on_code_files_with_no_ast_extractor(tmp_path, capsys):
    #.r/.R is in CODE_EXTENSIONS (counted as code) but has no AST extractor,
...
def test_extract_warns_when_sql_extra_missing(tmp_path, capsys, monkeypatch):
    #.sql HAS a dispatch entry, so the warning can't fire, and
    # extract_sql returns an "error" result when tree-sitter-sql is absent, so
    # the warning skips it too. The files must not vanish silently:
    ...
    assert 'omnigraph[sql]' in err
    assert "#1745" in err
    # the Python file still extracts normally
    labels = [n.get("label") for n in result["nodes"]]
    assert any(str(l).startswith("main") for l in labels)
    # failed sql sources must be surfaced so the CLI can leave them
...
def test_extract_sql_reports_load_failure_not_missing(tmp_path, monkeypatch):
    # an installed-but-broken grammar (e.g. a wheel built for a different
...
def test_extract_warns_sql_grammar_failed_to_load(tmp_path, capsys, monkeypatch):
    # the aggregated warning must surface a present-but-broken
    ...
    assert "omnigraph[sql]" not in err
    assert "pip install" not in err
    # still surfaced as failed so the incremental manifest retries them
    failed = {Path(p).name for p in result.get("failed_sources", [])}
    assert failed == {"schema.sql", "views.sql"}


def test_extract_progress_final_line_uses_consistent_denominator(tmp_path, capsys):
    # intermediate progress lines count against uncached_work; the final
...
def test_get_extractor_routes_matlab_m_away_from_objc(tmp_path):
    #.m is shared by Objective-C and MATLAB. A real ObjC.m still routes to
    ...
    from omnigraph.extract import _get_extractor, extract_objc
...
def test_matlab_m_not_extracted_as_garbage(tmp_path, capsys):
    ...
    # no-AST-extractor warning (+), rather than mis-parsed as ObjC.
...
def test_rewire_binds_cross_module_function_reference_to_definition():
    ...
    rewire targets)."""
    from omnigraph.extract import _rewire_unique_stub_nodes
    nodes = [
...
def test_extract_declined_data_json_is_not_failed(tmp_path, capsys):
    ...
    assert result.get("failed_sources") == []
    #...and no "produced zero nodes" noise for a deliberate decline.
    assert "zero nodes" not in err
...
def test_extract_genuinely_empty_json_still_failed(tmp_path, monkeypatch):
    ...
    pytest.importorskip("tree_sitter_json")
    import omnigraph.extract as _ex
...
# ── Authoritative Python Type Reference and Inheritance Rewiring ────────
...
def test_3252_unimported_ambiguous_type_remains_unresolved(tmp_path):
    """#3252: When multiple definitions exist but consumer has no import, OmniGraph
