"""Tests for omnigraph tree / tree_html.build_tree.
...
from omnigraph.tree_html import build_tree
...
def test_tree_cli_absolute_root_mismatch_exits_1(tmp_path):
    ...
    assert not (tmp_path / "omnigraph-out" / "GRAPH_TREE.html").exists()
...
def test_tree_cli_default_root_succeeds(tmp_path):
    ...
    html = (tmp_path / "omnigraph-out" / "GRAPH_TREE.html").read_text(encoding="utf-8")
...
def test_tree_cli_partial_match_root_succeeds(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "omnigraph-out" / "GRAPH_TREE.html").exists()
