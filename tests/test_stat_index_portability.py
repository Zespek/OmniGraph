...
def _stat_index_path(root: Path) -> Path:
    return root / "omnigraph-out" / "cache" / "stat-index.json"
...
def test_cache_hits_survive_corpus_move(tmp_path, monkeypatch):
    """Run A under tmp/a, copy the corpus (with omnigraph-out/) to tmp/b: run B
    ...
    # Move the corpus (omnigraph-out/ rides along; copy2 preserves mtime_ns).
    b = tmp_path / "b"
...
def test_semantic_cache_normalizes_absolute_source_file(tmp_path):
    ...
    entries = list((root / "omnigraph-out" / "cache" / "semantic").glob("*.json"))
