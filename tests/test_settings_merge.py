...
- back up to <name>.omnigraph-bak before any modifying write,
...
def test_claude_install_preserves_existing_settings(tmp_path):
    """#2167 core case: every key omnigraph does not own must survive install."""
    seeded = {
    ...
    # top-level keys omnigraph does not own are untouched
    assert result["mcpServers"] == seeded["mcpServers"]
    assert result["enabledPlugins"] == seeded["enabledPlugins"]
    assert result["theme"] == "dark"
    # hooks sections omnigraph does not manage are untouched
    assert result["hooks"]["PostToolUse"] == seeded["hooks"]["PostToolUse"]
    # the user's own PreToolUse entry survives alongside omnigraph's
...
def test_bom_settings_are_merged_not_clobbered(tmp_path, installer):
    ...
    assert any("omnigraph" in str(h) for h in result["hooks"][section])
...
def test_invalid_json_aborts_without_clobbering(tmp_path, installer, capsys):
    ...
    assert not settings_path.with_name(settings_path.name + ".omnigraph-bak").exists()
...
def test_backup_written_before_modify_and_stable_on_reinstall(tmp_path, installer):
    ...
    backup = settings_path.with_name(settings_path.name + ".omnigraph-bak")
    ...
    # nor the backup may be rewritten (the backup keeps the pre-omnigraph content).
    _run(tmp_path, installer)
...
def test_non_dict_hook_entry_is_preserved_not_fatal(tmp_path, installer):
    ...
    assert any("omnigraph" in str(h) for h in entries)
