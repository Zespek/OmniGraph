...
def test_pg_introspect_import_error():
    ...
    # the PyPI package is omnigraph (double-y), so the install hint must match
    assert "omnigraph[postgres]" in str(exc_info.value)
...
def test_pg_introspect_grammar_error_surfaces():
    ...
    with patch.dict("sys.modules", {"psycopg": mock_psycopg}):
        with patch(
            "omnigraph.pg_introspect.extract_sql",
            return_value={
                "nodes": [],
                "edges": [],
