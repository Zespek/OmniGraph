...
2. A template lint over the generated ``omnigraph/skill*.md`` bodies (and the
   ...
   ``omnigraph-out/graph.json`` with ``community_labels=labels``. This locks the
...
def _skill_bodies() -> list[Path]:
    paths = sorted(REPO_ROOT.glob("omnigraph/skill*.md"))
    assert paths, "no generated omnigraph/skill*.md found"
    return paths
...
def test_skill_step5_reexports_graph_json_with_curated_labels(path: Path):
    ...
    ``to_json(...omnigraph-out/graph.json...)`` call in that block without the
    ...
    for block in post_labels_blocks:
        assert "to_json(G, communities, 'omnigraph-out/graph.json', community_labels=labels)" in block, (
            f"{path.name}: the post-labels Step-5 block must re-export "
            f"omnigraph-out/graph.json with community_labels=labels (#2490)"
        )
        # No label-less graph.json export may coexist in the post-labels block.
        for line in block.splitlines():
            if "to_json(" in line and "omnigraph-out/graph.json" in line:
