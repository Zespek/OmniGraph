...
def test_extract_files_direct_routes_gemini_through_openai_compat(tmp_path, monkeypatch):
    ...
    with patch("omnigraph.llm._call_openai_compat", return_value=result) as call:
    ...
    # Source content is wrapped in an untrusted_source delimiter block
...
def test_openai_compat_backends_resolve_full_output_cap(tmp_path, monkeypatch, backend, env_key):
    # these configs define `max_tokens: 16384`, but the dispatch used to
    ...
    _clear_backend_env(monkeypatch)
    monkeypatch.delenv("OMNIGRAPH_MAX_OUTPUT_TOKENS", raising=False)
...
def test_gemini_model_can_be_overridden_by_env(tmp_path, monkeypatch):
    _clear_backend_env(monkeypatch)
    monkeypatch.setenv("GOOGLE_API_KEY", "google-key")
    monkeypatch.setenv("OMNIGRAPH_GEMINI_MODEL", "gemini-3.1-pro-preview")
    source = tmp_path / "note.md"
    source.write_text("# Architecture\n")
    result = {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 1, "output_tokens": 1}
...
# ---------------------------------------------------------------------------
# public entry points accept str paths, not just pathlib.Path
# ---------------------------------------------------------------------------
...
def test_extract_files_direct_accepts_str_paths(tmp_path, monkeypatch):
    ...
    with patch("omnigraph.llm._call_openai_compat", return_value=result):
...
def test_corpus_parallel_oversized_markdown_does_not_crash_on_fileslice(tmp_path, monkeypatch):
    # regression: a Markdown file large enough to be sliced into
    # FileSlice units must not crash extract_files_direct's Path() coercion
    #. The earlier str-path tests used tiny files, so slicing never ran.
    from omnigraph.llm import _FILE_CHAR_CAP
    _clear_backend_env(monkeypatch)
    monkeypatch.setenv("GOOGLE_API_KEY", "google-key")
    big = tmp_path / "big.md"
...
def test_adaptive_retry_splits_on_context_exceeded(tmp_path):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=fake_extract):
...
# ---------------------------------------------------------------------------
# Adaptive retry: timeout recovery
# ---------------------------------------------------------------------------
...
def test_adaptive_retry_splits_single_slice_on_timeout(tmp_path, capsys):
    import subprocess
    from omnigraph.file_slice import FileSlice
...
def test_adaptive_retry_timeout_caps_at_max_depth(tmp_path, capsys):
    ...
    with patch("omnigraph.llm.extract_files_direct", side_effect=always_timeout):
        result = llm._extract_with_adaptive_retry(
...
def test_call_openai_compat_labels_empty_content_hollow(monkeypatch):
    ...
    # cannot converge and costs 2**max_retry_depth billed calls.
...
# ---------------------------------------------------------------------------
# Ollama context-window fix: num_ctx + keep_alive in extra_body,
...
def test_ollama_extra_body_sets_num_ctx_and_keep_alive(monkeypatch):
    ...
    monkeypatch.delenv("OMNIGRAPH_OLLAMA_NUM_CTX", raising=False)
    monkeypatch.delenv("OMNIGRAPH_OLLAMA_KEEP_ALIVE", raising=False)
...
def test_ollama_num_ctx_scales_with_small_token_budget(monkeypatch):
    # Regression for follow-up: with --token-budget 8192, the old hardcoded
...
def test_ollama_num_ctx_env_override(monkeypatch):
    captured = _install_capturing_openai(monkeypatch)
    monkeypatch.setenv("OMNIGRAPH_OLLAMA_NUM_CTX", "65536")
    monkeypatch.delenv("OMNIGRAPH_OLLAMA_KEEP_ALIVE", raising=False)
...
# ---------------------------------------------------------------------------
# OMNIGRAPH_DISABLE_THINKING: opt-in disable-thinking for reasoning models like
# deepseek-v4-flash. Off by default — disabling thinking trades a rare reasoning
# leak for lower extraction quality/coverage, so it must not be forced.
# ---------------------------------------------------------------------------


def test_deepseek_thinking_on_by_default(monkeypatch):
    monkeypatch.delenv("OMNIGRAPH_DISABLE_THINKING", raising=False)
    captured = _install_capturing_openai(monkeypatch)
...
def test_deepseek_thinking_disabled_via_env(monkeypatch):
    monkeypatch.setenv("OMNIGRAPH_DISABLE_THINKING", "1")
    captured = _install_capturing_openai(monkeypatch)
...
def test_extract_corpus_parallel_ollama_runs_serially(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OMNIGRAPH_OLLAMA_PARALLEL", raising=False)

    with patch("omnigraph.llm.extract_files_direct", side_effect=fake_extract):
        with patch("omnigraph.llm.ThreadPoolExecutor") as mock_pool:
...
def test_extract_corpus_parallel_ollama_parallel_env_restores_concurrency(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNIGRAPH_OLLAMA_PARALLEL", "1")

    with patch("omnigraph.llm.extract_files_direct", return_value=_ok()):
        with patch("omnigraph.llm.ThreadPoolExecutor") as mock_pool:
...
def test_call_azure_uses_correct_client_params_and_max_completion_tokens(monkeypatch):
    ...
    monkeypatch.delenv("OMNIGRAPH_API_TIMEOUT", raising=False)
...
# ---------------------------------------------------------------------------
# Temperature resolution: omit temperature for reasoning models
# (o1/o3/o4/gpt-5) and honour OMNIGRAPH_LLM_TEMPERATURE.
# ---------------------------------------------------------------------------
...
def test_resolve_temperature_default_for_normal_model(monkeypatch):
    monkeypatch.delenv("OMNIGRAPH_LLM_TEMPERATURE", raising=False)
...
def test_resolve_temperature_env_var_numeric_overrides(monkeypatch):
    monkeypatch.setenv("OMNIGRAPH_LLM_TEMPERATURE", "0.7")
...
def test_resolve_temperature_env_var_none_omits(monkeypatch):
    monkeypatch.setenv("OMNIGRAPH_LLM_TEMPERATURE", "none")
...
def test_resolve_temperature_env_var_invalid_falls_back(monkeypatch):
    monkeypatch.setenv("OMNIGRAPH_LLM_TEMPERATURE", "hot")
...
def test_openai_compat_omits_temperature_for_o3_model(tmp_path, monkeypatch):
    # Regression for with a reasoning model the request must not carry a
    # `temperature` key at all, or the API returns HTTP 400.
    _clear_backend_env(monkeypatch)
    monkeypatch.delenv("OMNIGRAPH_LLM_TEMPERATURE", raising=False)
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setenv("OMNIGRAPH_OPENAI_MODEL", "o3-mini")
    captured = _install_capturing_openai(monkeypatch)
...
def test_openai_compat_sends_temperature_for_normal_model(tmp_path, monkeypatch):
    _clear_backend_env(monkeypatch)
    monkeypatch.delenv("OMNIGRAPH_LLM_TEMPERATURE", raising=False)
    monkeypatch.delenv("OMNIGRAPH_OPENAI_MODEL", raising=False)
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    captured = _install_capturing_openai(monkeypatch)
    (tmp_path / "f.py").write_text("x = 1\n")
...
def test_openai_compat_env_var_temperature_applied(tmp_path, monkeypatch):
    _clear_backend_env(monkeypatch)
    monkeypatch.setenv("OMNIGRAPH_LLM_TEMPERATURE", "0.3")
    monkeypatch.delenv("OMNIGRAPH_OPENAI_MODEL", raising=False)
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    captured = _install_capturing_openai(monkeypatch)
    (tmp_path / "f.py").write_text("x = 1\n")
...
def test_native_extraction_prompt_requests_hyperedges():
    ...
    extraction-spec does — otherwise `omnigraph extract --backend X` silently
...
def test_native_extraction_prompt_matches_skill_spec_on_hyperedges():
    ...
    behaviour whether built via the skill or `omnigraph extract --backend`.
    """
    spec = (
        Path(__file__).resolve().parents[1]
        / "omnigraph" / "skills" / "claude" / "references" / "extraction-spec.md"
...
# --- *_BASE_URL env overrides for kimi / gemini / deepseek -------------
...
def _backend_base_url(backend: str, env_extra: dict) -> str:
    out = subprocess.run(
        [_sys.executable, "-c",
         f"import omnigraph.llm as l; print(l.BACKENDS[{backend!r}]['base_url'])"],
...
# ---------------------------------------------------------------------------
# claude-cli subprocess.run must use errors="replace" so non-UTF-8
...
def test_resolve_max_retries_default_and_env(monkeypatch):
    ...
    monkeypatch.delenv("OMNIGRAPH_MAX_RETRIES", raising=False)
    assert llm._resolve_max_retries() >= 5
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRIES", "10")
    assert llm._resolve_max_retries() == 10
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRIES", "0")
    assert llm._resolve_max_retries() == 0          # disable is allowed
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRIES", "bogus")
...
def test_call_llm_claude_client_built_with_timeout_and_retries(monkeypatch):
    ...
    (then only max_retries), so OMNIGRAPH_API_TIMEOUT was silently ignored here."""
    import sys
    import types
    ...
    monkeypatch.setenv("OMNIGRAPH_API_TIMEOUT", "1")
    monkeypatch.delenv("OMNIGRAPH_MAX_RETRIES", raising=False)
...
def test_max_retry_depth_reads_the_env_var(monkeypatch):
    """#2880: max_retry_depth was a Python-API kwarg only, so a `omnigraph
    extract` operator had no way to lower it as a mitigation."""
    monkeypatch.delenv("OMNIGRAPH_MAX_RETRY_DEPTH", raising=False)
    assert llm._resolve_max_retry_depth() == 3
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRY_DEPTH", "0")
    assert llm._resolve_max_retry_depth() == 0
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRY_DEPTH", "1")
    assert llm._resolve_max_retry_depth() == 1
    # garbage and negatives fall back to the default
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRY_DEPTH", "banana")
    assert llm._resolve_max_retry_depth() == 3
    monkeypatch.setenv("OMNIGRAPH_MAX_RETRY_DEPTH", "-2")
    assert llm._resolve_max_retry_depth() == 3
...
def test_detection_is_not_widened_by_the_labelling_fallback(monkeypatch):
    ...
    Widening `detect_backend` itself would make `omnigraph extract` start shelling
