"""Tests for `omnigraph extract` CLI dispatch path in omnigraph.__main__."""
...
import omnigraph.__main__ as mainmod
...
def test_extract_exits_nonzero_when_ast_extraction_raises(
    ...
    import omnigraph.extract as extractmod
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "extract", str(corpus), "--code-only",
         "--out", str(out_dir)],
    )
    ...
    assert (
        "[omnigraph extract] AST extraction failed: worker pool failed"
        in capsys.readouterr().err
    )
    assert not (out_dir / "omnigraph-out" / "graph.json").exists(), (
...
def test_extract_allow_partial_continues_past_ast_failure(
    ...
    monkeypatch.setattr(
        "omnigraph.llm.extract_corpus_parallel", _one_chunk_succeeded
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "extract", str(corpus), "--backend", "claude",
    ...
    assert (out_dir / "omnigraph-out" / "graph.json").exists(), (
...
def test_extract_exits_nonzero_when_all_semantic_chunks_fail(
    ...
    The bug this guards: `pip install omnigraph` doesn't pull in `anthropic`,
    so `omnigraph extract --backend claude` would print per-chunk errors and
    ...
    monkeypatch.setattr(
        "omnigraph.llm.extract_corpus_parallel", _all_chunks_failed
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_extract_succeeds_when_at_least_one_chunk_completes(
    ...
    monkeypatch.setattr(
        "omnigraph.cache.save_semantic_cache", _capture_semantic_cache
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_incremental_partial_run_preserves_untouched_semantic_hash(
    ...
    monkeypatch.setattr(
        "omnigraph.llm.extract_corpus_parallel", _stamp_everything_sent
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    ...
    _run_extract()
    manifest_path = out_dir / "omnigraph-out" / "manifest.json"
...
def test_truncated_doc_semantic_hash_is_cleared_for_requeue(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _extract)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
...
def test_manifest_stamps_freshly_extracted_semantic_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _fresh_relative)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--backend", "claude",
    ...
    # The zero-node doc stays unstamped so detect_incremental re-queues it.
...
def test_stamped_manifest_files_normalizes_both_sides(tmp_path):
    ...
    from omnigraph.cli import _stamped_manifest_files
    ...
    sem_result = {
        ...
        # edge-only: must NOT count toward stamping
...
def test_stamped_manifest_files_counts_hyperedge_only_docs(tmp_path):
    ...
    # A doc with no output at all still stays unstamped.
    assert str(omitted_doc) not in out["document"]
...
def test_manifest_stamps_hyperedge_only_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _hyperedge_only)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--backend", "claude",
    ...
    manifest = json.loads((out_dir / "omnigraph-out" / "manifest.json").read_text())
    assert manifest.get("README.md", {}).get("semantic_hash"), (
        f"hyperedge-only doc must be stamped (#1920): {sorted(manifest)}"
    )


# --- --force and deep-mode dispatch over a warm cache -----------------
...
def test_extract_mode_deep_dispatches_over_warm_cache(monkeypatch, tmp_path):
    ...
    calls: list[dict] = []
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel",
                        _recording_extractor(calls))
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)

    # No --out: the default layout (omnigraph-out/ beside the sources) keeps the
    ...
    base = ["omnigraph", "extract", str(corpus), "--backend", "claude",
            "--no-cluster"]
    ...
    # nested under a p{prompt-fingerprint}/ subdir, hence the recursive glob.
    assert any((corpus / "omnigraph-out" / "cache" / "semantic-deep").glob("**/*.json"))
...
def test_extract_force_flag_redispatches_and_stamps_manifest(monkeypatch, tmp_path):
    ...
    # Entries nest under a p{prompt-fingerprint}/ subdir.
    assert any((corpus / "omnigraph-out" / "cache" / "semantic").glob("**/*.json"))
    manifest = json.loads(
        (corpus / "omnigraph-out" / "manifest.json").read_text()
    )
    assert manifest.get("README.md", {}).get("semantic_hash"), (
...
def test_extract_omnigraph_force_env_redispatches(monkeypatch, tmp_path):
    """OMNIGRAPH_FORCE=1 behaves like --force (env parity with `update`)."""
    corpus = _make_corpus(tmp_path)
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test-fake-key")
    monkeypatch.delenv("OMNIGRAPH_FORCE", raising=False)
    calls: list[dict] = []
    ...
    monkeypatch.setenv("OMNIGRAPH_FORCE", "1")
    _run_extract(monkeypatch, base)
    assert len(calls) == 2, "OMNIGRAPH_FORCE=1 must force a re-dispatch"
...
def test_cache_check_mode_deep_reads_deep_namespace(monkeypatch, tmp_path, capsys):
    ...
    from omnigraph.cache import save_semantic_cache
    ...
    _run_extract(monkeypatch, ["omnigraph", "cache-check", str(files_from),
                               "--root", str(tmp_path)])
...
def test_extract_codeonly_succeeds_without_api_key(monkeypatch, tmp_path):
    ...
    Regression: omnigraph extract validated a backend upfront and exited 1 with
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--out", str(out_dir)],
    )
    ...
    graph = out_dir / "omnigraph-out" / "graph.json"
...
def test_missing_manifest_code_only_preserves_semantic_layer(monkeypatch, tmp_path):
    """#1925: `omnigraph extract --code-only` with a MISSING manifest.json must
    ...
    (corpus / "README.md").write_text("# Notes\nCurated docs.\n")
    out_dir = tmp_path / "out"
    omnigraph_out = out_dir / "omnigraph-out"
    _clear_backend_keys(monkeypatch)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    ...
    _run_extract(monkeypatch, ["omnigraph", "extract", str(corpus),
                               "--code-only", "--out", str(out_dir)])
    graph_path = omnigraph_out / "graph.json"
    ...
    (omnigraph_out / "manifest.json").unlink()
    ...
    (omnigraph_out / "manifest.json").unlink(missing_ok=True)
    _run_extract(monkeypatch, ["omnigraph", "extract", str(corpus),
                               "--code-only", "--out", str(out_dir)])
...
def test_extract_out_keeps_project_root_clean(monkeypatch, tmp_path):
    """`extract --out DIR` routes every artifact to DIR/omnigraph-out/ and the
    scanned project must not grow a omnigraph-out/ (or anything else) beside
    its sources.
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", ".", "--out", str(external)],
    )
    ...
    assert not (corpus / "omnigraph-out").exists(), (
        "scanned project must not grow a omnigraph-out/ when --out is set"
    )
...
def test_extract_without_key_still_errors_when_docs_present(
    ...
    monkeypatch.setattr("omnigraph.llm.detect_backend", lambda: None)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--out", str(out_dir)],
...
def test_extract_timing_flag_emits_stage_timings(monkeypatch, tmp_path, capsys):
    """--timing prints per-stage `[omnigraph timing]` lines to stderr (#1490); omitting
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o1"), "--timing"],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
    err = capsys.readouterr().err
    assert "[omnigraph timing] detect:" in err
    assert "[omnigraph timing] total:" in err

    # without --timing => no timing lines
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o2")],
    )
    with pytest.raises(SystemExit) as exc2:
        mainmod.main()
    assert exc2.value.code == 0
    assert "omnigraph timing" not in capsys.readouterr().err
...
def test_pathless_postgres_extract_initializes_empty_detection(
    ...
    graph_path = out_root / "omnigraph-out" / "graph.json"
    manifest = out_root / "omnigraph-out" / "manifest.json"
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("omnigraph.pg_introspect.introspect_postgres", _introspect)
    ...
    (out_root / "omnigraph-out" / ".omnigraph_semantic_marker").write_text(
        '{"output_tokens": 1}'
    )
    ...
    backups = [
        path
        for path in (out_root / "omnigraph-out").iterdir()
...
# ---------------------------------------------------------------------------
# a newly-excluded file's nodes must be pruned from graph.json on the
# next incremental extract even when the manifest never listed the file (the
# pre- state every 0.9.16 graph is in), so the manifest-diff prune set
...
def test_incremental_extract_prunes_newly_excluded_file_not_in_manifest(
    ...
    .omnigraphignore, re-run extract: x.py's nodes must be gone even though it
...
def test_no_cluster_incremental_prunes_newly_excluded_file(
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(project), "--no-cluster", "--out", str(out_dir)],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
...
# ---------------------------------------------------------------------------
# a code file whose AST extraction FAILED (error result — e.g. missing
...
# only `rm -rf omnigraph-out` recovers. The extractor is swapped through
...
def _failing_sql(path):
    # Mirrors extractors/sql.py's missing-extra result.
    return {"nodes": [], "edges": [],
...
def test_failed_extra_is_retried_and_recovers(monkeypatch, tmp_path, capsys):
    ...
    graph_path = out_dir / "omnigraph-out" / "graph.json"
    manifest_path = out_dir / "omnigraph-out" / "manifest.json"
    argv = ["omnigraph", "extract", str(project), "--out", str(out_dir)]
...
def test_cache_check_prompt_file_scopes_hits_to_that_prompt(monkeypatch, tmp_path, capsys):
    ...
    base = ["omnigraph", "cache-check", str(files_from), "--root", str(tmp_path)]
...
# --- zero-node / edge-only semantic retry and manifest healing --------
...
def test_edge_only_semantic_extraction_not_stamped_and_retried(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _mock_llm)
    argv = ["omnigraph", "extract", str(corpus), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
    ...
    # ARCH.md has only edges -> NOT stamped in manifest
    ...
    from omnigraph.cache import file_hash
    cache_sem_dir = out_dir / "omnigraph-out" / "cache" / "semantic"
...
def test_stale_poisoned_manifest_semantic_source_healed(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _mock_llm)
    argv = ["omnigraph", "extract", str(project), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
...
# ---------------------------------------------------------------------------
# LLM output misattributed to a NON-dispatched file must not reach
...
def test_incremental_stray_attribution_preserves_undispatched_file(
    ...
    monkeypatch.setattr(
        "omnigraph.llm.extract_corpus_parallel", _seed_extraction
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    argv = ["omnigraph", "extract", str(project), "--out", str(out_dir)]
    _run_extract(monkeypatch, argv)
    ...
    monkeypatch.setattr(
        "omnigraph.llm.extract_corpus_parallel", _straying_extraction
    )
    _run_extract(monkeypatch, argv)
