...
from omnigraph.serve import _load_graph, _query_graph_text


def _write_calls_graph(tmp_path):
    """One `calls` edge, on-disk undirected — the `omnigraph extract` shape."""
    G = nx.Graph()
...
def test_traversal_view_keeps_multigraph_parallel_and_mutual_edges():
    from omnigraph.serve import _traversal_view
    G = nx.MultiDiGraph()
