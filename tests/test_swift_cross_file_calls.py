...
def test_swift_cross_file_member_calls_resolve(tmp_path: Path):
    # cross-file member calls (recv.method()), static/singleton calls, and
...
def test_swift_cross_file_member_calls_have_correct_confidence_and_resolve(tmp_path: Path):
    ...
    # they are EXTRACTED, matching the Python qualified-class-method pass.
...
def test_swift_ambiguous_type_does_not_over_connect(tmp_path: Path):
    # guard: when the receiver's type name is defined in 2+ files the
...
def test_cross_file_extension_does_not_erase_static_calls(tmp_path: Path):
    # swift_extensions[].nid is recorded pre-remap, so with absolute input
    ...
    # file was enough to zero the type's call graph, defeating.
...
def test_same_file_extension_still_merges(tmp_path: Path):
    # Type, extension, and caller in ONE file: the pre- behaviour must hold —
...
# ── attribute-argument and factory-returned receiver types ─────────────
...
def test_stateobject_annotated_receiver_still_resolves(tmp_path: Path):
    ...
    # var vm: ViewModel) resolved before and must keep resolving — the
...
def test_factory_receiver_resolves_through_cross_file_extension(tmp_path: Path):
    # composition: the called method lives in a cross-file `extension
