"""`omnigraph extract --code-only` indexes code without an LLM key (#1734).
...
def _run(repo: Path, *extra: str):
    ...
    env["OMNIGRAPH_OUT"] = str(repo / "omnigraph-out")
    return subprocess.run(
        [PYTHON, "-m", "omnigraph", "extract", ".", *extra],
...
def test_code_only_succeeds_without_key(tmp_path):
    ...
    graph = repo / "omnigraph-out" / "graph.json"
...
def test_extract_usage_advertises_code_only(tmp_path):
    ...
    by triggering the no-key error. `omnigraph extract` with no path prints usage."""
    r = subprocess.run(
        [PYTHON, "-m", "omnigraph", "extract"],
...
def _run_relative_out(repo: Path, *extra: str):
    """Like _run but with a RELATIVE OMNIGRAPH_OUT so --out/--output controls the
    parent dir (an absolute OMNIGRAPH_OUT would override the flag)."""
    env = {k: v for k, v in os.environ.items() if k not in _KEY_VARS}
    env["OMNIGRAPH_OUT"] = "omnigraph-out"
    return subprocess.run(
        [PYTHON, "-m", "omnigraph", "extract", ".", *extra],
        cwd=repo, capture_output=True, text=True, env=env,
    )
...
def test_output_flag_is_alias_of_out(tmp_path):
    ...
    to the default `<path>/omnigraph-out/`). It is now an alias of `--out`."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "app.py").write_text("def hello():\n    return 1\n")
    ...
    assert (custom / "omnigraph-out" / "graph.json").exists(), "--output was ignored (#2004)"
    assert not (repo / "omnigraph-out").exists(), "output must not go to the default dir"
...
def test_output_flag_inline_form(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (custom / "omnigraph-out" / "graph.json").exists()


def test_no_gitignore_indexes_vcs_ignored_code_but_keeps_omnigraphignore(tmp_path):
    repo = tmp_path / "repo"
    ...
    (repo / "proj" / ".omnigraphignore").write_text("hidden/\n")
    ...
    graph = json.loads((repo / "omnigraph-out" / "graph.json").read_text())
...
def test_no_gitignore_setting_persists_across_flagless_extract(tmp_path):
    ...
    `omnigraph extract` must NOT clobber it back to honoring .gitignore (which
    ...
    def _sources():
        g = json.loads((repo / "omnigraph-out" / "graph.json").read_text())
...
def test_explicit_exclude_replaces_persisted_setting_with_custom_out(tmp_path):
    ...
    graph_out = out_root / "omnigraph-out"
    def _sources():
    ...
    assert json.loads((graph_out / ".omnigraph_build.json").read_text()) == {
        "excludes": ["generated"]
    }
...
def test_code_only_force_rescan_re_resolves_tsconfig_paths_and_preserves_semantics(tmp_path: Path):
    ...
    assert r1.returncode == 0, r1.stderr
    graph_path = repo / "omnigraph-out" / "graph.json"
    ...
    # 2. Seed a semantic file and node into graph.json to verify semantic preservation
