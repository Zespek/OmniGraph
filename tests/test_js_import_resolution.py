...
from omnigraph.extract import _file_node_id, _file_stem, _make_id, extract
...
def test_ts_export_star_skips_same_named_interface_method_and_binds_the_function(tmp_path: Path):
    # `export *` can only forward top-level bindings. When the first
...
def test_ts_dynamic_import_does_not_create_phantom_cycle(tmp_path: Path):
    ...
    from omnigraph.analyze import find_import_cycles
...
# ── tsconfig path-alias fallback targets ──────────────────────────────
...
# ── wildcard tsconfig path patterns ────────────────────────────────────
...
# ── alias/workspace import targets orphaned by the full-path migration ──
...
def test_alias_import_edge_resolves_with_relative_input_paths(tmp_path, monkeypatch):
    ...
    # form) never rewrote it -> orphan -> dropped edge. Absolute/tmp_path
...
def test_alias_import_does_not_remap_an_owned_symbol_id(tmp_path, monkeypatch):
    ...
    #, while str(Path(...)) is the native spelling and so only matched
    # on POSIX hosts.
...
# --- (follow-up): alias re-exports THROUGH a barrel ---------------------
...
# ── the exports-map condition must follow the importer ────────────────
