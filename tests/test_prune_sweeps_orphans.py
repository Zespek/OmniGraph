...
On omnigraph's own package, pruning `omnigraph/callflow_html.py` removed 137 of its
139 nodes and stranded `omnigraph_callflow_html_py_path` (label `Path`) and
`omnigraph_callflow_html_py_counter` (label `Counter`) permanently.
...
from omnigraph.build import build_from_json, build_merge
