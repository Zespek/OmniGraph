"""Tests for omnigraph.transcribe — video/audio transcription support."""
...
from omnigraph.transcribe import (
    VIDEO_EXTENSIONS,
...
def test_build_whisper_prompt_env_override(monkeypatch):
    """OMNIGRAPH_WHISPER_PROMPT env var short-circuits LLM call."""
    monkeypatch.setenv("OMNIGRAPH_WHISPER_PROMPT", "Custom domain hint.")
...
def test_build_whisper_prompt_returns_topic_string():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("OMNIGRAPH_WHISPER_PROMPT", None)
...
def test_transcribe_force_reruns(tmp_path):
    ...
    with patch("omnigraph.transcribe._get_whisper", return_value=lambda *a, **kw: fake_model):
...
def test_transcribe_missing_faster_whisper(tmp_path):
    ...
    with patch("omnigraph.transcribe._get_whisper", side_effect=ImportError("faster-whisper not installed")):
...
def test_transcribe_all_skips_failed(tmp_path):
    ...
    with patch("omnigraph.transcribe.transcribe", side_effect=RuntimeError("boom")):
