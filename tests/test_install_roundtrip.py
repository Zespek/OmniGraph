...
Every platform omnigraph knows about installs its SKILL.md at a specific real
...
def test_skill_roundtrip_at_real_destination(platform, project, tmp_path, monkeypatch):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        assert (dst.parent / ".omnigraph_version").read_text() == mainmod.__version__
        ...
        assert not (dst.parent / ".omnigraph_version").exists()
...
def test_amp_user_install_at_corrected_agents_path(tmp_path, monkeypatch):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        assert dst == home / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md"
        assert dst.exists()
...
def test_amp_project_install_at_agents_path(tmp_path, monkeypatch):
    ...
    project_dir.mkdir()
    monkeypatch.chdir(project_dir)
    with patch("omnigraph.__main__.Path.home", return_value=tmp_path / "home"):
        dst = mainmod._copy_skill_file("amp", project=True, project_dir=project_dir)
        assert dst == project_dir / ".agents" / "skills" / "omnigraph" / "SKILL.md"
        assert dst.exists()
...
def test_vscode_install_uninstall_roundtrip(tmp_path, monkeypatch):
    """VS Code Copilot Chat round trip at ~/.copilot/skills/omnigraph + instructions file."""
    home = tmp_path / "home"
    project_dir = tmp_path / "proj"
    home.mkdir()
    project_dir.mkdir()
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        skill = home / ".copilot" / "skills" / "omnigraph" / "SKILL.md"
        instructions = project_dir / ".github" / "copilot-instructions.md"
        assert skill.exists()
        assert instructions.exists()
        assert "## omnigraph" in instructions.read_text(encoding="utf-8")
        ...
        # The omnigraph section is stripped from the instructions file.
        if instructions.exists():
            assert "## omnigraph" not in instructions.read_text(encoding="utf-8")
...
def test_install_entrypoint_roundtrip_for_progressive_and_monolith(tmp_path):
    ...
    """
    for platform, rel in (
        ("claude", Path(".claude") / "skills" / "omnigraph"),
        ("aider", Path(".aider") / "omnigraph"),
    ):
...
def test_monolith_to_progressive_upgrade(tmp_path, fake_progressive_bundle):
    ...
    (skill_dir / ".omnigraph_version").write_text("0.0.1", encoding="utf-8")
    assert not (skill_dir / "references").exists()
...
def test_failed_copytree_leaves_no_partial_references(tmp_path, fake_progressive_bundle):
    ...
    with patch("omnigraph.__main__.shutil.copytree", side_effect=boom):
