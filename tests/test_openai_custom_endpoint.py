...
def test_omnigraph_openai_model_wins_over_openai_model(monkeypatch):
    # model_env_key (OMNIGRAPH_OPENAI_MODEL) is resolved at call time and takes
    # precedence over the import-time OPENAI_MODEL default.
    monkeypatch.setenv("OPENAI_MODEL", "env-default-model")
    monkeypatch.setenv("OMNIGRAPH_OPENAI_MODEL", "omnigraph-override-model")
    reloaded = importlib.reload(llm)
    try:
        assert reloaded._default_model_for_backend("openai") == "omnigraph-override-model"
    finally:
        monkeypatch.undo()
        importlib.reload(llm)
