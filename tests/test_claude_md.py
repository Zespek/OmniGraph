"""Tests for omnigraph claude install / uninstall commands."""
from pathlib import Path
import pytest
from omnigraph.__main__ import claude_install, claude_uninstall, _CLAUDE_MD_MARKER, _CLAUDE_MD_SECTION
...
def test_install_contains_expected_rules(tmp_path):
    ...
    assert "omnigraph update" in content
...
def test_uninstall_removes_section(tmp_path):
    """Removes the omnigraph section after it was installed."""
    claude_install(tmp_path)
...
def test_uninstall_preserves_other_content(tmp_path):
    """Uninstall keeps pre-existing content outside the omnigraph section."""
    target = tmp_path / "CLAUDE.md"
...
def test_uninstall_no_op_when_not_installed(tmp_path, capsys):
    """Uninstall on a CLAUDE.md without omnigraph section prints a message and exits cleanly."""
    target = tmp_path / "CLAUDE.md"
...
def test_install_settings_json_idempotent(tmp_path):
    ...
    settings = json.loads(settings_path.read_text())
    hooks = settings.get("hooks", {}).get("PreToolUse", [])
    bash_hooks = [h for h in hooks if h.get("matcher") == "Bash|Grep" and "omnigraph" in str(h)]
...
def test_uninstall_preserves_other_content_in_local_md(tmp_path):
    """Uninstall keeps non-omnigraph content in CLAUDE.local.md."""
    claude_install(tmp_path)
    local_md = tmp_path / ".claude" / "CLAUDE.local.md"
