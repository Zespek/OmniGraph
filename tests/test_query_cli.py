"""Tests for omnigraph query CLI context filtering."""
...
def test_query_cli_explicit_context_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "extract", "--context", "call", "--graph", str(graph_path)],
    )
    mainmod.main()
...
def test_query_cli_heuristic_context_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "who calls extract", "--graph", str(graph_path)],
    )
    mainmod.main()
    out = capsys.readouterr().out
...
def _write_calls_graph(tmp_path):
    ...
    the standard `omnigraph extract`/`update` output shape (`"directed":
...
def test_query_cli_preserves_calls_direction_when_seeded_on_callee(monkeypatch, tmp_path, capsys):
    """`omnigraph query` must render `calls` edges caller->callee regardless of
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "callee_fn", "--graph", str(graph_path)],
    )
    mainmod.main()
    out = capsys.readouterr().out
...
def test_query_cli_preserves_calls_direction_when_seeded_on_caller(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "caller_fn", "--graph", str(graph_path)],
    )
    mainmod.main()
    out = capsys.readouterr().out
    assert "caller_fn --calls" in out
...
def test_query_cli_rejects_oversized_graph(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "extract", "--graph", str(graph_path)],
    )
