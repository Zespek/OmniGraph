...
def test_pdf_routed_through_pypdf_not_readtext(tmp_path, monkeypatch):
    ...
    import omnigraph.detect as detect
...
def test_capability_flags(monkeypatch):
    ...
    monkeypatch.delenv("OMNIGRAPH_OLLAMA_VISION", raising=False)
    assert not llm._backend_supports_vision("ollama")
    monkeypatch.setenv("OMNIGRAPH_OLLAMA_VISION", "1")
...
def test_anthropic_response_text_returns_first_text_block_not_concatenation():
    """Locks the first-wins semantics: omnigraph's claude calls return a single
...
def test_call_bedrock_honors_api_timeout(monkeypatch):
    # OMNIGRAPH_API_TIMEOUT must reach the botocore client's read_timeout; else
    ...
    monkeypatch.setenv("OMNIGRAPH_API_TIMEOUT", "1800")
