...
omnigraph-out/cache/ inside it. It now defaults to the current working directory;
...
import omnigraph.cache as cache
from omnigraph.cache import load_cached, file_hash
...
def test_default_cache_lands_in_cwd_not_source_tree(tmp_path, monkeypatch):
    ...
    # the key-root, leaving a stray omnigraph-out/ in a writable corpus).
    assert not (corpus / "omnigraph-out").exists(), (
        "cache/stat-index written into the analyzed source tree (#1774)"
    )
    assert (work / "omnigraph-out" / "cache").is_dir(), "cache should land under CWD"
...
def test_default_cache_does_not_leave_stat_index_in_source_tree(tmp_path, monkeypatch):
    ...
    assert not (corpus / "omnigraph-out").exists(), "stat-index leaked into the corpus"
    assert (work / "omnigraph-out" / "cache" / "stat-index.json").exists(), (
...
def test_explicit_cache_root_still_wins(tmp_path, monkeypatch):
    ...
    assert (out / "omnigraph-out" / "cache").is_dir()
    assert not (corpus / "omnigraph-out").exists()
    assert not (work / "omnigraph-out").exists()
