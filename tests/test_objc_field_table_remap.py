...
i.e. always via `omnigraph update <dir>`, so `[self.<field> …]` receiver
...
def test_the_in_process_remap_rewrites_the_table_keys():
    ...
    try:
        from omnigraph.extract import _remap_objc_field_tables
...
def test_cache_portability_rewrites_the_table_keys(tmp_path):
    ...
    from omnigraph.cache import _absolutize_ids_in, _relativize_ids_in
