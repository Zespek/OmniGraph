...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "omnigraph-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # omnigraph-out dir does not exist yet
    assert not (tmp_path / "omnigraph-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "omnigraph-out").is_dir()
...
def test_watched_extensions_excludes_noise():
    #.json is now indexed (bash/JSON extractors added in)
...
def test_batch_doc_only_deletion_triggers_rebuild(tmp_path):
    """#2580: deleting ONLY doc files while `omnigraph watch` runs must trigger
...
def test_doc_only_deletion_full_rebuild_evicts_md_nodes(tmp_path):
    ...
    graph_path = corpus / "omnigraph-out" / "graph.json"
...
def test_rebuild_code_reports_unclassified_files(tmp_path, capsys):
    """#3511: `omnigraph extract` has surfaced files it saw but could not
...
def test_check_update_with_flag_returns_true_and_prints(tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert "omnigraph --update" in out
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from omnigraph.watch import watch
...
def test_omnigraph_root_preserves_relative_when_invoked_with_relative_path(tmp_path, monkeypatch):
    """#777: ``.omnigraph_root`` stores the user-supplied path (``.``), not the
    resolved absolute, so a committed ``omnigraph-out/.omnigraph_root`` is
    ...
    saved = (corpus / "omnigraph-out" / ".omnigraph_root").read_text(encoding="utf-8")
    assert saved == ".", (
        f".omnigraph_root must preserve the user-supplied path; got {saved!r}"
    )


def test_rebuild_code_writes_community_name(tmp_path):
    """#1808: `omnigraph update` / _rebuild_code must forward community_labels to
    ...
    graph = json.loads((corpus / "omnigraph-out" / "graph.json").read_text(encoding="utf-8"))
...
def test_rebuild_code_keeps_a_visualization_when_over_the_viz_cap(tmp_path, monkeypatch):
    ...
    assert _rebuild_code(corpus, acquire_lock=False) is True
    html = corpus / "omnigraph-out" / "graph.html"
    ...
    monkeypatch.setenv("OMNIGRAPH_VIZ_NODE_LIMIT", str(cap))
    ...
    with monkeypatch.context() as failed_render:
        failed_render.setattr("omnigraph.paths.os.replace", fail_html_publish)
        assert _rebuild_code(corpus, acquire_lock=False) is True

    assert html.read_text(encoding="utf-8") == before, (
        "a failed aggregate publish must preserve the previous complete HTML"
    )
    assert (corpus / "omnigraph-out" / ".graph.html.stale").exists()
    ...
    assert not (corpus / "omnigraph-out" / ".graph.html.stale").exists()
    ...
    stable_paths = [
        corpus / "omnigraph-out" / "graph.json",
        corpus / "omnigraph-out" / "GRAPH_REPORT.md",
        corpus / "omnigraph-out" / ".omnigraph_labels.json",
        corpus / "omnigraph-out" / ".omnigraph_labels.json.sig",
    ]
    ...
    with monkeypatch.context() as recovery_patch:
        recovery_patch.setattr("omnigraph.cluster.cluster", fail_cluster)
        assert _rebuild_code(corpus, acquire_lock=False) is True
    ...
    monkeypatch.setenv("OMNIGRAPH_VIZ_NODE_LIMIT", "0")
    assert _rebuild_code(corpus, acquire_lock=False) is True
    assert not html.exists(), "OMNIGRAPH_VIZ_NODE_LIMIT=0 must disable the HTML viz outright"
...
def test_missing_html_recovery_preserves_multigraph_edge_counts(tmp_path, monkeypatch):
    ...
    from omnigraph.watch import _reconcile_graph_html
    ...
    monkeypatch.setenv("OMNIGRAPH_VIZ_NODE_LIMIT", "3")
    ...
    with monkeypatch.context() as failed_render:
        ...
        failed_render.setattr("omnigraph.paths.os.replace", fail_replace)
...
def test_update_rebuilds_with_nested_star_gitignore(tmp_path):
    """#1880: `omnigraph update` must not emit 0 nodes (and then refuse to
...
def test_update_discovers_newly_added_files_and_dirs(tmp_path):
    """#1837: after an initial build, a plain `omnigraph update` (full re-scan, no
    ...
    sources = {n.get("source_file", "") for n in
               json.loads((corpus / "omnigraph-out" / "graph.json").read_text())["nodes"]}
...
def test_rebuild_honors_persisted_excludes(tmp_path):
    ...
    import json
    from omnigraph.watch import _rebuild_code, _write_build_config
    ...
    _write_build_config(corpus / "omnigraph-out", excludes=["vendor"])
...
def test_rebuild_honors_persisted_no_gitignore(tmp_path):
    ...
    _write_build_config(
        corpus / "omnigraph-out", excludes=None, gitignore=False
    )

    assert _rebuild_code(corpus, no_cluster=True, acquire_lock=False) is True

    graph = json.loads((corpus / "omnigraph-out" / "graph.json").read_text())
...
def test_omnigraph_root_preserves_absolute_when_user_supplied(tmp_path):
    """When the caller supplies an absolute path, ``.omnigraph_root`` stores
...
def test_rebuild_code_deleted_cwd_without_repo_root_returns_false(tmp_path, monkeypatch, capsys):
    ...
    Without OMNIGRAPH_REPO_ROOT, the rebuild should fail cleanly before creating
    relative omnigraph-out queue/lock files.
    """
    from omnigraph.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.delenv("OMNIGRAPH_REPO_ROOT", raising=False)
...
def test_rebuild_code_deleted_cwd_uses_omnigraph_repo_root(tmp_path, monkeypatch):
    """OMNIGRAPH_REPO_ROOT lets detached hook rebuilds recover from a deleted CWD."""
    from omnigraph.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.setenv("OMNIGRAPH_REPO_ROOT", str(corpus))
    ...
    try:
        ...
        assert (corpus / "omnigraph-out" / "graph.json").exists()
    finally:
        os.chdir(old_cwd)


def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: omnigraph update (_rebuild_code with no changed_paths) must remove
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    """#1116: omnigraph update (_rebuild_code with no changed_paths) must prune a
    ...
    # shrink, so the shrink-guard must let `omnigraph update` refresh the graph
...
def test_rebuild_code_preupgrade_marker_less_node_one_cycle_lag(tmp_path):
    ...
    markers. On the first `omnigraph update` after upgrading, a symbol removed
    ...
    # Simulate a pre- graph: strip every `_origin` marker, then inject a
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    from omnigraph import cluster as cluster_mod
    from omnigraph.watch import _rebuild_code
    ...
    graph_path = tmp_path / "omnigraph-out" / "graph.json"
    report_path = tmp_path / "omnigraph-out" / "GRAPH_REPORT.md"
...
# --- .omnigraphignore honored in watch handler (gh-928) ---
...
def test_watch_handler_honors_omnigraphignore(tmp_path, monkeypatch):
    """gh-928: the watch Handler must short-circuit paths matching
    .omnigraphignore so busy volumes (node_modules churn, build artefacts,
    Time Machine writes, …) don't wake the rebuild pipeline.
    """
    import threading
    from omnigraph import watch as watch_mod

    watch_root = tmp_path / ".hidden-parent" / "corpus"
    watch_root.mkdir(parents=True)
    (watch_root / ".omnigraphignore").write_text("node_modules/\nbuild/\n", encoding="utf-8")
...
@pytest.mark.skipif(not _watchdog_available(), reason="watchdog not installed")
def test_watch_loads_omnigraphignore_once(tmp_path, monkeypatch):
    """gh-928: .omnigraphignore must be parsed exactly once at watch() startup,
    ...
    (tmp_path / ".omnigraphignore").write_text("ignored/\n", encoding="utf-8")
    (tmp_path / "ignored").mkdir()

    calls = {"n": 0}
    real_loader = detect_mod._load_omnigraphignore
    ...
    monkeypatch.setattr(watch_mod, "_load_omnigraphignore", counting_loader)
    ...
    time.sleep(0.7)
    assert calls["n"] == 1, f"_load_omnigraphignore called {calls['n']} times; expected 1"
...
def test_rebuild_code_accepts_repo_relative_changed_path_for_subdir_root(tmp_path):
    ...
    try:
        ...
        graph_path = src / "omnigraph-out" / "graph.json"
        before = json.loads(graph_path.read_text(encoding="utf-8"))
...
def test_rebuild_code_does_not_update_root_marker_when_write_is_refused(tmp_path, monkeypatch):
    ...
    try:
        ...
        marker = src / "omnigraph-out" / ".omnigraph_root"
...
def test_rebuild_code_incremental_rename_preserves_symlink_source_path(tmp_path):
    ...
    real = corpus / "real"
    real.mkdir()
    (corpus / ".omnigraphignore").write_text("real/\n", encoding="utf-8")
    old = real / "old.py"
...
# --- pending-changes queue prevents commit drops under lock contention ---
...
def test_queue_and_drain_pending_round_trip(tmp_path):
    ...
    from omnigraph.watch import _queue_pending, _drain_pending, _PENDING_FILENAME
...
def test_drain_pending_dedupes_and_skips_blank_lines(tmp_path):
    ...
    from omnigraph.watch import _queue_pending, _drain_pending
...
def test_queue_pending_noop_on_empty_list(tmp_path):
    ...
    from omnigraph.watch import _queue_pending, _PENDING_FILENAME
...
def test_rebuild_code_queues_on_lock_contention(tmp_path, monkeypatch, capsys):
    ...
    from omnigraph.watch import _rebuild_code, _rebuild_lock, _PENDING_FILENAME
...
def test_rebuild_code_drains_late_arrivals(tmp_path, monkeypatch):
    ...
    from omnigraph import watch as watch_mod
    from omnigraph.watch import _rebuild_code as orig_rebuild
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from omnigraph.watch import _merge_changed_paths
...
def test_rebuild_code_preserves_nodes_from_excluded_but_alive_file(tmp_path, capsys):
    ...
    (#2495): only .omnigraphignore/--exclude matches evict on the hook path.
    """
    import json
    from omnigraph.watch import _rebuild_code
...
# ── `omnigraph update` purges newly-ignored files that still exist ───────


def test_update_prunes_newly_omnigraphignored_file(tmp_path, monkeypatch, capsys):
    """#2495: a file added to .omnigraphignore while still on disk must be purged
    from the graph by a full `omnigraph update` — nodes, edges, and hyperedges —
    instead of being preserved forever by the fail-closed keep. The legitimate
    shrink passes without --force/OMNIGRAPH_FORCE, unchanged in-corpus files stay
    byte-identical, and the prune is reported distinctly from the fail-closed
    keep line."""
    from omnigraph.watch import _rebuild_code

    monkeypatch.delenv("OMNIGRAPH_FORCE", raising=False)
    corpus = tmp_path / "corpus"
    corpus.mkdir()
    ...
    (corpus / ".omnigraphignore").write_text("b.py\n", encoding="utf-8")
    capsys.readouterr()
...
def test_update_still_prunes_deleted_file_with_ignore_rules_present(tmp_path):
    ...
    corpus = tmp_path / "corpus"
    corpus.mkdir()
    (corpus / ".omnigraphignore").write_text("unrelated/\n", encoding="utf-8")
    (corpus / "a.py").write_text("def alpha():\n    return 1\n", encoding="utf-8")
    (corpus / "b.py").write_text("def helper():\n    return 2\n", encoding="utf-8")
...
def test_update_evicts_semantic_nodes_from_newly_ignored_non_ast_source(tmp_path, capsys):
    ...
    (corpus / ".omnigraphignore").write_text("notes.txt\n", encoding="utf-8")
    capsys.readouterr()
...
def test_gitignore_eviction_gated_on_full_rebuild(tmp_path, capsys):
    ...
    `omnigraph update` honors it and purges the file."""
    from omnigraph.watch import _rebuild_code
...
def test_omnigraphignore_eviction_applies_to_incremental_rebuilds(tmp_path, capsys):
    """#2495 policy split, other half: a .omnigraphignore match is unambiguous
    ...
    (corpus / ".omnigraphignore").write_text("vendor.py\n", encoding="utf-8")
    capsys.readouterr()
    ...
    assert "vendor.py" not in sources, (
        ".omnigraphignore match must evict on an incremental rebuild too (#2495)"
    )
    assert "app.py" in sources
    assert "newly-ignored file(s)" in capsys.readouterr().out


# --- semantic-backed docs must not be double-represented by the AST quick-scan ---
...
def _seed_semantic_doc_graph(corpus):
    ...
    Mimics a graph produced by the CLI ``omnigraph . --update`` path: code AST
...
# ── code-typed semantic nodes count as a doc's semantic layer ───────────
...
# ── deleted non-AST sources (docs/papers/images) get evicted ────────────

def test_rebuild_code_evicts_semantic_nodes_from_deleted_non_ast_source(tmp_path):
    """#2051: a full `omnigraph update` must evict semantic nodes whose non-AST
...
def test_rebuild_code_preserves_remote_source_across_repeated_updates(tmp_path):
    ...
    (gdoc://, s3://, http://) must survive REPEATED `omnigraph update`s. Path
    ...
    collapse."""
    from omnigraph.watch import _rebuild_code, _is_remote_source
...
# ── present-but-unextractable files in a change set are not deletions ───
...
# --- fail-closed load of the existing graph ---
...
def test_rebuild_refuses_overwrite_when_existing_graph_over_size_cap(
    ...
    """#2251: an existing graph.json over OMNIGRAPH_MAX_GRAPH_BYTES could not be
    ...
    monkeypatch.setenv("OMNIGRAPH_MAX_GRAPH_BYTES", "100")
    assert _rebuild_code(
...
# --- `omnigraph update` rebuild must inherit the on-disk directed flag ---

def test_rebuild_code_inherits_directed_flag_clustered(tmp_path):
    """#2342: the clustered `omnigraph update` rebuild path built the graph via
    ...
    assert after.get("directed") is True, (
        "omnigraph update (clustered) must preserve an existing directed=True graph"
    )
    edge = next(
...
def test_rebuild_code_inherits_directed_flag_no_cluster(tmp_path):
    ...
    assert after.get("directed") is True, (
        "omnigraph update --no-cluster must preserve an existing directed=True graph"
    )
...
# --- incremental shared-direct-call resolution helpers -----------------

def _2406_graph(corpus):
    import json
    return json.loads(
        (corpus / "omnigraph-out" / "graph.json").read_text(encoding="utf-8")
    )
...
def test_incremental_rebuild_does_not_reparse_unchanged_targets(tmp_path, monkeypatch):
    ...
    import omnigraph.extract as extract_mod
    from omnigraph.watch import _rebuild_code
...
# --- / member + indirect calls into unchanged files -------------
# The resolution context now also carries the unchanged corpus's
# contains/method edges (member-call resolvers) and the persisted
# `_callable`/`_callable_class` markers (indirect_call guard), so both
...
def test_incremental_rebuild_legacy_graph_without_callable_markers(tmp_path):
    ...
    # Simulate a pre- graph: strip the persisted callability markers.
...
def test_read_only_events_are_ignored():
    ...
    from omnigraph.watch import _is_read_only_event
...
def test_relativize_source_files_relativizes_definition_file(tmp_path):
    ...
    from omnigraph.watch import _relativize_source_files
...
def test_rebase_relative_source_files_rebases_definition_file(tmp_path):
    ...
    from omnigraph.watch import _rebase_relative_source_files
