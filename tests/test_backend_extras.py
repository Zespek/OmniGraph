...
Friction this guards: `uv tool install omnigraph` puts omnigraph in an isolated
...
from omnigraph.llm import _backend_pkg_hint
...
def test_backend_pkg_hint_points_at_uv_tool_and_extra():
    ...
    assert 'omnigraph[anthropic]' in msg
...
def test_all_extra_is_the_union_of_every_other_extra():
    ...
    an `[all]` install still failed on `omnigraph extract --postgres` (#3482)."""
    extras = _extras()
