"""Rigorous edge-case coverage for the `omnigraph hook-guard` subcommand (#522).
...
from omnigraph import __main__ as m


# --------------------------------------------------------------------------- #
# Direct-call harness: hermetic w.r.t. the ambient OMNIGRAPH_OUT env.
# --------------------------------------------------------------------------- #
def _invoke(kind, payload, tmp_path, monkeypatch, *, graph=True, out_name="omnigraph-out"):
    monkeypatch.setattr("omnigraph.paths.OMNIGRAPH_OUT", out_name)
    monkeypatch.setattr("omnigraph.paths.OMNIGRAPH_OUT_NAME", out_name)
...
@pytest.mark.parametrize("command", [
    ...
    # nudged on it only because the string contains 'grep'; the graph
...
def test_read_nudges(tool_input, tmp_path, monkeypatch):
    ...
    assert "omnigraph query" in out, f"{tool_input!r} should nudge"
...
@pytest.mark.parametrize("tool_input", [
    ...
    {"file_path": "omnigraph-out/GRAPH_REPORT.md"},  # the graph's own output
    {"file_path": ""},
    {},                                          # nothing at all
])
...
def test_read_respects_custom_output_dir_name(tmp_path, monkeypatch):
    ...
    # not just the literal 'omnigraph-out/'.
...
def test_search_out_path_error_is_swallowed(tmp_path, monkeypatch):
    ...
    def _boom(*a, **k):
        raise OSError("boom")
    monkeypatch.setattr("omnigraph.paths.out_path", _boom)
...
def test_gemini_allow_with_nudge(tmp_path, monkeypatch):
    ...
    assert "omnigraph query" in payload["additionalContext"]
...
def _cli(args, tmp_path, stdin=""):
    return subprocess.run(
        [sys.executable, "-m", "omnigraph", *args],
...
def test_read_nudge_em_dash_survives_utf8(tmp_path):
    ...
    (tmp_path / "omnigraph-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-guard", "read"],
