...
from omnigraph.detect import classify_file, count_words, detect, detect_incremental, save_manifest, FileType, _looks_like_paper, _is_ignored, _load_omnigraphignore, _is_sensitive
from omnigraph import detect as detect_mod
...
def test_classify_powershell_module():
    #.psm1 modules were never indexed (CODE_EXTENSIONS gap).
    assert classify_file(Path("Utils.psm1")) == FileType.CODE

def test_classify_powershell_manifest():
    #.psd1 manifests must be classified as CODE so the manifest extractor runs.
...
def test_detect_skips_noise_dot_dirs():
    """Noise dot dirs (.next, .nuxt, .omnigraph cache, …) are skipped (#873).
    Non-noise dot dirs (.github, .claude, …) are now allowed through."""
    result = detect(FIXTURES)
    for files in result["files"].values():
        for f in files:
            # omnigraph's own cache is always skipped
            assert "/.omnigraph/" not in f
...
def test_classify_attention_paper():
    ...
    paper_path = Path("/home/zespek/omnigraph_eval/papers/attention_is_all_you_need.md")
    if paper_path.exists():
        result = classify_file(paper_path)
        assert result == FileType.PAPER


def test_omnigraphignore_excludes_file(tmp_path):
    """Files matching .omnigraphignore patterns are excluded from detect()."""
    (tmp_path / ".omnigraphignore").write_text("vendor/\n*.generated.py\n")
    ...
    assert result["omnigraphignore_patterns"] == 2


def test_omnigraphignore_matches_nfd_path_with_nfc_pattern(tmp_path):
    ...
    (tmp_path / ".omnigraphignore").write_text(f"{nfc_name}/\n", encoding="utf-8")
...
def test_omnigraphignore_matches_nfc_path_with_nfd_pattern(tmp_path):
    """The reverse direction also holds: NFD pattern, NFC path on disk."""
    nfc_name = unicodedata.normalize("NFC", "Or\u00e7amento")
    nfd_name = unicodedata.normalize("NFD", nfc_name)

    (tmp_path / ".omnigraphignore").write_text(f"{nfd_name}/\n", encoding="utf-8")
    d = tmp_path / nfc_name
    d.mkdir()
...
def test_omnigraphignore_ascii_patterns_unaffected(tmp_path):
...
def test_omnigraphignore_missing_is_fine(tmp_path):
    """No .omnigraphignore is not an error."""
    (tmp_path / "main.py").write_text("x = 1")
    result = detect(tmp_path)
    assert result["omnigraphignore_patterns"] == 0


def test_omnigraphignore_comments_ignored(tmp_path):
    """Comment lines in .omnigraphignore are not treated as patterns."""
    (tmp_path / ".omnigraphignore").write_text("# this is a comment\n\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_omnigraphignore_utf8_bom_first_pattern_honored(tmp_path):
    """A UTF-8 BOM at the start of .omnigraphignore must not corrupt the first
    pattern (#2163): git strips a single leading BOM, so `*.log` on line 1
    must still exclude app.log."""
    (tmp_path / ".omnigraphignore").write_bytes(b"\xef\xbb\xbf*.log\nbuild/\n")
...
def test_omnigraphignore_bom_only_file(tmp_path):
    """A .omnigraphignore containing only a BOM yields zero patterns, not one
    bogus U+FEFF pattern (#2163)."""
    (tmp_path / ".omnigraphignore").write_bytes(b"\xef\xbb\xbf")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_omnigraphignore_bom_then_comment(tmp_path):
    """A BOM followed by a comment must still parse as a comment, not become
    a `\\ufeff# comment` pattern (#2163)."""
    (tmp_path / ".omnigraphignore").write_bytes(b"\xef\xbb\xbf# comment\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("main.py" in f for f in result["files"]["code"])
    assert any("other.py" in f for f in result["files"]["code"])
    assert result["omnigraphignore_patterns"] == 1, "BOM'd comment became a pattern"
...
def test_git_info_exclude_utf8_bom(tmp_path):
    ...
    pattern either (#2163) — second read site in _load_omnigraphignore."""
...
def test_omnigraphignore_hermetic_without_vcs(tmp_path):
    """Without a VCS root, parent .omnigraphignore does NOT apply (hermetic)."""
    (tmp_path / ".omnigraphignore").write_text("vendor/\n")
    ...
    # parent .omnigraphignore must NOT leak into a non-VCS scan
    assert any("vendor" in f for f in code_files)
    assert result["omnigraphignore_patterns"] == 0


def test_omnigraphignore_discovered_from_parent_in_vcs(tmp_path):
    """Inside a VCS repo, parent .omnigraphignore applies to subdirectory scans."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".omnigraphignore").write_text("vendor/\n")
    sub = tmp_path / "packages" / "mylib"
    sub.mkdir(parents=True)
    ...
    assert result["omnigraphignore_patterns"] >= 1


def test_omnigraphignore_stops_at_git_boundary(tmp_path):
    """Upward search stops at the git repo root (.git directory)."""
    (tmp_path / ".omnigraphignore").write_text("main.py\n")
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    sub = repo / "sub"
...
def test_omnigraphignore_at_git_root_is_included(tmp_path):
    """A .omnigraphignore at the git repo root is included when scanning a subdir."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".omnigraphignore").write_text("vendor/\n")
...
def test_omnigraphignore_still_excludes_git_tracked_file(tmp_path):
    ...
    (tmp_path / ".omnigraphignore").write_text("storage/\n", encoding="utf-8")
...
def test_detect_incremental_propagates_follow_symlinks(requires_symlinks, tmp_path, monkeypatch):
    ...
    # Store manifest inside omnigraph-out/ so it is pruned by _SKIP_DIRS
    # and doesn't get re-detected as a code file now that .json is indexed.
    manifest_dir = tmp_path / "omnigraph-out"
    manifest_dir.mkdir()
...
def test_classify_video_extensions():
    ...
    from omnigraph.detect import FileType
...
def test_detect_converts_google_workspace_shortcuts_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.detect.convert_google_workspace_file", fake_convert)
...
def test_detect_office_sidecar_survives_a_gitignored_output_dir(tmp_path, monkeypatch):
    """#3504: the documented .gitignore advice puts omnigraph-out/ (and so
    omnigraph-out/converted/, where Office sidecars land) inside a gitignored
    ...
    follows that advice."""
    (tmp_path / ".gitignore").write_text("omnigraph-out/\n", encoding="utf-8")
    ...
    monkeypatch.setattr("omnigraph.detect.convert_office_file", fake_convert)
...
def test_collect_files_keeps_coverage_code_namespace(tmp_path):
    ...
    from omnigraph.extract import collect_files
...
# --- dot dirs allowed, framework caches blocked ---
...
def test_detect_honors_git_info_exclude(tmp_path):
    ...
    .omnigraphignore — otherwise nested worktree copies get fully indexed (#1810)."""
    (tmp_path / ".git" / "info").mkdir(parents=True)
...
def test_git_info_exclude_ranks_below_gitignore_negation(tmp_path):
    ...
    from omnigraph.detect import _load_omnigraphignore, _is_ignored
    (tmp_path / ".git" / "info").mkdir(parents=True)
    ...
    patterns = _load_omnigraphignore(tmp_path)
...
def test_detect_skips_omnigraph_own_cache(tmp_path):
    """.omnigraph/ (extraction cache) must never be re-indexed as source (#873)."""
    cache = tmp_path / ".omnigraph" / "cache"
    cache.mkdir(parents=True)
    (cache / "abc123.json").write_text('{"nodes": [], "edges": []}')
    (tmp_path / "app.py").write_text("def go(): pass")
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert not any(".omnigraph" in f for f in all_files)
    assert any("app.py" in f for f in all_files)


# --- gitignore parent-exclusion rule for ! re-includes ---
...
def test_anchored_root_wildcard_negation_reincludes_subtree(tmp_path):
    ...
    for rel in ("src/app/main.py", "src/lib/util.py", "docs/guide.md", "README.md"):
        ...
        path.write_text("x\n")
    (tmp_path / ".omnigraphignore").write_text("/*\n!/src/\n")
...
def test_path_pattern_single_star_does_not_cross_segment(tmp_path):
    ...
    for pattern in ("/src/*.py", "src/*.py"):
        (tmp_path / ".omnigraphignore").write_text(f"{pattern}\n")
        result = detect(tmp_path)
        files = as_posix_list(
...
def test_anchored_double_star_crosses_path_segments(tmp_path):
    ...
    direct.write_text("x\n")
    nested.write_text("x\n")
    (tmp_path / ".omnigraphignore").write_text("/src/**/generated.py\n")
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    from omnigraph.detect import _is_ignored, _load_omnigraphignore
    android = tmp_path / "android" / "app" / "src"
    android.mkdir(parents=True)
    victim = android / "Main.kt"
    victim.write_text("fun main() {}")
    (tmp_path / ".omnigraphignore").write_text("android/\n!src/\n")
    patterns = _load_omnigraphignore(tmp_path)
...
def test_negation_works_when_no_ancestor_excluded(tmp_path):
    ...
    keep = src / "keep.py"
    keep.write_text("x = 1")
    (tmp_path / ".omnigraphignore").write_text("*.py\n!src/keep.py\n")
    patterns = _load_omnigraphignore(tmp_path)
...
def test_negation_does_not_disable_directory_pruning(tmp_path, monkeypatch):
    ...
    """
    import os
    import omnigraph.detect as det

    (tmp_path / ".omnigraphignore").write_text("myignored/\n*.md\n!docs/**\n")
...
# Regression tests for - anchored patterns must not match basename deep in tree
...
def test_anchored_dir_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".omnigraphignore").write_text("/inbox/\n")
    patterns = _load_omnigraphignore(tmp_path)
    assert not _is_ignored(f, tmp_path, patterns), (
...
def test_anchored_file_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".omnigraphignore").write_text("/build\n")
    patterns = _load_omnigraphignore(tmp_path)
...
def test_anchored_multi_segment_pattern(tmp_path):
    ...
    (tmp_path / ".omnigraphignore").write_text("/src/inbox/\n")
    patterns = _load_omnigraphignore(tmp_path)
...
def test_detect_does_not_ignore_scan_root_itself_via_parent_gitignore(tmp_path):
    ...
    corpus_dir = tmp_path / "omnigraph-corpus"
    corpus_dir.mkdir()
    ...
    (tmp_path / ".gitignore").write_text("omnigraph-corpus/\n", encoding="utf-8")
...
# Tests for - memoise _is_ignored/_eval results via a per-detect() cache
...
def test_is_ignored_cache_evaluates_each_dir_once():
    ...
    """
    from omnigraph.detect import _is_ignored
...
# a 76k-file vault with a 29k-file directory listed in .omnigraphignore pinned a
...
def test_ignored_dir_pruned_walk_never_lists_contents(tmp_path, monkeypatch):
    ...
    a regenerated notes/ dir listed in .omnigraphignore.
    """
    import omnigraph.detect as det

    (tmp_path / ".omnigraphignore").write_text("omnigraph-notes/\n")
    for d in range(5):
        sub = tmp_path / "omnigraph-notes" / f"note-dir-{d}"
        sub.mkdir(parents=True)
        for f in range(4):
    ...
    assert not any("omnigraph-notes" in Path(v).parts for v in visited), (
    ...
    assert not any("omnigraph-notes" in p for p in all_files)
    ...
    assert sum("omnigraph-notes" in p for p in ignored) == 1
...
def test_scan_ignore_cost_is_per_directory_not_per_file(tmp_path, monkeypatch):
    ...
    for d in range(n_ignored_dirs):
        sub = tmp_path / "omnigraph-notes" / f"note-dir-{d:03d}"
        sub.mkdir(parents=True)
    ...
    def counting_scan(path, *args, **kwargs):
        scan_calls["total"] += 1
        if "omnigraph-notes" in path.parts:
...
# Regression tests for - sensitive pattern misses underscore-prefixed names
...
# ── Stage 1 dir check gets the same source carve-out as Stage 3 ──
...
def test_sensitive_does_not_flag_source_under_secrets_dir():
    # exact cases: real source under ambiguous dir names survives.
...
def test_sensitive_still_flags_data_under_secrets_dir():
    # guard: the carve-out is ONLY for real source — data/config files
...
def test_sensitive_flags_everything_under_credential_store_dirs():
    # dedicated stores get no carve-out — even source-classified files
...
def test_sensitive_dir_carveout_does_not_bypass_name_screens():
    # rescued source still falls through to Stages 2-3, so a NON-source
    ...
    # `service_account.py` is real source (e.g. Google's oauth2 lib), not a
...
def test_sensitive_dir_carveout_still_drops_tfvars_values_store():
    # follow-up: genuine source under secrets/ is rescued, but.tfvars is
...
# ── Issue failed-chunk files must not be frozen in manifest ─────────────
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from omnigraph.cache import save_cached
...
def test_save_manifest_clear_semantic_erases_stale_hash_for_omitted_file(tmp_path):
    ...
    doc.parent.mkdir()
    doc.write_text("# Doc\n\ncontent")
    manifest_path = str(tmp_path / "omnigraph-out" / "manifest.json")
...
def test_save_manifest_clear_ast_blanks_both_hashes_for_failed_extra(tmp_path):
    ...
    the next extract re-queues the file without deleting omnigraph-out/."""
    import json
...
# Regression tests for -.gitignore fallback when no.omnigraphignore exists

def test_gitignore_fallback_when_no_omnigraphignore(tmp_path):
    """When no .omnigraphignore exists, .gitignore patterns are honored (#945)."""
    (tmp_path / ".git").mkdir()
...
def test_omnigraphignore_and_gitignore_are_merged(tmp_path):
    """When both exist, their patterns are MERGED — a file excluded only by
    .gitignore stays excluded even though .omnigraphignore says nothing about it
    (#1363). Previously the presence of a .omnigraphignore silently disabled the
    ...
    (tmp_path / ".omnigraphignore").write_text("other.py\n")  # says nothing about main.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("other.py" in f for f in code)  # omnigraphignore applied
    assert any("keep.py" in f for f in code)       # neither excludes it


def test_omnigraphignore_negation_overrides_gitignore(tmp_path):
    """.omnigraphignore is evaluated after .gitignore, so a `!` negation in it can
    re-include a file the .gitignore excluded (last-match-wins, #1363)."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".gitignore").write_text("*.py\n")           # exclude all .py
    (tmp_path / ".omnigraphignore").write_text("!keep.py\n")  # but rescue keep.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "keep.py").write_text("x = 2")

    result = detect(tmp_path)
    code = result["files"]["code"]
    assert any("keep.py" in f for f in code)      # rescued by omnigraphignore negation
    assert not any("main.py" in f for f in code)  # still excluded


# Regression tests for -.worktrees/ skipped and --exclude flag
...
# relative keys so a committed ``omnigraph-out/`` round-trips across machines
...
def test_save_manifest_relativizes_keys_when_root_given(tmp_path):
    ...
    import json
    from omnigraph.detect import save_manifest, load_manifest
...
def test_load_manifest_absolutizes_relative_keys(tmp_path):
    ...
    import json
    from omnigraph.detect import load_manifest

    manifest_path = tmp_path / "omnigraph-out" / "manifest.json"
...
def test_detect_incremental_portable_across_paths(tmp_path):
    ...
    import json
    from omnigraph.detect import save_manifest, detect_incremental
    ...
    manifest_a = str(repo_a / "omnigraph-out" / "manifest.json")
    files = {
    ...
    (repo_b / "omnigraph-out").mkdir()
    manifest_b = repo_b / "omnigraph-out" / "manifest.json"
...
def test_convert_office_file_sidecar_name_stable_across_checkouts(tmp_path, monkeypatch):
    ...
    produces the same sidecar name (no unbounded duplicates when omnigraph-out/ is
    ...
    def _sidecar(root):
        ...
        out_dir = root / "omnigraph-out" / "converted"
    ...
    fallback = detect_mod.convert_office_file(
        checkout_a / "docs" / "report.xlsx", checkout_a / "omnigraph-out" / "converted"
    )
...
def test_convert_office_file_outside_root_falls_back(tmp_path, monkeypatch):
    ...
    root = tmp_path / "repo"
    (root / "omnigraph-out" / "converted").mkdir(parents=True)
...
def test_detect_office_conversion_respects_cache_root(tmp_path, monkeypatch):
    ...
    cache_root/OMNIGRAPH_OUT/converted, leaving the scanned corpus untouched, while
    ...
    assert not (corpus / detect_mod.OMNIGRAPH_OUT).exists(), (
        "detect() must not write omnigraph-out into the scanned corpus tree when cache_root is provided (#2787)"
    )

    # 2. Converted sidecar must exist under cache_root
    converted_dir = cache_out / detect_mod.OMNIGRAPH_OUT / "converted"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("omnigraph-out/nlp", False, None, id="default-parent"),
...
def test_nested_omnigraph_out_prunes_only_configured_path(
    ...
    probe = subprocess.run(
        ...
        env={**os.environ, "OMNIGRAPH_OUT": configured_out},
        check=True,
        capture_output=True,
        text=True,
    )
...
def test_detect_records_unclassified_extensionless_files(tmp_path):
    # extensionless, non-shebang project files (Dockerfile, Makefile,...)
...
def test_omnigraphinclude_is_inert_and_not_unclassified(tmp_path, capsys):
    """#2112: .omnigraphinclude support was removed (dead since #873).

    A leftover .omnigraphinclude must not error, must not surface in the
    ...
    (tmp_path / ".omnigraphinclude").write_text(".github/\ndocs/**\n")
    result = detect(tmp_path)

    # not surfaced as an unclassified scan input
    assert not any(".omnigraphinclude" in p for p in result["unclassified"])
    # real files are indexed exactly as before; the file changes nothing
    assert result["files"] == baseline["files"]
    assert any("main.py" in f for f in result["files"]["code"])
    # one-time stderr note, matching the [omnigraph] warning convention
    err = capsys.readouterr().err
    assert err.count("[omnigraph] WARNING: .omnigraphinclude is no longer supported") == 1
...
# ---------------------------------------------------------------------------
# manifest must not retain scan-excluded files as permanent
# "deleted" entries. Full-scan saves prune excluded-but-alive rows; subset
# saves keep preserving untouched rows; out-of-root rows never prune.
# ---------------------------------------------------------------------------
...
# ── manifest seen timestamps preserved for unchanged entries ──
...
def test_save_manifest_noop_skips_disk_write(tmp_path):
    ...
    a = tmp_path / "a.py"
    a.write_text("x = 1\n", encoding="utf-8")
    manifest_path = Path(tmp_path / "omnigraph-out" / "manifest.json")
...
def test_save_manifest_ast_kind_noop_then_change(tmp_path):
    """#2838's literal path: `omnigraph update` calls save_manifest with kind='ast'.
...
# ── sensitive-filter over-match (prose/source rescued, real secrets kept) ──
...
def test_sensitive_filter_indexes_topic_prose_and_source(path):
    from omnigraph.detect import _is_sensitive
...
@pytest.mark.parametrize("path", [
    ...
    ".npmrc", ".pypirc", "secring.gpg", ".git-credentials",   # newly-caught
    "Secrets/creds.json", "SECRETS/db.json", "ID_RSA",        # case variants
...
# ── / committed dotenv templates (.env.example etc.) are graphable ──
...
def test_lexical_relative_matches_pathlib_relative_to():
    ...
    from pathlib import Path
    from omnigraph.detect import _lexical_relative, _nfc
...
def test_globstar_matcher_leaves_no_reference_cycle():
    ...
    import gc
    from omnigraph.detect import _match_anchored_ignore_pattern
