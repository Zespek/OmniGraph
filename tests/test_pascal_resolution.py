...
file's own scope. omnigraph.pascal_resolution closes that gap as a
...
from omnigraph.extract import extract, extract_pascal
...
def test_calls_resolve_across_files_via_inherits_chain(tmp_path):
    # cache_root only controls where omnigraph-out/cache/ is written -- it has
...
def test_pascal_resolver_registered():
    from omnigraph.resolver_registry import registered_resolvers
