...
- `omnigraph uninstall --project` for codebuddy no longer nukes the global skill
"""
...
def _plant_skill_tree(root: Path, dot_dir: str) -> Path:
    """Create <root>/<dot_dir>/skills/omnigraph/{SKILL.md, references/x.md, .omnigraph_version}."""
    skill_dir = root / dot_dir / "skills" / "omnigraph"
    (skill_dir / "references").mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("# omnigraph skill\n", encoding="utf-8")
    (skill_dir / "references" / "x.md").write_text("ref\n", encoding="utf-8")
    (skill_dir / ".omnigraph_version").write_text("0.0.0-test", encoding="utf-8")
    return skill_dir
...
def test_project_dir_call_never_touches_global(uninstall_fn, platform, dot_dir, tmp_path):
    ...
    assert (global_tree / ".omnigraph_version").exists()
...
def test_project_uninstall_codebuddy_spares_global(tmp_path):
    """`omnigraph uninstall --project` (codebuddy branch) must not delete ~/.codebuddy (#2215)."""
