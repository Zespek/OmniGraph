...
Both callers now delegate to :mod:`omnigraph.ids`, so they share one
...
from omnigraph.build import _normalize_id
from omnigraph.extract import _make_id
from omnigraph.ids import make_id, normalize_id
...
# the contract in. Kept separate from CONTRACT_CASES because a few of them
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as omnigraph.ids.normalize_id."""
    # build._normalize_id is imported directly from omnigraph.ids.
    ...
    # of truth" leaks back into copy-pasted forks.
    from omnigraph.mcp_ingest import _make_id as _mcp_make_id
    from omnigraph.symbol_resolution import _bash_make_id
...
# The plain st.text() property above already existed when shipped, and did
