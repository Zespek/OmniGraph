"""`omnigraph global add` repo-tag inference, and addressing the tag it produces.

The tag was `source.parent.parent.name`, which is empty whenever the graph is not two
levels below a named directory — `omnigraph global add /tmp/graph.json`, or any relative
path such as `omnigraph-out/graph.json`. The empty tag then prefixes every node with
...
def store(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.global_graph._GLOBAL_DIR", where)
    monkeypatch.setattr("omnigraph.global_graph._GLOBAL_GRAPH", where / "global-graph.json")
    monkeypatch.setattr("omnigraph.global_graph._GLOBAL_MANIFEST", where / "global-manifest.json")
    return where
...
def test_bare_graph_path_gets_a_non_empty_tag(monkeypatch, tmp_path, store):
    ...
    assert _run(monkeypatch, ["omnigraph", "global", "add", "graph.json"]) == 0
...
def test_relative_path_infers_from_the_resolved_repo_dir(monkeypatch, tmp_path, store):
    repo = tmp_path / "myrepo"
    _write_graph(repo / "omnigraph-out" / "graph.json")
    monkeypatch.chdir(repo)

    assert _run(monkeypatch, ["omnigraph", "global", "add", "omnigraph-out/graph.json"]) == 0
...
def test_explicit_as_tag_still_wins(monkeypatch, tmp_path, store):
    ...
    assert _run(monkeypatch, ["omnigraph", "global", "add", str(graph), "--as", "chosen"]) == 0

    assert list(_manifest(store)) == ["chosen"]


def test_remove_without_a_tag_is_still_a_usage_error(monkeypatch, store):
    assert _run(monkeypatch, ["omnigraph", "global", "remove"]) == 1
...
def test_remove_can_address_a_repo_registered_under_an_empty_tag(monkeypatch, tmp_path, store):
    ...
    assert _run(monkeypatch, ["omnigraph", "global", "remove", ""]) == 0
