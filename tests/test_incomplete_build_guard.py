"""Tests for the incomplete-build shrink-guard on `omnigraph extract`.
...
def _seed_to_json_recorder(monkeypatch, *, returns=True):
    ...
    monkeypatch.setattr("omnigraph.export.to_json", _stub)
    return rec
...
def _arm_extract(monkeypatch, tmp_path, *, chunk_total, chunk_succeeded, extra_argv=()):
    ...
    monkeypatch.setattr("omnigraph.llm.extract_corpus_parallel", _stub_corpus)
...
def test_partial_extraction_refuses_to_shrink_existing_graph(monkeypatch, tmp_path, capsys):
    ...
    assert not (out_dir / "omnigraph-out" / "manifest.json").exists()
...
def _arm_no_cluster(monkeypatch, tmp_path, *, extra_argv=()):
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["omnigraph", "extract", str(corpus), "--backend", "claude", "--no-cluster",
         "--out", str(out_dir), *extra_argv],
    )
...
def test_no_cluster_incomplete_build_refuses_to_shrink(tmp_path, monkeypatch, capsys):
    ...
    # first — — so a partial run no longer shrinks there; see below.)
    import json
