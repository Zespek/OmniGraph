"""#1688 - omnigraph's shared-file section update must not destroy user content.

_replace_or_append_section used to locate its marker (`## omnigraph`) as a
...
from omnigraph.__main__ import _replace_or_append_section

MARKER = "## omnigraph"
NEW = "## omnigraph\n\nThis project has a knowledge graph at omnigraph-out/.\n"
...
def test_inline_reference_to_marker_is_not_treated_as_the_section():
    before = (
        "# My Project\n\n"
        "## Setup\n"
        "- See the `## omnigraph` section for graph usage.\n\n"
        "## Release Process\n"
        "Critical steps that must not be lost.\n"
    )
    after = _replace_or_append_section(before, MARKER, NEW)
    assert "See the `## omnigraph` section" in after       # bullet preserved
    assert "Critical steps that must not be lost" in after  # later section preserved
    assert "knowledge graph at omnigraph-out/" in after      # section still added
...
def test_real_section_is_replaced_in_place():
    ...
    assert "knowledge graph at omnigraph-out/" in after
