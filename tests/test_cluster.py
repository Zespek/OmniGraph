...
from omnigraph.cluster import cluster, cohesion_score, remap_communities_to_previous, score_all
...
def test_cluster_does_not_write_to_stderr(capsys):
    ...
    cluster(G)
    captured = capsys.readouterr()
    # Allow logging output (starts with [omnigraph]) but no raw ANSI codes
...
def test_native_leiden_matches_graspologic_wrapper(monkeypatch):
    ...
    import omnigraph.cluster as cl
...
def test_partition_is_invariant_to_edge_endpoint_orientation():
    ...
    from omnigraph.cluster import _partition
