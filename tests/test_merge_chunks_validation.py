"""Tests that `omnigraph merge-chunks` validates untrusted subagent chunk JSON.

merge-chunks concatenates agent-written `.omnigraph_chunk_*.json` files. Those are
...
def test_merge_chunks_skips_chunk_with_path_escape_id(tmp_path, monkeypatch, capsys):
    good = tmp_path / ".omnigraph_chunk_0.json"
    _write(good, {"nodes": [{"id": "pkg.mod.good", "label": "G"}], "edges": [], "hyperedges": []})
    bad = tmp_path / ".omnigraph_chunk_1.json"
    # A node id with a path separator would escape the chunk directory.
    _write(bad, {"nodes": [{"id": "../../etc/passwd", "label": "B"}], "edges": [], "hyperedges": []})
    out = tmp_path / "merged.json"

    _run_merge(monkeypatch, ["omnigraph", "merge-chunks", str(good), str(bad), "--out", str(out)])
...
def test_merge_chunks_fails_closed_when_every_chunk_is_invalid(tmp_path, monkeypatch, capsys):
    bad = tmp_path / ".omnigraph_chunk_0.json"
    _write(bad, {"nodes": "not-a-list", "edges": []})
    out = tmp_path / "merged.json"
    out.write_text('{"previous": "semantic result"}', encoding="utf-8")

    with pytest.raises(SystemExit) as exc:
        _run_merge(monkeypatch, ["omnigraph", "merge-chunks", str(bad), "--out", str(out)])
...
def test_merge_chunks_accepts_valid_empty_chunk(tmp_path, monkeypatch):
    ...
    empty = tmp_path / ".omnigraph_chunk_0.json"
    _write(empty, {"nodes": [], "edges": [], "hyperedges": []})
    out = tmp_path / "merged.json"

    _run_merge(monkeypatch, ["omnigraph", "merge-chunks", str(empty), "--out", str(out)])
...
def test_merge_chunks_fails_closed_without_chunk_arguments(tmp_path, monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as exc:
        _run_merge(monkeypatch, ["omnigraph", "merge-chunks", "--out", str(out)])
...
def test_merge_chunks_fails_closed_on_unmatched_glob(tmp_path, monkeypatch, capsys):
    ...
    unmatched = str(tmp_path / ".omnigraph_chunk_*.json")

    with pytest.raises(SystemExit) as exc:
        _run_merge(monkeypatch, ["omnigraph", "merge-chunks", unmatched, "--out", str(out)])
...
def test_merge_chunks_accepts_synonym_file_type(tmp_path, monkeypatch):
    ...
    # a validation failure — the chunk must merge, not be silently dropped.
    c = tmp_path / ".omnigraph_chunk_0.json"
    _write(c, {"nodes": [{"id": "pkg.readme", "label": "Readme", "file_type": "markdown"},
                         {"id": "pkg.tool", "label": "Tool", "file_type": "tool"}],
               "edges": [], "hyperedges": []})
    out = tmp_path / "merged.json"
    _run_merge(monkeypatch, ["omnigraph", "merge-chunks", str(c), "--out", str(out)])
    merged = json.loads(out.read_text())
...
def test_validate_semantic_fragment_accepts_synonyms_and_unicode():
    from omnigraph.semantic_cleanup import validate_semantic_fragment
...
def test_merge_chunks_merges_valid_chunks(tmp_path, monkeypatch):
    c0 = tmp_path / ".omnigraph_chunk_0.json"
    _write(c0, {"nodes": [{"id": "a", "label": "A"}], "edges": [], "hyperedges": [],
               "input_tokens": 10, "output_tokens": 5})
    c1 = tmp_path / ".omnigraph_chunk_1.json"
    _write(c1, {"nodes": [{"id": "b", "label": "B"}], "edges": [], "hyperedges": [],
               "input_tokens": 7, "output_tokens": 3})
    out = tmp_path / "merged.json"

    _run_merge(monkeypatch, ["omnigraph", "merge-chunks", str(c0), str(c1), "--out", str(out)])
