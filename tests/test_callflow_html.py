...
from omnigraph.callflow_html import derive_sections_from_communities, write_callflow_html


def _make_omnigraph_out(tmp_path: Path) -> Path:
...
def test_write_callflow_html_creates_file_and_uses_report(tmp_path):
    out = _make_omnigraph_out(tmp_path)

    html_path = write_callflow_html(
        tmp_path,
        output="omnigraph-out/callflow.html",
        max_sections=4,
    )
...
def test_export_callflow_html_cli_creates_file(tmp_path):
    ...
    html_path = tmp_path / "omnigraph-out" / "from-cli.html"
...
def test_export_callflow_html_cli_accepts_positional_graph_path(tmp_path):
    _make_omnigraph_out(tmp_path)
    external_out = tmp_path / "GitNexus" / "omnigraph-out"
    ...
    (external_out / ".omnigraph_labels.json").write_text(json.dumps({"0": "External Runtime", "1": "External Export"}), encoding="utf-8")
...
def test_derive_sections_groups_by_architecture_keywords():
    nodes = [
        {"id": "extract_py", "label": "extract_python", "source_file": "omnigraph/extract.py", "community": 0},
        {"id": "extract_js", "label": "extract_js", "source_file": "omnigraph/extract.py", "community": 0},
        {"id": "to_html", "label": "to_html", "source_file": "omnigraph/export.py", "community": 1},
...
def test_load_graph_rejects_oversized_file(monkeypatch, tmp_path):
    ...
    import pytest
    from omnigraph.callflow_html import load_graph
...
def test_load_graph_preserves_edge_direction(tmp_path):
    ...
    from omnigraph.callflow_html import generate_call_table_rows, load_graph
...
def test_call_table_counts_indirect_call_relation():
    ...
    from omnigraph.callflow_html import generate_call_table_rows
