...
from omnigraph.build import build_from_json, build, build_merge, edge_data, edge_datas, dedupe_edges, dedupe_nodes

FIXTURES = Path(__file__).parent / "fixtures"


def test_dedupe_edges_collapses_exact_parallels():
    # --no-cluster / incremental update concatenate edge lists raw.
    edges = [
...
def test_dedupe_nodes_collapses_by_id_last_wins():
    # a shared module anchor is emitted once per importing file; the
    # --no-cluster raw writer must collapse same-id node dicts.
    nodes = [
...
def test_legacy_node_name_path_aliases_folded():
    ...
    from omnigraph.validate import validate_extraction
...
# Regression tests for — edge_data / edge_datas helpers must tolerate
...
def test_build_relativizes_absolute_source_file(tmp_path):
    ...
    # re-keys main_fn (old stem "main") to the full-path form "src_main".
...
def test_build_from_json_ambiguous_old_stem_alias_stays_dangling(tmp_path):
    ...
    extraction = {
        "nodes": [
            ...
            # form, matching what a real omnigraph update run would already have
...
def test_merge_raw_extraction_tier_scoped(tmp_path):
    ...
    from omnigraph.build import merge_raw_extraction
...
def test_build_merge_root_collapses_convention_drift(tmp_path):
    ...
    # re-keys the author-chosen short ids to the canonical full-path stem.
...
def test_graph_has_legacy_ids_detects_old_scheme():
    ...
    from omnigraph.build import graph_has_legacy_ids
...
# ── globally-scoped MCP node ids are not file-stem derived ──────────────
...
def test_semantic_rekey_relative_vs_absolute_source_file():
    ...
    from omnigraph.build import _semantic_id_remap
...
# mirror of, reported as) or joins it under the scan root.
#
...
def test_derive_prune_root_recovers_root_from_posix_absolute_prune_sources():
    ...
    direction.
    """
    from omnigraph.build import _derive_prune_root
...
# ── Cross-file stub nodes must not pollute AST replacement set ────────
...
def test_build_annotations_all_resolve():
    ...
    from omnigraph import build as build_module
