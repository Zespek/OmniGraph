...
from omnigraph.cross_repo_calls import (
...
def _merge(tmp_path: Path, a: Path, b: Path) -> dict:
    ...
    result = subprocess.run([PYTHON, "-m", "omnigraph", "merge-graphs", str(a), str(b),
                             "--out", str(out)],
...
def _build(tmp_path: Path, repo: str, name: str, body: str) -> Path:
    ...
    result = extract([path], cache_root=tmp_path / repo / "omnigraph-out")
    graph = tmp_path / repo / "omnigraph-out" / "graph.json"
...
def test_merge_graphs_cli_finishes_the_parked_call(tmp_path: Path):
    ...
    a = tmp_path / "app" / "omnigraph-out" / "graph.json"
    b = tmp_path / "lib" / "omnigraph-out" / "graph.json"
