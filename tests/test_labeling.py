...
from omnigraph.llm import label_communities, generate_community_labels
...
def test_label_communities_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.llm._call_llm", fake_call)
...
def test_label_cli_passes_model_override(tmp_path, monkeypatch):
    import omnigraph.__main__ as cli
    ...
    monkeypatch.setattr("omnigraph.llm.generate_community_labels", fake_generate)
    monkeypatch.setattr("omnigraph.export.to_html", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_label_cli_missing_only_preserves_existing_labels(tmp_path, monkeypatch):
    ...
    (out / "graph.json").write_text(json.dumps(graph), encoding="utf-8")
    (out / ".omnigraph_labels.json").write_text(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["omnigraph", "label", str(tmp_path), "--missing-only", "--backend", "gemini", "--no-viz"],
    )

    cli.main()

    assert set(captured["communities"]) == {1}
    labels = json.loads((out / ".omnigraph_labels.json").read_text(encoding="utf-8"))
    assert labels == {"0": "Order Management", "1": "Payment Flow"}


def test_label_communities_partial_reply_fills_placeholder(monkeypatch):
    G, communities = _graph()
    monkeypatch.setattr("omnigraph.llm._call_llm",
...
def test_generate_community_labels_no_backend(monkeypatch):
    ...
    # Keep hermetic: the claude-cli labelling fallback probes PATH, so a
    # machine with `claude` installed would otherwise take the CLI path here.
    monkeypatch.setattr("omnigraph.llm._claude_cli_available", lambda: False)
...
def test_label_communities_batches_when_over_batch_size(monkeypatch):
    ...
    def fake_call(prompt, *, backend, max_tokens=200):
        ...
        # prompt lines are now "<cid>: <names>" — the old "Community {cid}:"
...
def test_label_communities_all_batches_fail_raises(monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.llm._call_llm", always_fail)
...
# --- parallel labeling (--max-concurrency) + --batch-size --------------
...
def test_label_communities_parallel_matches_sequential(monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.llm._label_batch_with_retry", fake_batch)
...
def test_label_communities_salvages_truncated_reply(monkeypatch):
    # a reply truncated mid-object (a stingy token budget or model
...
def test_label_communities_accumulates_token_usage(monkeypatch):
    # cluster-only mode reported zero labeling cost because token usage
...
def test_label_communities_counts_tokens_for_failed_batch(monkeypatch):
    ...
    # its tokens must be counted even though it contributes no label.
    G, communities = _graph()
...
def test_cluster_commands_render_aggregated_html_above_viz_limit(
    ...
    monkeypatch.setattr(
        "omnigraph.llm.generate_community_labels",
    ...
    argv = ["omnigraph", command, str(tmp_path)]
...
def test_cluster_only_preserves_but_does_not_claim_unusable_aggregate(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("omnigraph.cluster"),
        "cluster",
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["omnigraph", "cluster-only", str(tmp_path), "--no-label"],
    )
...
def test_cluster_only_restores_html_after_unexpected_render_failure(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("omnigraph.export"),
        "to_html",
        fail_render,
    )
    monkeypatch.setattr(
...
def test_cluster_only_marks_html_stale_before_report_generation(
    ...
    monkeypatch.setenv("OMNIGRAPH_VIZ_NODE_LIMIT", "3")
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("omnigraph.report.generate", interrupt_report)
    monkeypatch.setattr(
        sys,
        "argv",
        ["omnigraph", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_refused_graph_write_preserves_existing_stale_marker(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("omnigraph.export.to_json", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        sys,
        "argv",
        ["omnigraph", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_no_label_does_not_persist_placeholders(tmp_path, monkeypatch):
    """#2073: --no-label must not write .omnigraph_labels.json with 'Community N'
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("omnigraph.export.to_html", lambda *a, **k: None)
    # No-backend fallback shape: returns placeholders, which must not clobber hubs.
    monkeypatch.setattr("omnigraph.llm.generate_community_labels",
                        lambda G, comms, **k: ({cid: f"Community {cid}" for cid in comms}, "none"))

    monkeypatch.setattr(sys, "argv", ["omnigraph", "cluster-only", str(tmp_path), "--no-label", "--no-viz"])
    cli.main()
    assert not labels_path.exists(), "--no-label persisted a placeholder labels file (#2073)"
    assert not (out / ".omnigraph_labels.json.sig").exists()

    # A later normal run generates real labels (no sticky placeholders blocking it).
    monkeypatch.setattr(sys, "argv", ["omnigraph", "cluster-only", str(tmp_path), "--no-viz"])
    cli.main()
...
def test_cluster_only_heals_persisted_placeholder_but_reuses_genuine(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.llm.generate_community_labels", _fail_generate)
...
# ── case 2: prompt key must not collide with the placeholder sentinel ──
...
def test_label_prompt_lines_use_bare_cid_keys():
    ...
    from omnigraph.llm import _community_label_lines
...
def test_label_cli_drops_sentinel_and_bare_key_echoes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["omnigraph", "label", str(tmp_path), "--backend", "gemini", "--no-viz"],
    )
