"""Tests for omnigraph/security.py - URL validation, safe fetch, path guards, label sanitisation."""
...
from omnigraph.security import (
...
def test_safe_fetch_returns_bytes(tmp_path):
    ...
    with patch("omnigraph.security._build_opener") as mock_opener_fn:
...
def test_validate_graph_path_default_base_discovers_output_dir(tmp_path):
    ...
    parents for the configured output-dir name (default 'omnigraph-out')."""
    base = tmp_path / "omnigraph-out"
    base.mkdir()
    graph = base / "graph.json"
    graph.write_text("{}")
...
def test_validate_graph_path_default_base_honours_omnigraph_out_override(tmp_path, monkeypatch):
    """The base=None discovery must honour OMNIGRAPH_OUT, not the hardcoded
    'omnigraph-out' literal — otherwise a renamed output dir validates against the
    wrong base or raises spuriously (#1423)."""
    monkeypatch.setattr("omnigraph.security.OMNIGRAPH_OUT_NAME", "custom-out")
    monkeypatch.setattr("omnigraph.security.OMNIGRAPH_OUT", "custom-out")
    out = tmp_path / "custom-out"
    out.mkdir()
    graph = out / "graph.json"
    graph.write_text("{}")
    # No base passed → must discover custom-out by name rather than omnigraph-out.
    assert validate_graph_path(str(graph)) == graph.resolve()
...
def test_sanitize_label_none_returns_empty():
    # a node with source_file=None / label=None (synthetic/aggregate
...
def test_graph_size_cap_error_message_includes_size_and_cap(monkeypatch, tmp_path):
    monkeypatch.setattr("omnigraph.security._MAX_GRAPH_FILE_BYTES", 8)
    p = tmp_path / "graph.json"
...
def test_graph_size_cap_at_boundary_passes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("omnigraph.security._MAX_GRAPH_FILE_BYTES", 32)
    assert check_graph_file_size_cap(p) is None
    monkeypatch.setattr("omnigraph.security._MAX_GRAPH_FILE_BYTES", 31)
    with pytest.raises(ValueError):
        check_graph_file_size_cap(p)
