"""`omnigraph query` must render every edge between visited nodes (#2323).
...
from omnigraph.serve import _bfs, _dfs, _filter_graph_by_context, _query_graph_text
...
def test_query_cli_renders_the_edge_between_two_seeds(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "query", "checkout discounted_total", "--graph", str(graph_path)],
    )
