...
`sed q`) is routine. omnigraph used to keep writing after the reader disconnected,
...
def test_help_survives_reader_closing_pipe_early():
    """`omnigraph --help | head -n1` must leave omnigraph exiting 0, not 255."""
    producer = subprocess.Popen(
        [PYTHON, "-m", "omnigraph", "--help"], stdout=subprocess.PIPE
    )
...
def test_small_buffered_output_survives_reader_that_reads_nothing():
    ...
    producer = subprocess.Popen(
        [PYTHON, "-m", "omnigraph", "--version"], stdout=subprocess.PIPE
    )
    reader = subprocess.Popen(
