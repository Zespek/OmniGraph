...
Since #522 it runs as the shell-agnostic `omnigraph hook-guard search` subcommand
...
from omnigraph.__main__ import _claude_pretooluse_hooks
...
def _env():
    e = dict(os.environ)
    e.pop("OMNIGRAPH_OUT", None)
    return e


def _run(command, cwd, *, graph: bool):
    if graph:
        (cwd / "omnigraph-out").mkdir(parents=True, exist_ok=True)
        (cwd / "omnigraph-out" / "graph.json").write_text("{}", encoding="utf-8")
    stdin = json.dumps({"tool_input": {"command": command}})
    return subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-guard", "search"],
...
def test_matcher_targets_bash_and_grep():
    # content search goes through Claude Code's dedicated Grep tool, so
...
def test_hook_command_has_no_backslashes(monkeypatch):
    ...
    # escape character and strips it (C:\Users\me\omnigraph.EXE -> C:Usersme...),
    # breaking every guard. The emitted command must use forward slashes.
    from omnigraph.__main__ import _resolve_omnigraph_exe
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\Users\me\omnigraph.EXE")
    assert _resolve_omnigraph_exe() == "C:/Users/me/omnigraph.EXE"
    for h in _claude_pretooluse_hooks():
        assert "\\" not in h["hooks"][0]["command"]


def test_command_has_no_shell_syntax():
    # no POSIX bash that Windows cmd.exe/PowerShell can't parse.
    ...
    assert "omnigraph" in cmd and "hook-guard search" in cmd
...
def test_nudges_on_search_commands_with_graph(tmp_path):
    for command in (
        ...
        assert "omnigraph query" in out, f"{command!r} should nudge"
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "omnigraph query" in payload["hookSpecificOutput"]["additionalContext"]


def test_fails_open_on_malformed_stdin(tmp_path):
    (tmp_path / "omnigraph-out").mkdir()
    (tmp_path / "omnigraph-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-guard", "search"],
...
def test_honors_omnigraph_out_override(tmp_path):
    """The guard resolves the graph via OMNIGRAPH_OUT, not a hardcoded path."""
    custom = tmp_path / "custom-out"
    custom.mkdir()
    (custom / "graph.json").write_text("{}", encoding="utf-8")
    env = dict(os.environ, OMNIGRAPH_OUT=str(custom))
    stdin = json.dumps({"tool_input": {"command": "grep -rn foo ."}})
    r = subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-guard", "search"],
        input=stdin, capture_output=True, text=True, cwd=tmp_path, env=env,
    )
    assert "omnigraph query" in r.stdout


# ---------------------------------------------------------------------------
# the dedicated Grep tool (pattern/path/glob, no command) must nudge too
# ---------------------------------------------------------------------------
...
def test_grep_tool_input_nudges_with_graph(tmp_path):
    for tool_input in (
        ...
        assert "omnigraph query" in out, f"Grep input {tool_input!r} should nudge"
...
def test_bash_non_search_with_stray_pattern_key_does_not_nudge(tmp_path):
    ...
    (tmp_path / "omnigraph-out").mkdir(parents=True, exist_ok=True)
    (tmp_path / "omnigraph-out" / "graph.json").write_text("{}", encoding="utf-8")
