"""#2316: `omnigraph update <target>` must write manifest.json into the TARGET's
omnigraph-out/, not the process CWD's.

`_rebuild_code` computes ``out = watch_path / _OMNIGRAPH_OUT`` and routes every
...
def test_manifest_lands_in_the_target_not_the_cwd(two_projects, monkeypatch, no_cluster):
    ...
    target_manifest = proj_b / "omnigraph-out" / "manifest.json"
    cwd_manifest = proj_a / "omnigraph-out" / "manifest.json"

    assert target_manifest.exists(), (
        "manifest.json must land next to the graph it describes, in the target's "
        f"omnigraph-out/ (#2316); {target_manifest} is missing"
    )
...
def test_second_run_reaches_the_same_topology_early_return(two_projects, monkeypatch, capsys):
    ...
    assert target_manifest.exists(), (
        ...
        "into the target's omnigraph-out/ (#2316)"
    )
    assert not (proj_a / "omnigraph-out" / "manifest.json").exists(), (
...
def test_update_does_not_destroy_the_cwd_projects_own_manifest(two_projects, monkeypatch):
    ...
    own_manifest = proj_a / "omnigraph-out" / "manifest.json"
...
def test_built_at_commit_comes_from_the_target_repo(two_projects, monkeypatch):
    ...
    graph = json.loads((proj_b / "omnigraph-out" / "graph.json").read_text(encoding="utf-8"))
...
def test_relative_target_manifest_is_consumable_by_detect_incremental(
    ...
    """
    from omnigraph.detect import detect_incremental
    from omnigraph.watch import _rebuild_code
    ...
    manifest_path = proj_b / "omnigraph-out" / "manifest.json"
