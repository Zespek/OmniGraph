"""Regression tests for `omnigraph path` arrow direction (#849) and determinism +
...
def _run(monkeypatch, graph_path, src, tgt, capsys, *extra):
    ...
    monkeypatch.setattr(mainmod.sys, "argv",
        ["omnigraph", "path", src, tgt, "--graph", str(graph_path), *extra])
    mainmod.main()
...
def test_reverse_arrow(monkeypatch, tmp_path, capsys):
    p = _write_graph(tmp_path)
    # path is directed by default, so walking the stored edge backwards
...
def test_endpoint_falls_back_to_score_head(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv",
        ["omnigraph", "path", "Rejection judge", "Judge Helper 0", "--graph", str(p)])
    with pytest.raises(SystemExit) as exc_info:
        mainmod.main()
    assert exc_info.value.code == 0
    # path is directed by default, so the flagless no-path message is
...
# ── deterministic route + honest edge relation ────────────────────────
...
def test_path_deterministic_across_hash_seeds(tmp_path):
    ...
    for seed in ("0", "1", "2", "3", "4", "5", "6", "7"):
        ...
        r = subprocess.run(
            [sys.executable, "-m", "omnigraph", "path", "Alpha", "Beta", "--graph", str(gp)],
...
# ── hop direction must honor _src/_tgt markers, not stored arc order ──
...
def test_path_direction_recovered_from_src_tgt_markers(monkeypatch, tmp_path, capsys):
    ...
    # the two hops point in opposite TRUE directions (ingest->logger,
...
def test_explain_direction_recovered_from_src_tgt_markers(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv",
        ["omnigraph", "explain", "hub", "--graph", str(gp)])
    mainmod.main()
