...
`omnigraph install` hard-exit with "not found in package" for real users.
...
def _skill_bodies() -> list[Path]:
    ...
    one of these). A body missing from the wheel makes `omnigraph install
...
def wheel_namelist(tmp_path_factory) -> set[str]:
    ...
    wheels = list(out.glob("omnigraph-*.whl"))
...
def test_skill_artifact_ships_in_wheel(artifact: Path, wheel_namelist: set[str]) -> None:
    rel = "omnigraph/" + artifact.relative_to(PKG).as_posix()
    assert rel in wheel_namelist, (
        f"{rel} is committed in the repo but NOT in the built wheel — "
        f"`omnigraph install` would hard-exit for this host. Check the "
