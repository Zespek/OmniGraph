...
These strings live in omnigraph/__main__.py and are written into project-local
files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursor/rules/, .kiro/steering/, etc.)
or into in-process hook payloads. Earlier versions of omnigraph told every
assistant to "ALWAYS read omnigraph-out/GRAPH_REPORT.md before answering" —
...
from omnigraph.__main__ import (
    _SEARCH_NUDGE,
    _READ_NUDGE,
    _skill_registration,
    _CLAUDE_MD_SECTION,
...
def test_every_install_surface_recommends_omnigraph_query():
    """All ten install surfaces must point the assistant at `omnigraph query`
    ...
    for name, text in _INSTALL_TEXTS.items():
        if "omnigraph query" not in text:
            missing.append(name)
    assert not missing, (
        f"these install surfaces no longer mention `omnigraph query`: {missing}. "
...
def test_agents_section_does_not_skip_dirty_graph_output():
    assert "Dirty omnigraph-out/ files are expected" in _AGENTS_MD_SECTION
    assert "not a reason to skip omnigraph" in _AGENTS_MD_SECTION


def test_agents_section_uses_generic_omnigraph_instruction():
    assert "`skill` tool" not in _AGENTS_MD_SECTION
    assert 'skill: "omnigraph"' not in _AGENTS_MD_SECTION
    assert "use the installed omnigraph skill" in _AGENTS_MD_SECTION


def test_skill_registration_uses_host_generic_instruction():
    reg = _skill_registration()
    assert 'skill: "omnigraph"' not in reg
    assert "Skill tool" not in reg
    assert "use the installed omnigraph skill or instructions" in reg
