...
`omnigraph-out/` resolves against the CWD. Running `omnigraph query` from a parent
...
from omnigraph.serve import _display_graph_path, _query_graph_text
...
def test_header_names_the_graph_and_its_size(tmp_path, monkeypatch):
    ...
    gp = tmp_path / "omnigraph-out" / "graph.json"
    gp.parent.mkdir(parents=True)
    gp.write_text("{}", encoding="utf-8")
    header = _header(_graph(4), graph_path=str(gp))
    assert header.startswith("Graph: omnigraph-out/graph.json (4 nodes) | ")
...
def test_a_graph_outside_the_cwd_is_shown_in_full(tmp_path, monkeypatch):
    ...
    other = tmp_path / "elsewhere" / "omnigraph-out"
    here.mkdir()
...
def test_the_node_count_is_the_graphs_not_the_traversals():
    ...
    meant to surface."""
    G = _graph(6)
    header = _header(G, graph_path="omnigraph-out/graph.json")
...
def test_display_is_relative_under_cwd(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    p = tmp_path / "omnigraph-out" / "graph.json"
    p.parent.mkdir(parents=True)
    p.write_text("{}", encoding="utf-8")
    assert _display_graph_path(str(p)) == "omnigraph-out/graph.json"
...
def test_two_projects_produce_distinguishable_headers(tmp_path, monkeypatch):
    ...
    sub = parent / "Sub"
    (parent / "omnigraph-out").mkdir(parents=True)
    (sub / "omnigraph-out").mkdir(parents=True)
    for d in (parent, sub):
        (d / "omnigraph-out" / "graph.json").write_text("{}", encoding="utf-8")

    monkeypatch.chdir(parent)
    h_parent = _header(_graph(9), graph_path=str(parent / "omnigraph-out" / "graph.json"))
    h_sub = _header(_graph(3), graph_path=str(sub / "omnigraph-out" / "graph.json"))
    assert h_parent != h_sub
    assert "(9 nodes)" in h_parent and "(3 nodes)" in h_sub
    assert "Sub/omnigraph-out/graph.json" in h_sub
