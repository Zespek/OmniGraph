...
`_check_skill_version` used to advise `omnigraph install` on ANY version
...
def _make_skill(tmp_path: Path, stamped: str) -> Path:
    ...
    skill_dst.write_text("# omnigraph skill\n", encoding="utf-8")
    (skill_dst.parent / ".omnigraph_version").write_text(stamped, encoding="utf-8")
    return skill_dst
...
def test_skill_older_than_package_recommends_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'omnigraph install' to update" in err
...
def test_skill_newer_than_package_recommends_upgrade_not_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'omnigraph install' to update" not in err
