...
`omnigraph install --platform X` must only advance `.omnigraph_version` for the
...
omnigraph A, package is B" staleness warning was suppressed even though the
...
# A prior omnigraph version stamped into an unrelated, not-reinstalled platform.
...
def test_install_does_not_bump_other_platforms_stamp(tmp_path, monkeypatch):
    ...
    platform's `.omnigraph_version` untouched, so its staleness warning stays
    truthful (#2694)."""
    home = tmp_path / "home"
    home.mkdir()
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        codex_stamp = codex_skill.parent / ".omnigraph_version"
        ...
        assert (claude_skill.parent / ".omnigraph_version").read_text() == mainmod.__version__
...
def test_stale_untouched_platform_still_emits_warning(tmp_path, monkeypatch, capsys):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        codex_skill.parent.mkdir(parents=True, exist_ok=True)
        codex_skill.write_text("stale skill body", encoding="utf-8")
        (codex_skill.parent / ".omnigraph_version").write_text(_STALE_STAMP, encoding="utf-8")
