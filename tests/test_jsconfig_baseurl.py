...
from omnigraph.extract import _make_id, extract
...
def _widget(tmp_path: Path, base_url: str = "app/javascript") -> str:
    ...
    # (canonicalizes cross-file edge targets).
...
def test_declared_paths_alias_still_wins_over_baseurl(tmp_path):
    # Guards a declared alias must not be shadowed by the baseUrl
    ...
    _write(tmp_path / "jsconfig.json",
           ...
           '    "paths": { "@mods/*": ["real/*"] }\n'
           '  }\n}\n')
    # paths targets are relative to baseUrl, so "real/*" is
    # <baseUrl>/real/*.
...
def test_tsconfig_paths_alias_unchanged(tmp_path):
    # Pre-existing behavior, green before and after this change: a
...
# --- config edits must survive the per-process caches ---------------
#
...
# shows. `omnigraph watch` and the MCP server are the opposite shape: one process,
