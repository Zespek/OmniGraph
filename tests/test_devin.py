"""Tests for omnigraph devin install / uninstall commands."""
...
def _skill_path_user(tmp_path):
    return tmp_path / ".config" / "devin" / "skills" / "omnigraph" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".devin" / "skills" / "omnigraph" / "SKILL.md"


def _rules_path(project_dir):
    return project_dir / ".windsurf" / "rules" / "omnigraph.md"


# ---------------------------------------------------------------------------
# User-scope install (omnigraph install --platform devin / omnigraph devin install)
# ---------------------------------------------------------------------------

def test_devin_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.config/devin/skills/omnigraph/SKILL.md."""
...
def test_devin_skill_file_references_omnigraph_query(tmp_path):
...
# ---------------------------------------------------------------------------
# Project-scope install (omnigraph devin install --project)
# ---------------------------------------------------------------------------

def test_devin_install_project_creates_skill_file(tmp_path, monkeypatch):
    """Project-scope install copies skill to .devin/skills/omnigraph/SKILL.md."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "devin", "install", "--project"])
...
def test_devin_install_project_creates_rules_file(tmp_path, monkeypatch):
    """Project-scope install writes .windsurf/rules/omnigraph.md."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    assert rules.exists()
    assert "omnigraph" in rules.read_text()
    assert "GRAPH_REPORT.md" in rules.read_text()


def test_devin_rules_content_recommends_omnigraph_query(tmp_path):
    """The rules file installed by devin must use query-first policy."""
    from omnigraph.__main__ import _devin_rules_install
    _devin_rules_install(tmp_path)
    content = _rules_path(tmp_path).read_text()
    assert "omnigraph query" in content
...
def test_devin_uninstall_user_removes_skill_file(tmp_path):
    ...
    from omnigraph.__main__ import _remove_skill_file
    with patch("omnigraph.__main__.Path.home", return_value=tmp_path):
...
def test_devin_uninstall_user_noop_when_not_installed(tmp_path, capsys):
    ...
    try:
        os.chdir(tmp_path)
        with patch("omnigraph.__main__.Path.home", return_value=tmp_path):
            sys.argv = ["omnigraph", "devin", "uninstall"]
            main()
    finally:
        os.chdir(old_cwd)
    out = capsys.readouterr().out
...
def test_devin_uninstall_project_removes_skill_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .devin/skills/omnigraph/SKILL.md."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "devin", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["omnigraph", "devin", "uninstall", "--project"])
        main()
    assert not _skill_path_project(project).exists()


def test_devin_uninstall_project_removes_rules_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .windsurf/rules/omnigraph.md."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_devin_rules_uninstall_noop_when_not_installed(tmp_path):
    ...
    from omnigraph.__main__ import _devin_rules_uninstall
...
def test_devin_skill_file_exists_in_package():
    ...
    import omnigraph
    skill = Path(omnigraph.__file__).parent / "skill-devin.md"
...
def test_devin_skill_file_uses_python_c_syntax():
    ...
    All mature omnigraph skills use the interpreter-detection pattern
    ``$(cat omnigraph-out/.omnigraph_python) -c "..."`` rather than bare
    ``python -c "..."`` so they work in pipx / venv environments.
    """
    import omnigraph
    skill = (Path(omnigraph.__file__).parent / "skill-devin.md").read_text()
    assert '.omnigraph_python) -c "' in skill, (
...
def test_devin_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.config/devin/skills/omnigraph/SKILL.md."""
    from omnigraph.__main__ import _platform_skill_destination
    with patch("omnigraph.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("devin", project=False)
    assert dst == tmp_path / ".config" / "devin" / "skills" / "omnigraph" / "SKILL.md"


def test_devin_in_main_help_text(capsys, monkeypatch):
    """`omnigraph --help` must list devin in the platform list and in the per-platform section."""
    from omnigraph.__main__ import main
    ...
    assert "|devin)" in captured or "|devin |" in captured or "|devin" in captured, (
        "devin missing from `omnigraph --help` platform list"
    )
...
def test_devin_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.devin/skills/omnigraph/SKILL.md."""
    from omnigraph.__main__ import _platform_skill_destination
    dst = _platform_skill_destination("devin", project=True, project_dir=tmp_path)
    assert dst == tmp_path / ".devin" / "skills" / "omnigraph" / "SKILL.md"
