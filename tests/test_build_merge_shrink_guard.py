...
- #2446: with `root` omitted and a non-standard layout (graph.json not under
  <root>/omnigraph-out/), the inferred root was wrong, absolute prune_sources
...
import omnigraph.build as buildmod
from omnigraph.build import build_merge, merge_raw_extraction
...
def test_no_existing_graph_path_unchanged(tmp_path):
    ...
    gp = tmp_path / "omnigraph-out" / "graph.json"  # does not exist
...
# ── absolute prune_sources with a wrong/missing inferred root ──────────

def test_absolute_prune_custom_layout_derives_root(tmp_path):
    """graph.json directly in <root> (no omnigraph-out dir, no marker): the
...
def test_absolute_prune_standard_layout_marker_still_prunes(tmp_path):
    ...
    .omnigraph_root marker keeps working without root=."""
    root = tmp_path / "repo"
    out = root / "omnigraph-out"
    out.mkdir(parents=True)
    (out / ".omnigraph_root").write_text(str(root), encoding="utf-8")
    gp = out / "graph.json"
...
def test_zero_match_prune_warns_instead_of_already_clean(tmp_path, capsys):
    ...
    gp = root / "omnigraph-out" / "graph.json"
