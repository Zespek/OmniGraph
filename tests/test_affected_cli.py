...
def test_affected_cli_reverse_traverses_impact_edges(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "affected", "Foo", "--graph", str(graph_path)],
    )
...
def test_affected_cli_relation_filter_limits_reverse_traversal(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "affected", "Foo", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_forces_directed_on_undirected_graph(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "affected", "B", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_loads_edges_keyed_graph(monkeypatch, tmp_path, capsys):
    """omnigraph's `extract` writes graph.json with an "edges" key (not networkx's
    ...
    # Emulate omnigraph extract output: top-level "edges" key instead of "links".
    data = json_graph.node_link_data(graph, edges="links")
...
def test_resolve_seed_bare_name_matches_callable_label():
    from omnigraph.affected import resolve_seed
...
def test_affected_cli_source_file_path_uses_file_level_node(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "affected", source_file, "--graph", str(graph_path)],
    )
...
def test_affected_reports_call_site_line_not_def_line(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(mainmod.sys, "argv",
                        ["omnigraph", "affected", "transition_state", "--graph", str(gp)])
    mainmod.main()
    out = capsys.readouterr().out
...
def test_affected_falls_back_to_def_line_when_edge_has_no_location(monkeypatch, tmp_path, capsys):
    ...
    gp.write_text(json.dumps(json_graph.node_link_data(g, edges="links")), encoding="utf-8")
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["omnigraph", "affected", "target", "--graph", str(gp)])
    mainmod.main()
...
def test_affected_absolute_seed_resolves_via_graph_root_off_cwd(tmp_path, monkeypatch, capsys):
    ...
    location (`<root>/omnigraph-out/graph.json`).
    """
    from omnigraph.paths import OMNIGRAPH_OUT_NAME

    repo_root = tmp_path / "repo"
    out_dir = repo_root / OMNIGRAPH_OUT_NAME
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["omnigraph", "affected", abs_seed, "--graph", str(gp)])
    mainmod.main()
...
def test_affected_absolute_seed_outside_root_misses_cleanly(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv",
                        ["omnigraph", "affected", outside_seed, "--graph", str(gp)])
    mainmod.main()
...
def test_affected_absolute_seed_with_graph_not_under_out_dir(tmp_path, monkeypatch, capsys):
    ...
    omnigraph-out dir, the root is the graph's own parent (`else gp.parent`)."""
    repo_root = tmp_path / "repo"
    ...
    g.add_node("caller", label="X()", source_file="app.py", source_location="L4")
    g.add_edge("caller", "target", relation="calls")
    gp = repo_root / "graph.json"  # directly under repo_root, not omnigraph-out/
    gp.write_text(json.dumps(json_graph.node_link_data(g, edges="links")), encoding="utf-8")
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv",
                        ["omnigraph", "affected", abs_seed, "--graph", str(gp)])
    mainmod.main()
