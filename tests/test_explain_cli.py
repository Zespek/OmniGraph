"""Regression tests for `omnigraph explain` arrow direction (#853)."""
...
def _run(monkeypatch, graph_path, label, capsys):
    ...
    monkeypatch.setattr(mainmod.sys, "argv",
        ["omnigraph", "explain", label, "--graph", str(graph_path)])
    mainmod.main()
...
def _write_sidecar(tmp_path, nodes):
    (tmp_path / ".omnigraph_learning.json").write_text(
...
# --- high-degree nodes must not silently hide the cut connections ------
