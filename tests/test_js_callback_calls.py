...
def _extract(tmp_path, files: dict[str, str]):
    ...
    r = extract([tmp_path / n for n in files],
                cache_root=tmp_path / "omnigraph-out", parallel=False)
...
def test_callback_member_call_is_origin_gated(tmp_path):
    # Coupling guard: makes the callback body visible to the member-call
    # resolver's origin gate must then block the name-only `Repo` match
...
def test_sibling_closure_param_does_not_suppress_indirect_call(tmp_path):
    # closure 1's param `alpha` must not shadow closure 2's reference
...
def test_multi_closure_direct_calls_still_captured(tmp_path):
    # guard for the multi-closure shape: a direct call inside the first
