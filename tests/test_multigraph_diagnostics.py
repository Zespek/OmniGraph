...
def test_diagnose_file_reads_json_and_formats_report(tmp_path: Path) -> None:
    ...
    assert "[omnigraph] MultiDiGraph edge-collapse diagnostic" in report
...
def test_diagnose_multigraph_cli_human_output(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "diagnose", "multigraph", "--graph", str(graph_path)],
    )

    mainmod.main()

    out = capsys.readouterr().out
    assert "[omnigraph] MultiDiGraph edge-collapse diagnostic" in out
...
def test_diagnose_multigraph_cli_undirected_override(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "diagnose", "multigraph", "--graph", str(graph_path), "--undirected"],
    )
...
def test_diagnose_multigraph_cli_json_output(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["omnigraph", "diagnose", "multigraph", "--graph", str(graph_path), "--json"],
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["wrong"], "Usage: omnigraph diagnose multigraph"),
...
def test_diagnose_multigraph_cli_usage_errors(
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["omnigraph", "diagnose", *argv_tail])
