...
def test_hook_skips_head_on_exe():
    ...
    from omnigraph.hooks import _PYTHON_DETECT
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    omnigraph launcher on PATH (uv tool / pipx isolation, #1127).

    When omnigraph is installed via `uv tool install omnigraph` or `pipx install
    omnigraph`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v omnigraph` fails, the python3/python
    fallbacks cannot import omnigraph (wrong venv), and the hook silently exits 0.
...
def test_hook_check_no_additionalContext(tmp_path):
    """omnigraph hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    ...
    result = subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-check"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
...
# ── background rebuild must not rely on nohup (missing on Git for Windows) ──
...
def test_hooks_limit_windows_workers_by_default(name, script):
    ...
    assert 'export OMNIGRAPH_MAX_WORKERS="${OMNIGRAPH_MAX_WORKERS:-1}"' in script


def _launcher_payload(script: str) -> str:
    """Extract the `python -c "<payload>"` the hook hands to OMNIGRAPH_PYTHON.
...
def test_rebuild_bodies_read_omnigraph_root(name, body):
    """The rebuild must honour the persisted scan root rather than hardcoding the
    repo top (#1173). Both bodies read <output-dir>/.omnigraph_root and pass the
    recovered root to _rebuild_code instead of the bare Path('.')."""
    assert ".omnigraph_root" in body, f"{name} ignores .omnigraph_root (#1173)"
    # The output dir is resolved from OMNIGRAPH_OUT at hook-run time, not hardcoded
    # to omnigraph-out/, so a renamed output dir is still found.
    assert "OMNIGRAPH_OUT" in body, f"{name} ignores the OMNIGRAPH_OUT override (#1423)"
    ...
    # into the path; the codec is a no-op on a BOM-less marker.
    assert "read_text(encoding='utf-8-sig')" in body, f"{name} root read is not single-quoted"


def test_rebuild_bodies_with_omnigraph_root_are_valid_python():
    """The .omnigraph_root snippet must parse so a quoting slip can't ship a hook
...
def _extract_root_resolution(body: str) -> str:
    """Pull the `.omnigraph_root` -> `_root` snippet out of a rebuild body, so a
...
def test_rebuild_bodies_reject_an_out_of_repo_omnigraph_root(name, body, tmp_path, monkeypatch):
    """#3265: `.omnigraph_root` sits inside omnigraph-out/, a directory the
    ...
    what gets read, and what gets written into the same committed omnigraph-out/
    ...
    (repo / "omnigraph-out").mkdir(parents=True)
    outside.mkdir()
    (repo / "omnigraph-out" / ".omnigraph_root").write_text(str(outside), encoding="utf-8")
...
def test_rebuild_bodies_honour_an_in_repo_omnigraph_root(name, body, tmp_path, monkeypatch):
    """The legitimate case the #3265 guard must not break: a subdirectory-scoped
    root (#1173) is still recovered."""
    repo = tmp_path / "repo"
    (repo / "omnigraph-out").mkdir(parents=True)
    (repo / "backend").mkdir()
    (repo / "omnigraph-out" / ".omnigraph_root").write_text("backend", encoding="utf-8")
    monkeypatch.chdir(repo)
    ns = {"Path": Path, "os": os}
    exec(compile(_extract_root_resolution(body), "<rebuild_body>", "exec"), ns)
...
def test_rebuild_bodies_survive_a_omnigraph_root_symlink_loop(name, body, tmp_path, monkeypatch):
    """A committed `.omnigraph_root` naming a path that resolves through a
    ...
    (repo / "omnigraph-out" / ".omnigraph_root").write_text("loop_a", encoding="utf-8")
    monkeypatch.chdir(repo)
    ns = {"Path": Path, "os": os}
    exec(compile(_extract_root_resolution(body), "<rebuild_body>", "exec"), ns)
...
def test_detached_launch_targets_omnigraph_python():
    """The launcher must run via the resolved $OMNIGRAPH_PYTHON, not a bare
    `python`, so it uses the same interpreter the detection block selected."""
    snippet = _detached_launch(_REBUILD_BODY_COMMIT)
    assert snippet.startswith('"$OMNIGRAPH_PYTHON" -c "')
...
# ── reject Windows-style hooks paths instead of creating a junk dir ───
...
def test_windows_hookspath_rejected_no_junk_dir_on_posix(tmp_path, monkeypatch, winpath):
    ...
    monkeypatch.setattr("omnigraph.hooks.os.name", "posix")
    repo = _make_git_repo(tmp_path)
...
def test_probes_use_find_spec_not_full_import():
    """`python -c "import omnigraph"` executes the FULL package import — 10s+ on a
    ...
    from omnigraph.hooks import _PYTHON_DETECT
    assert '-c "import omnigraph"' not in _PYTHON_DETECT, (
...
def test_shebang_read_is_null_byte_safe():
    """On Windows, `command -v omnigraph` can return the launcher path WITHOUT its
...
def test_probe_prefers_sibling_python_exe_on_windows_layouts():
    """pip on Windows puts Scripts/omnigraph(.exe) beside ..\\python.exe (or
...
# ── interpreter resolution under uv tool installs ───────────────────

def _detect_run(tmp_path, home, stub_bin, env_extra=None):
    """Run the emitted _PYTHON_DETECT under a real sh in a controlled
    environment — dead pin, no .omnigraph_python, an unparseable launcher first
    on PATH, and a python3 that cannot import omnigraph (#2852's uv-tool
    Windows machine reproduced on POSIX) — and report what OMNIGRAPH_PYTHON
    ...
    script.write_text(
        _PYTHON_DETECT + '\necho "RESOLVED=$OMNIGRAPH_PYTHON"\n',
...
def _broken_uv_machine(tmp_path):
    """#2852's machine: the only omnigraph-importable python lives in the uv
    tool venv; the launcher on PATH is a binary trampoline reached WITHOUT its
    .exe suffix (Git-Bash command -v); ambient python3 cannot import omnigraph.
    Returns (home, stub_bin)."""
    home = tmp_path / "home"
    stub_bin = tmp_path / "stubbin"
    stub_bin.mkdir(parents=True)
    launcher = stub_bin / "omnigraph"
    launcher.write_bytes(b"MZ\x90\x00\x03" + bytes(range(60)))
    launcher.chmod(0o755)
    # Ambient pythons answer the probe with "no module named omnigraph" —
    # under uv tool install no system python can see the isolated venv.
...
def _tool_venv(home, tool, rel, ok):
    ...
    ok=False simulates a venv without omnigraph (the probe must reject it)."""
...
def test_uv_tool_env_rescues_hook_when_pin_and_launcher_fail(tmp_path):
    """#2852: `uv tool install` puts omnigraph in an isolated venv no ambient
    ...
    mine = _tool_venv(home, "omnigraph", "bin/python", ok=True)
...
def test_uv_tool_env_honors_uv_tool_dir_and_windows_layout(tmp_path):
    ...
    py = tools / "omnigraph" / "Scripts" / "python.exe"
...
@pytest.mark.skipif(shutil.which("sh") is None, reason="sh required to run emitted probe chain")
def test_uv_tool_env_without_omnigraph_still_fails_loudly(tmp_path):
    """The scan must not adopt a tool env whose python lacks omnigraph; the
...
def test_file_path_allowlist_accepts_windows_backslash_path(winpath, tmp_path):
    """#2126: the .omnigraph_python FILE allowlist must accept real Windows paths
...
def test_shebang_allowlist_accepts_windows_backslash_path(shebang_path, tmp_path):
    ...
    pattern = _extract_case_pattern('OMNIGRAPH_PYTHON="" ;;')
    _assert_harness_can_reject(pattern, tmp_path)
...
def test_python_detect_allowlists_still_reject_shell_metacharacters(dangerous, tmp_path):
    ...
    for marker in ('_FROM_FILE=""', 'OMNIGRAPH_PYTHON="" ;;'):
...
def test_hooks_honor_skip_env(name, script):
    """OMNIGRAPH_SKIP_HOOK=1 must suppress BOTH hooks. post-checkout previously
    lacked the check, so the var stopped commit rebuilds but not branch-switch
    ones (#1809)."""
    assert '[ "${OMNIGRAPH_SKIP_HOOK:-0}" = "1" ] && exit 0' in script, (
        f"{name} does not honor OMNIGRAPH_SKIP_HOOK"
    )
...
def test_checkout_hook_skips_same_head_noop_at_runtime():
    ...
    from omnigraph.hooks import _CHECKOUT_SCRIPT
    ...
    # omnigraph-out check / detached launch so nothing is actually rebuilt.
    # The block runs inside a subshell; close it after the sentinel.
...
def _worktree_guard_snippet() -> str:
    from omnigraph.hooks import _WORKTREE_GUARD
...
# ── duplicate keys in.git/config must not trigger spurious warnings ──
...
# ── hook install must register the graph.json union merge driver ─────

def test_install_registers_merge_driver(tmp_path):
    """install() must set merge.omnigraph.* via git config and add the
    .gitattributes line that README/CHANGELOG 0.7.0 document (#1902)."""
    repo = _make_git_repo(tmp_path)
    result = install(repo)
    res = subprocess.run(
        ["git", "-C", str(repo), "config", "--get", "merge.omnigraph.driver"],
        capture_output=True, text=True,
    )
    ...
    assert any(
        "graph.json" in line and "merge=omnigraph" in line
...
def test_install_merge_driver_idempotent(tmp_path):
    ...
    matches = [l for l in lines if "merge=omnigraph" in l]
...
def test_install_preserves_existing_gitattributes(tmp_path):
    ...
    assert "merge=omnigraph" in content


def test_uninstall_removes_merge_driver_keeps_other_attrs(tmp_path):
    """uninstall() must unset merge.omnigraph.* and remove only the omnigraph
    ...
    content = (repo / ".gitattributes").read_text(encoding="utf-8")
    assert "*.png binary" in content
    assert "merge=omnigraph" not in content


@pytest.mark.parametrize("exe", [
    r"C:\Users\First Last\AppData\Roaming\uv\tools\omnigraph\Scripts\python.exe",
    r"C:\Program Files\Python312\python.exe",
    "/home/first last/.local/share/uv/tools/omnigraph/bin/python",
])
def test_pinned_python_accepts_paths_containing_spaces(exe, monkeypatch):
    ...
    from omnigraph.hooks import _pinned_python
...
def test_merge_driver_quotes_interpreter_with_spaces(tmp_path, monkeypatch):
    ...
    from omnigraph.hooks import install

    exe = r"C:\Users\First Last\AppData\Roaming\uv\tools\omnigraph\Scripts\python.exe"
    repo = _make_git_repo(tmp_path)
    monkeypatch.setattr(_sys, "executable", exe)
    install(repo)
    ...
    assert driver.endswith("-m omnigraph merge-driver %O %A %B")
...
def test_omnigraphrc_parsing(tmp_path):
    """Test 1: .omnigraphrc parsing for valid and invalid values."""
    from omnigraph.hooks import _load_omnigraphrc

    rc = tmp_path / ".omnigraphrc"
    rc.write_text("# comment\nviz_node_limit=0\n", encoding="utf-8")
    cfg = _load_omnigraphrc(tmp_path)
    ...
    with pytest.raises(ValueError, match="Invalid viz_node_limit"):
        _load_omnigraphrc(tmp_path)


def test_no_config_preserves_existing_hook(tmp_path):
    """Test 2: Without .omnigraphrc, generated hooks omit OMNIGRAPH_VIZ_NODE_LIMIT export."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    checkout_hook = (repo / ".git" / "hooks" / "post-checkout").read_text()
...
def test_baked_viz_limit_yields_to_an_explicit_per_run_override(tmp_path):
    ...
    OMNIGRAPH_VIZ_NODE_LIMIT: the baked line uses the `${VAR:-<n>}` default form,
    so an already-set env value wins (mirrors OMNIGRAPH_MAX_WORKERS)."""
    repo = _make_git_repo(tmp_path)
    (repo / ".omnigraphrc").write_text("viz_node_limit=100\n", encoding="utf-8")
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()

    # default form, not an unconditional assignment that would override the env
    assert 'export OMNIGRAPH_VIZ_NODE_LIMIT="${OMNIGRAPH_VIZ_NODE_LIMIT:-100}"' in commit_hook
    assert 'export OMNIGRAPH_VIZ_NODE_LIMIT="100"' not in commit_hook
    # prove the shell semantics: an explicit env value survives the export line
    line = 'export OMNIGRAPH_VIZ_NODE_LIMIT="${OMNIGRAPH_VIZ_NODE_LIMIT:-100}"'
    out = subprocess.run(
        ["sh", "-c", f'OMNIGRAPH_VIZ_NODE_LIMIT=7; {line}; echo "$OMNIGRAPH_VIZ_NODE_LIMIT"'],
        capture_output=True, text=True, check=True,
    )
    assert out.stdout.strip() == "7"


def test_status_survives_a_malformed_omnigraphrc(tmp_path):
    """A typo in the committed .omnigraphrc must not turn the read-only `status`
    diagnostic into a traceback; it reports the problem and continues."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    (repo / ".omnigraphrc").write_text("viz_node_limit=not-an-int\n", encoding="utf-8")
...
def test_changing_config_updates_existing_hook(tmp_path):
    """Test 4: Re-running install updates existing OmniGraph hook block with new config."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".omnigraphrc"
    ...
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    assert 'export OMNIGRAPH_VIZ_NODE_LIMIT="${OMNIGRAPH_VIZ_NODE_LIMIT:-0}"' in commit_hook
    assert 'export OMNIGRAPH_VIZ_NODE_LIMIT="${OMNIGRAPH_VIZ_NODE_LIMIT:-5000}"' not in commit_hook
    assert commit_hook.count("# omnigraph-hook-start") == 1


def test_user_hook_content_survives_update(tmp_path):
    """Test 5: User hook content outside omnigraph markers survives hook update."""
    repo = _make_git_repo(tmp_path)
    ...
    assert 'export OMNIGRAPH_VIZ_NODE_LIMIT="${OMNIGRAPH_VIZ_NODE_LIMIT:-0}"' in content
    assert "old block" not in content


def test_status_reports_configuration(tmp_path):
    """Test 6: omnigraph hook status exposes configured viz node limit and detects out-of-date hooks."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".omnigraphrc"
    rc.write_text("viz_node_limit=0\n", encoding="utf-8")
    install(repo)
...
def test_rebuild_bodies_tolerate_a_bom_in_omnigraph_root(name, body):
    ...
    assert "encoding='utf-8-sig'" in body, (
        f"{name} rebuild body must read .omnigraph_root BOM-tolerantly"
    )
...
@pytest.mark.parametrize("exe", [
    "/home/dev/snap/code/259/.local/share/uv/tools/omnigraph/bin/python",
    "/home/dev/snap/code/current/.local/share/uv/tools/omnigraph/bin/python",
    "/root/snap/pycharm-community/42/.local/share/uv/tools/omnigraph/bin/python",
])
def test_pinned_python_refuses_a_rotating_snap_revision(exe, monkeypatch):
    ...
    omnigraph installed from inside a snap-confined editor lives under
    ...
    not locate a Python with omnigraph installed" and the graphs quietly stopped
    tracking the code.
...
def test_pinned_python_still_pins_a_stable_path(monkeypatch):
    ...
    for exe in (
        "/home/dev/.local/share/uv/tools/omnigraph/bin/python",
        "/usr/bin/python3",
...
def test_hook_probes_snap_confined_uv_tool_dirs():
    ...
    """
    from omnigraph.hooks import _HOOK_SCRIPT
