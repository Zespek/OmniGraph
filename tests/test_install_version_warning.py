...
A plain `omnigraph install` refreshes only the detected platform, so a stale
`.omnigraph_version` at ANOTHER platform's destination made the warning
permanent: it named no path, its advice ("run omnigraph install") did not
...
from omnigraph.install import __version__
from omnigraph.install import _copy_skill_file, _platform_skill_destination
...
def _stale(tmp_path):
    d = tmp_path / "skills" / "omnigraph"
    d.mkdir(parents=True)
    (d / "SKILL.md").write_text("---\nname: omnigraph\n---\nbody\n", encoding="utf-8")
    (d / ".omnigraph_version").write_text("0.8.36", encoding="utf-8")
    return d / "SKILL.md"
...
def test_the_warning_names_the_stale_destination_and_the_exact_command(tmp_path, capsys):
    ...
    assert str(tmp_path / "skills" / "omnigraph") in err, "the stale path must be named"
    assert "omnigraph install --platform antigravity" in err
...
def test_without_a_platform_name_the_generic_command_is_kept(tmp_path, capsys):
    ...
    err = capsys.readouterr().err
    assert "omnigraph install" in err


def test_a_current_skill_stays_silent(tmp_path, capsys):
    skill = _stale(tmp_path)
    (skill.parent / ".omnigraph_version").write_text(__version__, encoding="utf-8")
...
def test_the_version_stamp_is_written_beside_the_skill(sandbox_home):
    _copy_skill_file("claude")
    dst = _platform_skill_destination("claude")
    assert (dst.parent / ".omnigraph_version").read_text(encoding="utf-8") == __version__
