...
from omnigraph.extractors.resolution import _resolve_js_module_path, _resolve_js_import_target
from omnigraph.extract import _resolve_rescued_specifier
...
def test_3357_minimal_reproduction_no_config_emits_calls_edge(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("omnigraph.detect._find_vcs_root", lambda start: None)
    adapter = _write(
        tmp_path / "adapter.js",
...
def test_unresolved_at_alias_does_not_infer_call_to_unrelated_definition(tmp_path: Path):
    ...
    happens to export a lone matching function missingFn(), OmniGraph must NOT emit
