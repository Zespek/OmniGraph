...
Run it manually; do NOT wire this into CI (wall-clock assertions are flaky):
    ...
    uv run python tests/bench_query_scoring.py \\
        --graph omnigraph-out/graph.json \\
...
def _load_real_graph(path: str) -> nx.Graph:
    ...
    `omnigraph-out/graph.json`, skipping the size cap and work-memory overlay
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--graph", default=None,
                   help="optional path to a real omnigraph-out/graph.json; overrides --nodes")
