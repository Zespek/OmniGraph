...
# tree-sitter-dm is an optional extra - it ships no Linux/Mac wheel, so it
...
def test_cpp_string_tests_punctuation_only_names_stay_distinct(tmp_path):
    ...
    r = extract_cpp(f)
    from omnigraph.extract import _make_id, _file_stem
...
def test_metal_is_code_extension():
    from omnigraph.detect import CODE_EXTENSIONS
...
def test_kotlin_enum_entries_have_case_of_edge():
    # (Kotlin half): enum entries must be nodes with case_of edges to the enum.
    r = extract_kotlin(FIXTURES / "sample.kt")
    labels = _labels(r)
...
def test_kotlin_emits_in_file_calls():
    ...
    `identifier` rename — see omnigraph-kmp's PythonParityTest."""
    r = extract_kotlin(FIXTURES / "sample.kt")
    calls = _calls(r)
...
def test_swift_no_dangling_edges():
    ...
    for e in r["edges"]:
        assert e["source"] in node_ids
        # targets must resolve to a node too, else build.py prunes the edge.
        assert e["target"] in node_ids, f"dangling target {e['target']} ({e['relation']})"


def test_swift_imports_survive_build():
    # `import Foundation` / `import UIKit` previously emitted edges to bare
    ...
    # Imported modules are tagged type=module (anchor nodes/).
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    r = extract(paths, cache_root=Path("/tmp/omnigraph-test-no-cache"))
...
from omnigraph.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from omnigraph.extract import extract_objc
...
def test_objc_header_dispatch_routes_objc_not_c(tmp_path):
    ...
    from omnigraph.extract import _get_extractor, extract_objc as _eo, extract_c as _ec
...
def test_objc_quoted_import_edges_resolve_to_real_nodes(tmp_path):
    ...
    for e in import_edges:
        ...
        # header file node, not get salted back to the importing `.m`.
...
def test_powershell_psm1_dispatched_and_extracted(tmp_path):
    #.psm1 modules were never indexed — no dispatch entry, no CODE_EXTENSIONS.
...
# ── PowerShell: Import-Module + dot-source ───────────────────────────
...
from omnigraph.extract import extract_markdown
...
# ── Header/impl class merge +.h routing (C++ ObjC/Swift) ─────────
from omnigraph.extract import (
...
# --- ObjC paired header/impl + bridging header -------------------------
...
# --- Swift extension folds onto canonical ObjC class -------------------
...
def test_markdown_frontmatter_fallback_parses_flat_keys():
    ...
    from omnigraph.extractors.markdown import _parse_frontmatter_fallback
...
# ── Zig ───────────────────────────────────────────────────────────────────────
from omnigraph.extract import extract_zig
...
def test_cl_ids_are_path_qualified_across_directories(tmp_path):
    ...
    they land in separate extract batches (what `omnigraph update` does) build()
...
# ---------------------------------------------------------------------------
# Robot Framework (.robot /.resource) - official robot.api parser
# ---------------------------------------------------------------------------
...
def test_robot_keyword_call_edges_incl_fixtures_and_loops():
    from omnigraph.extractors.robot import _kw_id
    r = extract_robot(FIXTURES / "sample.robot")
...
def test_robot_curdir_and_execdir_imports_resolve_without_double_prefix():
    ...
    # would double it for relative scan paths (bot finding on PR).
    # Pure path logic - needs no robotframework install.
    from pathlib import Path as P
    from omnigraph.extractors.robot import _resolve_robot_import
...
def test_robot_extractor_degrades_gracefully_without_robotframework():
    ...
    # omnigraph.extract must work and the extractor must return the
    ...
    script = (
        ...
        "from pathlib import Path\n"
        "from omnigraph.extract import extract_robot\n"
