"""`omnigraph merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "omnigraph"] + args, cwd=cwd,
...
def test_merge_graphs_mixed_directed_and_multigraph(tmp_path):
    a = tmp_path / "r1" / "omnigraph-out" / "graph.json"
    b = tmp_path / "r2" / "omnigraph-out" / "graph.json"
    c = tmp_path / "r3" / "omnigraph-out" / "graph.json"
...
def test_merge_graphs_same_named_repo_dirs_do_not_collapse(tmp_path):
    # two graphs under a same-named repo dir (src/omnigraph-out and
    # frontend/src/omnigraph-out both → tag "src") share the `src::` prefix, so a
    ...
    a = tmp_path / "src" / "omnigraph-out" / "graph.json"
    b = tmp_path / "frontend" / "src" / "omnigraph-out" / "graph.json"
...
def test_distinct_repo_tags_unit(tmp_path):
    from omnigraph.build import distinct_repo_tags
    # distinct repo dirs pass through unchanged
    assert distinct_repo_tags([
        Path("backend/omnigraph-out/graph.json"),
        Path("web/omnigraph-out/graph.json"),
    ]) == ["backend", "web"]
    # same-named repo dirs are widened to stay distinct
    tags = distinct_repo_tags([
        Path("proj/src/omnigraph-out/graph.json"),
        Path("proj/frontend/src/omnigraph-out/graph.json"),
    ])
    ...
    tags3 = distinct_repo_tags([
        Path("a/src/omnigraph-out/graph.json"),
        Path("b/src/omnigraph-out/graph.json"),
        Path("c/src/omnigraph-out/graph.json"),
    ])
    assert len(set(tags3)) == 3, tags3


def test_merge_graphs_preserves_import_edge_direction(tmp_path):
    # merge-graphs loaded graph.json into an undirected nx.Graph without
    ...
    a = tmp_path / "repo1" / "omnigraph-out" / "graph.json"
    b = tmp_path / "repo2" / "omnigraph-out" / "graph.json"
    a.parent.mkdir(parents=True, exist_ok=True)
    b.parent.mkdir(parents=True, exist_ok=True)
...
def test_merge_graphs_carries_hyperedges_from_all_inputs(tmp_path):
    # prefix_graph_for_global never rewrote G.graph["hyperedges"], and
    ...
    a = tmp_path / "alpha" / "omnigraph-out" / "graph.json"
    b = tmp_path / "beta" / "omnigraph-out" / "graph.json"
...
def test_merge_graphs_reads_top_level_only_hyperedges(tmp_path):
    # skew on the input side: node_link_graph restores only the nested
...
def test_merge_graphs_offsets_communities_so_repos_do_not_fuse(tmp_path):
    # every input numbers its communities from 0, so merge-graphs
