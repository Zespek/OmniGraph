...
A post-commit hook runs `omnigraph update . &` in a short-lived worktree; the
...
`omnigraph-out/cache/stat-index.json` — 81 of them over a few weeks.
"""
...
def test_the_index_is_still_written_for_a_live_run(tmp_path):
    """A first run writes the index before omnigraph-out/ exists at all; that
    stays as it was — the root is live, so creating cache/ under it is fine."""
    corpus = tmp_path / "c"
    corpus.mkdir()
    _dirty(corpus)
    cache._flush_stat_index()
    assert (corpus / "omnigraph-out" / "cache" / "stat-index.json").is_file()
