...
`omnigraph install --platform agents` (and the friendly `--platform skills`
...
The bare ``omnigraph install`` behaviour (claude/windows only) is unchanged; the
named platform is opt-in. The ``omnigraph agents install`` subcommand is the
...
def test_agents_user_destination_is_user_global_dot_agents(tmp_path):
    ...
    assert dst == tmp_path / ".agents" / "skills" / "omnigraph" / "SKILL.md"
...
def _run(tmp_path, argv, home):
    ...
    try:
        os.chdir(tmp_path)
        with patch.object(sys, "argv", ["omnigraph", *argv]):
...
def test_install_platform_agents_writes_user_global_skill_only(tmp_path, platform_arg):
    """`omnigraph install --platform agents|skills` writes ~/.agents/skills/...
    ...
    skill = home / ".agents" / "skills" / "omnigraph" / "SKILL.md"
    assert skill.exists()
    assert (skill.parent / ".omnigraph_version").read_text() == mainmod.__version__
...
def test_uninstall_platform_agents_removes_user_global_skill(tmp_path):
    """Bare `omnigraph uninstall` clears the ~/.agents/skills skill the AGENTS.md and
...
def test_uninstall_platform_flag_global_removes_skill(tmp_path, platform_arg):
    """`omnigraph uninstall --platform agents|skills` (global) clears ~/.agents/skills.
...
def test_project_uninstall_all_removes_agents_skill(tmp_path):
    """`omnigraph uninstall --project` (no platform) removes the agents project skill
    ...
    project_skill = proj / ".agents" / "skills" / "omnigraph" / "SKILL.md"
...
def test_install_platform_agents_project_writes_dot_agents(tmp_path):
    """`omnigraph install --project --platform agents` writes ./.agents/skills and
...
# --- the amp-twin subcommand (omnigraph agents install) -------------------------


def test_agents_subcommand_install_also_wires_agents_md(tmp_path):
    """`omnigraph agents install` is the amp-twin: skill at ~/.agents/skills PLUS a
    `## omnigraph` section in AGENTS.md (so the rendered hooks reference, which
    points at `omnigraph agents install`, stays honest)."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert "## omnigraph" in agents_md.read_text(encoding="utf-8")
    ...
    assert not agents_md.exists() or "## omnigraph" not in agents_md.read_text(encoding="utf-8")


def test_agents_subcommand_install_is_idempotent(tmp_path):
    """Running `omnigraph agents install` twice leaves a single AGENTS.md section."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert body.count("## omnigraph") == 1, "AGENTS.md gained a duplicate omnigraph section"


def test_skills_subcommand_is_the_agents_subcommand(tmp_path):
    """`omnigraph skills install`/`uninstall` behaves exactly like the agents form:
...
def test_bare_install_does_not_touch_dot_agents(tmp_path):
    """`omnigraph install` (no platform) stays single-platform claude/windows and
