"""Tests for omnigraph install --platform routing."""
import os
...
PLATFORMS = {
    "claude": (".claude/skills/omnigraph/SKILL.md",),
    "codebuddy": (".codebuddy/skills/omnigraph/SKILL.md",),
    "codex": (".codex/skills/omnigraph/SKILL.md",),
    "opencode": (".config/opencode/skills/omnigraph/SKILL.md",),
    "kilo": (
        ".config/kilo/skills/omnigraph/SKILL.md",
        ".config/kilo/command/omnigraph.md",
    ),
    "claw": (".openclaw/skills/omnigraph/SKILL.md",),
    "droid": (".factory/skills/omnigraph/SKILL.md",),
    "trae": (".trae/skills/omnigraph/SKILL.md",),
    "trae-cn": (".trae-cn/skills/omnigraph/SKILL.md",),
    "windows": (".claude/skills/omnigraph/SKILL.md",),
}
...
def test_install_default_claude(tmp_path):
    ...
    assert (tmp_path / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
...
def test_install_survives_a_winerror_17_replace(tmp_path, monkeypatch):
    ...
    skill = tmp_path / ".aider" / "omnigraph" / "SKILL.md"
    assert skill.exists()
...
def test_install_claude_md_defaults_to_home_when_config_dir_unset(tmp_path, monkeypatch):
    ...
    assert md.exists()
    assert "~/.claude/skills/omnigraph/SKILL.md" in md.read_text()
...
def test_install_survives_unwritable_claude_md(tmp_path, monkeypatch, capsys):
    ...
    assert (home / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists(), (
...
def test_install_survives_unwritable_codebuddy_md(tmp_path, monkeypatch, capsys):
    ...
    assert (home / ".codebuddy" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert target.read_text() == "# my rules\n"
...
def test_install_codebuddy(tmp_path):
    ...
    assert (tmp_path / ".codebuddy" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (
        tmp_path / ".config" / "opencode" / "skills" / "omnigraph" / "SKILL.md"
    ).exists()
...
def test_install_positional_platform_opencode(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "opencode"])
    with patch("omnigraph.__main__.Path.home", return_value=tmp_path):
        main()
    assert (tmp_path / ".config" / "opencode" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (tmp_path / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
...
def test_install_project_claude_writes_project_scope(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "--project"])
    with patch("omnigraph.__main__.Path.home", return_value=home):
        main()
    assert (project / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert (project / ".claude" / "CLAUDE.md").exists()
    assert not (home / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert ".claude/skills/omnigraph/SKILL.md" in (project / ".claude" / "CLAUDE.md").read_text()
    assert "~/.claude/skills/omnigraph/SKILL.md" not in (project / ".claude" / "CLAUDE.md").read_text()
...
def test_install_project_codex_writes_skill_and_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "--project", "--platform", "codex"])
    with patch("omnigraph.__main__.Path.home", return_value=home):
        main()
    assert (project / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    assert (project / ".codex" / "hooks.json").exists()
    assert not (home / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()
...
def test_claude_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".claude" / "skills" / "omnigraph" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "claude", "install", "--project"])
        main()
        assert (project / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
        assert (project / ".claude" / "CLAUDE.md").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["omnigraph", "claude", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
...
def test_codex_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".codex" / "skills" / "omnigraph" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "codex", "install", "--project"])
        main()
        assert (project / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()
        assert (project / "AGENTS.md").exists()
        assert (project / ".codex" / "hooks.json").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["omnigraph", "codex", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
    hooks_path = project / ".codex" / "hooks.json"
    assert hooks_path.exists()
    assert "omnigraph" not in hooks_path.read_text()
...
def test_antigravity_install_project_writes_project_skill(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "antigravity", "install", "--project"])
    with patch("omnigraph.__main__.Path.home", return_value=home):
        main()
    assert (project / ".agents" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (home / ".agents" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_help_does_not_install_default(tmp_path, monkeypatch, capsys):
    from omnigraph.__main__ import main
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "opencode", "--help"])
    with patch("omnigraph.__main__.Path.home", return_value=tmp_path):
        main()
    out = capsys.readouterr().out
    assert "Usage: omnigraph install" in out
...
def test_install_claw(tmp_path):
    ...
    assert (tmp_path / ".openclaw" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "omnigraph" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    skill = (Path(omnigraph.__file__).parent / "skill-codex.md").read_text()
    assert "spawn_agent" in skill


def test_codex_skill_uses_omnigraph_with_existing_graph():
    ...
    assert "skip Steps 1–5 entirely and jump straight to `## For /omnigraph query`" in skill
    assert "omnigraph query" in skill
    assert "omnigraph explain" in skill
    assert "omnigraph path" in skill


def test_codex_agents_install_mentions_dirty_graph_output(tmp_path):
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert "Dirty omnigraph-out/ files are expected" in content
    assert "not a reason to skip omnigraph" in content
...
def test_opencode_skill_contains_mention():
    ...
    skill = (Path(omnigraph.__file__).parent / "skill-opencode.md").read_text()
...
def test_kilo_skill_mentions_task_tool():
    ...
    skill = (Path(omnigraph.__file__).parent / "skill-kilo.md").read_text()
...
def test_claw_skill_uses_agent_tool_dispatch():
    ...
    skill = (Path(omnigraph.__file__).parent / "skill-claw.md").read_text()
    b2 = skill[skill.index("**Step B2"):skill.index("**Step B3")]
...
def test_all_skill_files_exist_in_package():
    ...
    pkg = Path(omnigraph.__file__).parent
    for name in (
        "skill.md",
        "skill-codex.md",
        "skill-opencode.md",
...
def test_codebuddy_install_writes_codebuddy_md(tmp_path):
    ...
    assert md.exists()
    assert "omnigraph-out/GRAPH_REPORT.md" in md.read_text()
...
def test_claude_hook_is_shell_agnostic(tmp_path):
    # the installed PreToolUse hooks must be plain exe invocations, not
    # POSIX bash (which fails on Windows cmd.exe/PowerShell).
    import json as _json
    from omnigraph.__main__ import _install_claude_hook
    ...
    for h in hooks:
        ...
        assert "omnigraph" in cmd and "hook-guard" in cmd
...
def test_claude_hook_install_idempotent_and_replaces_old_bash_hook(tmp_path):
    ...
    # Pre-seed a legacy bash-style omnigraph hook (the thing shipped before).
    settings_path.write_text(_json.dumps({"hooks": {"PreToolUse": [
        {"matcher": "Bash", "hooks": [{"type": "command",
         "command": "[ -f omnigraph-out/graph.json ] && echo '{...}' || true"}]},
    ]}}), encoding="utf-8")
    _install_claude_hook(tmp_path)
...
def test_codebuddy_install_merges_existing_codebuddy_md(tmp_path):
    ...
    assert "omnigraph-out/GRAPH_REPORT.md" in content
...
def test_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "--project", "--platform", "codex"])
        main()
        monkeypatch.setattr(sys, "argv", ["omnigraph", "uninstall", "--project", "--platform", "codex"])
        main()
    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
...
def test_uninstall_project_without_platform_removes_project_installs(tmp_path, monkeypatch):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["omnigraph", "uninstall", "--project"])
        main()
    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (project / ".claude" / "CLAUDE.md").exists()
...
def test_antigravity_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    # Global skill lives at ~/.gemini/config/skills/ (per fix)
    global_skill = home / ".gemini" / "config" / "skills" / "omnigraph" / "SKILL.md"
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "antigravity", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["omnigraph", "antigravity", "uninstall", "--project"])
        main()
    assert global_skill.exists(), "project uninstall must not touch global skill"
    assert not (project / ".agents" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_antigravity_global_install_writes_gemini_config_skills(tmp_path, monkeypatch):
    """Global `omnigraph antigravity install` must write to ~/.gemini/config/skills/ (#1079)."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "antigravity", "install"])
        main()
    global_skill = home / ".gemini" / "config" / "skills" / "omnigraph" / "SKILL.md"
    wrong_skill = home / ".agents" / "skills" / "omnigraph" / "SKILL.md"
    ...
    assert (project / ".agents" / "rules" / "omnigraph.md").exists()
    assert (project / ".agents" / "workflows" / "omnigraph.md").exists()


def test_antigravity_global_uninstall_removes_gemini_config_skill(tmp_path, monkeypatch):
    """Global `omnigraph antigravity uninstall` must remove from ~/.gemini/config/skills/ (#1079)."""
    from omnigraph.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["omnigraph", "antigravity", "uninstall"])
        main()
    assert not global_skill.exists(), f"skill not removed from {global_skill} after uninstall"
    # workspace files also cleaned up
    assert not (project / ".agents" / "rules" / "omnigraph.md").exists()
    assert not (project / ".agents" / "workflows" / "omnigraph.md").exists()
...
def _agents_install(tmp_path, platform):
    from omnigraph.__main__ import _agents_install as _install_fn

    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from omnigraph.__main__ import _agents_uninstall as _uninstall_fn

    _uninstall_fn(tmp_path, platform=platform)


def _kilo_install(project_dir, home_dir):
    from omnigraph.__main__ import _kilo_install as _install_fn

    with patch("omnigraph.__main__.Path.home", return_value=home_dir):
        _install_fn(project_dir)


def _kilo_uninstall(project_dir, home_dir):
    from omnigraph.__main__ import _kilo_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "omnigraph" in agents_md.read_text()
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained omnigraph section
...
def test_remove_marker_section_matches_exact_heading_only(tmp_path):
    """#2062: the strip helper must match omnigraph's own `## omnigraph` heading
    exactly, never a substring inside a user's `### omnigraph` H3."""
    from omnigraph.install import _remove_marker_section
    m = "## omnigraph"
    ...
    assert _remove_marker_section("see the ## omnigraph bullet\n", m) is None

    # A real H2 section alongside a user H3: remove only the H2 section.
    content = "# Doc\n\n### omnigraph\n\nmy notes\n\n## omnigraph\n\nomnigraph stuff\n"
    out = _remove_marker_section(content, m)
    assert out is not None
    assert "### omnigraph" in out and "my notes" in out
    assert not any(l.strip() == "## omnigraph" for l in out.splitlines())
    assert "omnigraph stuff" not in out
...
def test_agents_uninstall_preserves_user_h3_omnigraph_heading(tmp_path):
    """#2062 end-to-end: uninstall strips omnigraph's own H2 section but leaves a
    user-authored `### omnigraph` H3 (and everything else) byte-intact."""
    agents_md = tmp_path / "AGENTS.md"
    agents_md.write_text(
        "# My rules\n\n"
        "### omnigraph\n\n"
        "My own notes on how I use omnigraph. Keep this.\n\n"
        "## Other\n\nUnrelated content.\n"
    )
    _agents_install(tmp_path, "codex")  # appends a genuine `## omnigraph` H2 section
    assert "## omnigraph" in agents_md.read_text()

    _agents_uninstall(tmp_path)
    content = agents_md.read_text()
    assert "### omnigraph" in content, "user's H3 heading was deleted (#2062)"
    assert "My own notes on how I use omnigraph. Keep this." in content
    assert "## Other" in content and "Unrelated content." in content
    assert not any(l.strip() == "## omnigraph" for l in content.splitlines())


def test_uninstall_untouched_when_only_user_h3_present(tmp_path, capsys):
    """#2062: a file with only a user `### omnigraph` H3 (omnigraph never installed)
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/omnigraph.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "omnigraph.js"
...
def test_opencode_plugin_reminder_has_no_backticks(tmp_path):
    ...
    the very omnigraph command we are only suggesting.
    """
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "omnigraph.js"
...
def test_opencode_plugin_uses_semicolon_not_ampersand(tmp_path):
    ...
    _agents_install(tmp_path, "opencode")
    body = (tmp_path / ".opencode" / "plugins" / "omnigraph.js").read_text()
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("omnigraph.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("omnigraph.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("omnigraph.js" in p for p in config.get("plugin", []))
...
def test_kilo_agents_install_writes_plugin(tmp_path):
    _agents_install(tmp_path, "kilo")
    plugin = tmp_path / ".kilo" / "plugins" / "omnigraph.js"
    assert plugin.exists()
    assert "tool.execute.before" in plugin.read_text()
...
def test_kilo_agents_install_registers_plugin_in_config(tmp_path):
    ...
    config = _json.loads(config_file.read_text())
    assert (
        tmp_path / ".kilo" / "plugins" / "omnigraph.js"
...
def test_kilo_agents_install_idempotent(tmp_path):
    ...
    plugin_uri = (tmp_path / ".kilo" / "plugins" / "omnigraph.js").resolve().as_uri()
    assert content.count("## omnigraph") == 1
...
def test_kilo_install_writes_global_and_project_artifacts(tmp_path):
    ...
    assert (home_dir / ".config" / "kilo" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert (home_dir / ".config" / "kilo" / "command" / "omnigraph.md").exists()
    assert (project_dir / "AGENTS.md").exists()
    assert (project_dir / ".kilo" / "plugins" / "omnigraph.js").exists()
...
def test_kilo_uninstall_removes_plugin_registration_and_command(tmp_path):
    ...
    assert not (home_dir / ".config" / "kilo" / "command" / "omnigraph.md").exists()
    assert not (
        home_dir / ".config" / "kilo" / "skills" / "omnigraph" / "SKILL.md"
    ).exists()
    assert not (project_dir / ".kilo" / "plugins" / "omnigraph.js").exists()
    config_file = project_dir / ".kilo" / "kilo.json"
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert (
            project_dir / ".kilo" / "plugins" / "omnigraph.js"
        ).resolve().as_uri() not in config.get("plugin", [])
...
def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/omnigraph.mdc."""
    from omnigraph.__main__ import _cursor_install

    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "omnigraph.mdc"
    assert rule.exists()
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from omnigraph.__main__ import _cursor_install, _cursor_uninstall
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from omnigraph.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from omnigraph.__main__ import gemini_install
...
def test_gemini_uninstall_removes_section(tmp_path):
    from omnigraph.__main__ import gemini_install, gemini_uninstall
...
def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from omnigraph.__main__ import gemini_uninstall

    gemini_uninstall(tmp_path)  # should not raise


def test_amp_user_install_lands_in_config_agents(tmp_path, monkeypatch):
    """`omnigraph amp install` (user scope) must drop the skill into an Amp search
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "amp", "install"])
    with patch("omnigraph.__main__.Path.home", return_value=home):
        main()

    correct = home / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md"
    old = home / ".amp" / "skills" / "omnigraph" / "SKILL.md"
...
def test_amp_install_cleans_legacy_amp_skills_dir(tmp_path, monkeypatch):
    """A pre-fix ~/.amp/skills/omnigraph install is removed on the next install."""
    from omnigraph.__main__ import main

    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    legacy = home / ".amp" / "skills" / "omnigraph"
    ...
    assert not legacy.exists(), "legacy ~/.amp/skills/omnigraph should be cleaned up"
    assert (home / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_amp_user_uninstall_removes_skill_and_agents(tmp_path, monkeypatch):
    """`omnigraph amp uninstall` removes the user-scope skill and AGENTS.md section."""
    from omnigraph.__main__ import main
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["omnigraph", "amp", "install"])
        main()
        skill = home / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md"
        assert skill.exists()

        monkeypatch.setattr(sys, "argv", ["omnigraph", "amp", "uninstall"])
        main()
...
def test_amp_project_install_lands_in_dot_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["omnigraph", "amp", "install", "--project"])
    with patch("omnigraph.__main__.Path.home", return_value=home):
        main()

    assert (project / ".agents" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert not (project / ".amp" / "skills" / "omnigraph" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    # User scope untouched.
    assert not (home / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md").exists()


def test_uninstall_all_removes_amp_user_skill(tmp_path, monkeypatch):
    """The user-scope `omnigraph uninstall` enumeration removes the amp skill."""
    from omnigraph.__main__ import main
...
def test_hermes_skill_destination_windows_uses_localappdata():
    ...
    with patch("omnigraph.__main__.platform.system", return_value="Windows"), \
         patch.dict(os.environ, {"LOCALAPPDATA": str(Path("/tmp/AppDataLocal"))}):
        dst = _platform_skill_destination("hermes", project=False)
    assert dst == Path("/tmp/AppDataLocal") / "hermes" / "skills" / "omnigraph" / "SKILL.md", dst


def test_hermes_skill_destination_posix_uses_home():
    """Non-Windows hermes destination is unchanged (~/.hermes/skills)."""
    from omnigraph.__main__ import _platform_skill_destination
    with patch("omnigraph.__main__.platform.system", return_value="Linux"):
        dst = _platform_skill_destination("hermes", project=False)
    assert str(dst).endswith(".hermes/skills/omnigraph/SKILL.md"), dst


def _cli_dispatched_commands() -> set[str]:
    """Subcommand names the CLI actually dispatches.

    `omnigraph`'s dispatcher is an `elif cmd == "..."` chain rather than a declarative
...
def test_codex_hook_command_is_a_real_cli_subcommand(tmp_path):
    ...
    from omnigraph.install import _install_codex_hook
    ...
    entries = [
        ...
        if "omnigraph" in h.get("command", "")
    ]
    assert entries, "codex install must register a omnigraph PreToolUse hook"
...
# --- project-scoped installs must not embed a machine-absolute path ----
#
...
# hook must name `omnigraph` and let PATH resolve it, the way the git-hook layer
# already does with `command -v omnigraph` (hooks.py). The user-profile install
...
def _run_project_install(project, home, platform):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        with patch("sys.argv", ["omnigraph", "install", "--project", "--platform", platform]):
            main()
...
def test_project_install_hook_command_is_portable(tmp_path, monkeypatch, platform):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    # Resolution would otherwise find a real omnigraph on this machine; pin it so
    # the assertion fails loudly if the project path ever resolves again.
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\Users\installer\omnigraph.EXE")
    ...
    for command in commands:
        assert command.startswith("omnigraph "), command
...
def test_user_profile_install_still_resolves_absolute_path(tmp_path, monkeypatch, platform):
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        with patch("sys.argv", ["omnigraph", platform, "install"]):
            main()

    commands = _hook_commands((project / _PROJECT_HOOK_FILES[platform]).read_text(encoding="utf-8"))
    assert commands, f"{platform} install registered no hook command"
    for command in commands:
        assert command.startswith("C:/Users/installer/omnigraph.EXE "), command
...
def test_project_uninstall_removes_the_bare_hook_command(tmp_path, monkeypatch):
    """The uninstall filter matches on "omnigraph", so a bare command still goes."""
    from omnigraph.__main__ import main
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        with patch("sys.argv", ["omnigraph", "claude", "uninstall", "--project"]):
            main()

    assert not [c for c in _hook_commands(settings.read_text(encoding="utf-8")) if "omnigraph" in c]
