"""Tests for semantic evidence-binding in omnigraph.llm.
...
def test_node_attributed_to_undispatched_file_is_left_to_out_of_scope(tmp_path):
    ...
    out = _by_label(_run([src], nodes, tmp_path))
    # Not in the dispatched set ->'s domain, not evidence-binding's.
...
def test_diagnostics_reports_unverified_node_count():
    ...
    from omnigraph.diagnostics import diagnose_extraction, format_diagnostic_report
