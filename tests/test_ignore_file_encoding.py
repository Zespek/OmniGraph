...
`_load_dir_own_ignore` / `_load_omnigraphignore` read .gitignore,
.omnigraphignore and $GIT_DIR/info/exclude with `errors="ignore"`, which turns a
...
def test_ansi_encoded_rule_still_excludes(tmp_path):
    """The reported case: a cp1252 .omnigraphignore must still exclude."""
...
def test_ansi_encoded_rule_warns_once_naming_the_file(tmp_path, capsys):
    import omnigraph.detect as detect_mod
    detect_mod._warned_ignore_encodings.clear()
    _corpus(tmp_path, f"{NAME}/\n".encode("cp1252"))
    detect(tmp_path)
    err = capsys.readouterr().err
    assert ".omnigraphignore" in err and "UTF-8" in err, err
...
def test_ascii_rules_are_untouched(tmp_path):
    (tmp_path / ".omnigraphignore").write_bytes(b"vendor/\n")
...
def test_utf16_bom_encoded_rule_still_excludes(tmp_path):
    """A UTF-16 (BOM) .omnigraphignore — what PowerShell Set-Content and Notepad
