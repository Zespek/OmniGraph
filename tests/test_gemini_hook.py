...
Since #522 it runs as `omnigraph hook-guard gemini` (not a `python -c` one-liner
...
from omnigraph.__main__ import _gemini_hook
...
def _run(cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "omnigraph", "hook-guard", "gemini"],
...
def test_matcher_and_command_shape():
    ...
    # no bare `python` dependency, no embedded quote/backtick soup.
    assert "python -c" not in cmd
    assert "omnigraph" in cmd and "hook-guard gemini" in cmd
...
def test_honors_omnigraph_out_override(tmp_path):
    ...
    assert "omnigraph query" in json.loads(r.stdout).get("additionalContext", "")
