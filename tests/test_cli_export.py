"""Integration tests for omnigraph export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/omnigraph-out/."""
    out = tmp_path / "omnigraph-out"
    out.mkdir()

    extraction = json.loads((FIXTURES / "extraction.json").read_text())
    from omnigraph.build import build_from_json
    from omnigraph.cluster import cluster, score_all
    from omnigraph.analyze import god_nodes, surprising_connections
    from omnigraph.export import to_json
    ...
    (out / ".omnigraph_analysis.json").write_text(json.dumps(analysis))
...
# ── omnigraph export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_omnigraph_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["OMNIGRAPH_OUT"] = custom_out.name
...
def test_extract_writes_to_omnigraph_out_env(tmp_path):
    ...
    assert not (tmp_path / "omnigraph-out").exists(), "extract ignored OMNIGRAPH_OUT and wrote omnigraph-out/"
    # Manifest keys are relative to the scan root (portable) —.
...
def test_path_uses_omnigraph_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_omnigraph_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# ── omnigraph export unknown format ───────────────────────────────────────────
...
# Regression test for - cluster-only crashes when omnigraph-out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when omnigraph-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default omnigraph-out/ location
    # so we can point --graph at it while omnigraph-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "omnigraph-out" / "GRAPH_REPORT.md").exists()


def test_cluster_only_graph_in_omnigraph_out_writes_beside_it(tmp_path):
    """#1747 Case 2: `cluster-only --graph <elsewhere>/omnigraph-out/graph.json`
...
def test_cluster_only_persists_analysis_sidecar(tmp_path):
    ...
    """
    out = _make_graph(tmp_path)
    analysis_path = out / ".omnigraph_analysis.json"
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .omnigraph_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".omnigraph_labels.json"
...
# Without these tests, `omnigraph export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    ...
    (out / ".omnigraph_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".omnigraph_analysis.json").read_text(encoding="utf-8"))
...
def test_graph_json_node_ids_are_portable_across_checkout_paths(tmp_path):
    ...
    def _build(root: Path):
        ...
        assert r.returncode == 0, r.stderr
        data = json.loads((root / "omnigraph-out" / "graph.json").read_text())
...
# ── omnigraph query names its graph ───────────────────────────────────
...
def test_query_command_header_names_the_graph(tmp_path):
    ...
    assert first_line.startswith("Graph: omnigraph-out/graph.json ("), first_line
