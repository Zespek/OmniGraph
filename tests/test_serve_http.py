...
from omnigraph import serve as serve_mod  # noqa: E402
...
def test_app_builds_and_initialize_succeeds(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert payload["result"]["serverInfo"]["name"] == "omnigraph"
...
def test_api_key_bearer_ok(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert resp.status_code == 200
        assert resp.json()["result"]["serverInfo"]["name"] == "omnigraph"
...
def _project_with_graph(tmp_path, node_count: int, name: str = "proj") -> str:
    """Create ``<proj>/omnigraph-out/graph.json`` and return the project dir."""
    proj = tmp_path / name
    (proj / "omnigraph-out").mkdir(parents=True)
    graph = {
        "directed": True,
        "nodes": [{"id": f"n{i}", "label": f"N{i}", "community": 0} for i in range(node_count)],
        "edges": [],
    }
    (proj / "omnigraph-out" / "graph.json").write_text(json.dumps(graph), encoding="utf-8")
    return str(proj)
...
def test_max_server_contexts_parsing(monkeypatch, value, expected):
    if value is None:
        monkeypatch.delenv("OMNIGRAPH_MAX_CONTEXTS", raising=False)
    else:
        monkeypatch.setenv("OMNIGRAPH_MAX_CONTEXTS", value)
...
def test_project_context_cache_is_lru_and_pins_default_graph(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNIGRAPH_MAX_CONTEXTS", "2")
    ...
    first_graph = str((Path(projects[0]) / "omnigraph-out" / "graph.json").resolve())
    second_graph = str((Path(projects[1]) / "omnigraph-out" / "graph.json").resolve())
...
def test_corrupt_project_graph_is_a_tool_error_without_killing_server(tmp_path):
    ...
    (project / "omnigraph-out" / "graph.json").write_text("{not json", encoding="utf-8")
    app = serve_mod._build_http_app(_graph_file(tmp_path), json_response=True)
    with _client(app) as client:
        headers = _init_session(client)
...
def test_cli_defaults_to_stdio(monkeypatch):
    ...
    serve_mod._main(["omnigraph-out/graph.json"])
    assert calls.get("stdio") == "omnigraph-out/graph.json"
...
def test_cli_api_key_from_env(monkeypatch):
    captured = {}
    monkeypatch.setenv("OMNIGRAPH_API_KEY", "from-env")
...
def test_pr_tool_failure_sets_iserror(tmp_path, monkeypatch):
    ...
    import omnigraph.prs as prs_mod
