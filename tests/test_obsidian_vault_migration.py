...
#1506 added `.omnigraph_obsidian_manifest.json` so a re-export can update its own
notes while refusing to touch the user's. A vault created by an EARLIER omnigraph
has no manifest, so `_owned` starts empty and every note omnigraph itself wrote
last time reads as a user file: the re-export writes fresh notes beside the
stale ones, and warns that omnigraph "did not create" files it did (#2863).

The fix adopts, once, the notes carrying omnigraph's own signature:

  * node notes    - YAML frontmatter with a tag in the ``omnigraph/`` namespace
  * community notes - no frontmatter at all, so matched on omnigraph's own
                      filename prefix AND the Dataview query it writes

`.obsidian/graph.json` is deliberately NOT adopted: omnigraph writes one, but so
...
from omnigraph.export import to_obsidian

try:
    from omnigraph.export import _adopt_pre_manifest_notes, _is_omnigraph_note
except ImportError:  # pre-fix tree
    _adopt_pre_manifest_notes = _is_omnigraph_note = None

MANIFEST = ".omnigraph_obsidian_manifest.json"
...
def pre_manifest_vault(tmp_path):
    """A vault as an older omnigraph would have left it: its notes, no manifest."""
...
def test_omnigraph_no_longer_claims_it_did_not_write_its_own_notes(pre_manifest_vault):
...
def test_a_note_merely_mentioning_omnigraph_is_not_adopted(pre_manifest_vault):
    mine = pre_manifest_vault / "Notes.md"
    mine.write_text("# Notes\n\nI use omnigraph/document tags manually.\n", encoding="utf-8")
    _export(pre_manifest_vault, ["Alpha", "Beta", "Gamma"])
    assert "I use omnigraph" in mine.read_text(encoding="utf-8")
...
def test_obsidian_config_is_not_adopted(pre_manifest_vault):
    """omnigraph writes .obsidian/graph.json, but so does Obsidian. With no
...
@pytest.mark.skipif(_is_omnigraph_note is None, reason="pre-fix tree")
def test_detector_accepts_a_omnigraph_node_note(tmp_path):
    p = tmp_path / "n.md"
    p.write_text('---\nsource_file: "a.md"\ntags:\n  - omnigraph/document\n---\n\n# N\n',
                 encoding="utf-8")
    assert _is_omnigraph_note(p)
...
def test_detector_rejects_everything_else(tmp_path, body):
    ...
    assert not _is_omnigraph_note(p)
