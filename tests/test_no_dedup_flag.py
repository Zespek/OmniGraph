"""`omnigraph extract --no-dedup` (#2881).
...
def _capture_dedup(monkeypatch):
    ...
    Patching `omnigraph.build` does reach the CLI's `_build` / `_build_merge`
    aliases, even though it refers to them by those names: the

        from omnigraph.build import build as _build, build_merge as _build_merge
...
def _assert_spied(seen: dict, entry_point: str) -> None:
    ...
    assert entry_point in seen, (
        ...
        f"omnigraph.build symbol — the spy is inert and every dedup assertion "
