...
build_merge backs `omnigraph --update`. Two regressions covered here:
...
from omnigraph.build import build_merge, _infer_merge_root
...
# ── root-less prune (absolute deleted paths vs relative node keys) ──────

def test_prune_without_root_removes_ghost_nodes_via_grandparent_fallback(tmp_path):
    root = tmp_path / "corpus"
    (root / "omnigraph-out").mkdir(parents=True)
    graph_path = root / "omnigraph-out" / "graph.json"
    nodes = [
...
def test_prune_without_root_uses_omnigraph_root_marker(tmp_path):
    # graph.json not under a <root>/omnigraph-out layout, so grandparent wouldn't
    # help — the committed .omnigraph_root marker must be honored instead.
    out = tmp_path / "out"
    out.mkdir()
    graph_path = out / "graph.json"
    real_root = tmp_path / "elsewhere" / "repo"
    real_root.mkdir(parents=True)
    (out / ".omnigraph_root").write_text(str(real_root), encoding="utf-8")
...
def test_prune_matches_across_symlinked_root(tmp_path):
    ...
    real = tmp_path / "real"
    (real / "omnigraph-out").mkdir(parents=True)
    link = tmp_path / "link"
    os.symlink(real, link)
    graph_path = real / "omnigraph-out" / "graph.json"
...
# ── form-insensitive prune (absolute node vs relative prune, and back) ──
...
def test_prune_matches_node_stored_absolute_against_relative_delete(tmp_path):
    ...
    nodes = [
        ...
        # it that way); keep.py's is relative.
    ...
    # Runbook-style: NO root passed (eff_root inferred from the omnigraph-out
...
def test_omnigraph_root_marker_with_a_utf8_bom_still_resolves(tmp_path):
    ...
    real_root = tmp_path / "elsewhere" / "repo"
    real_root.mkdir(parents=True)
    (out / ".omnigraph_root").write_bytes(
