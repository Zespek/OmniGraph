...
`omnigraph hook-guard read` subcommand (not inline bash), so it works on Windows
too. These tests invoke that subcommand with crafted stdin JSON and assert it
nudges only for a source/doc file outside omnigraph-out/ when a graph exists, and
...
def _env():
    # The guard resolves the graph via OMNIGRAPH_OUT (default "omnigraph-out",
...
def test_command_has_no_shell_syntax():
    # the command must be a plain exe invocation, not POSIX bash.
    ...
    assert "omnigraph" in cmd and "hook-guard read" in cmd
...
def test_silent_on_omnigraph_out_targets(tmp_path):
    """Reading the graph's own report must not start a go-read-the-graph loop."""
    out = _run({"file_path": "omnigraph-out/GRAPH_REPORT.md"}, tmp_path, graph=True).stdout
    assert out.strip() == ""
...
def test_nudges_on_framework_source(tmp_path):
    ...
    for path in ("src/components/Hero.astro", "src/App.vue", "src/Card.svelte"):
        out = _run({"file_path": path}, tmp_path, graph=True).stdout
        assert "omnigraph query" in out, f"{path} should nudge"
