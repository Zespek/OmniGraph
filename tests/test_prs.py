...
class TestFetchWorktrees:
    def test_normal_case_maps_branch_to_path(self):
        ...
        with patch("omnigraph.prs.subprocess.run", return_value=mock_result):
...
class TestDetectDefaultBranch:
    ...
    def test_falls_back_to_git_symbolic_ref(self):
        ...
        with patch("omnigraph.prs._gh", return_value=None), patch(
            "omnigraph.prs.subprocess.run", return_value=mock_result
        ):
    ...
    def test_gh_returns_empty_dict_falls_back(self):
        ...
        with patch("omnigraph.prs._gh", return_value={}), patch(
            "omnigraph.prs.subprocess.run", return_value=mock_result
        ):
    ...
    def test_explicit_repo_gh_fails_returns_main_without_local_git(self):
        ...
        with patch("omnigraph.prs._gh", return_value=None), \
             patch("omnigraph.prs.subprocess.run") as mock_git:
            branch = _detect_default_branch(repo="owner/repo")
        assert branch == "main"
...
# ── Windows cp1252 subprocess-decode hardening (decode-side sibling of) ──
