"""Tests for omnigraph/cache.py."""
import pytest
from pathlib import Path
from omnigraph.cache import file_hash, cache_dir, load_cached, save_cached, cached_files, clear_cache, _body_content
...
def test_clear_cache(tmp_file, cache_root):
    """clear_cache removes all .json files from omnigraph-out/cache/ (all subdirs)."""
    save_cached(tmp_file, {"nodes": [], "edges": []}, root=cache_root)
    # Since v0.5.3 entries go into cache/ast/, not the flat cache/ dir
    cache_base = cache_root / "omnigraph-out" / "cache"
...
# --- frontmatter delimiters must be whole `---` lines -----------------
...
# --- portable cache source_file fields --------------------------------
# ``save_cached`` relativizes ``source_file`` entries inside the cache file
# so a committed ``omnigraph-out/cache/`` is portable across machines and
...
def test_save_cached_relativizes_source_file(tmp_path):
    ...
    import json
    from omnigraph.cache import save_cached, file_hash, cache_dir
...
def test_load_cached_absolutizes_source_file(tmp_path):
    ...
    from omnigraph.cache import save_cached, load_cached
...
def test_load_cached_passes_through_legacy_absolute_source_file(tmp_path):
    """Cache entries written by an older omnigraph (with absolute source_file
    inside) must still load correctly: the absolutize step is a no-op for
    already-absolute values."""
    import json
    from omnigraph.cache import load_cached, file_hash, cache_dir
...
def test_cache_portable_across_roots(tmp_path):
    ...
    import json
    import shutil
    from omnigraph.cache import save_cached, load_cached, file_hash, cache_dir
...
def test_warm_cache_from_another_root_does_not_leak_that_root(tmp_path, monkeypatch):
    ...
    AND omnigraph-out to root B, extract under B on the warm cache.
    ...
    import omnigraph.extract as ex
    ...
    entries = sorted((corpus_a / "omnigraph-out" / "cache" / "ast").rglob("*.json"))
    ...
    # Move the corpus; omnigraph-out/ (cache + stat index) rides along. copy2
    ...
    paths_b = sorted(p for p in corpus_b.rglob("*") if p.is_file()
                     and "omnigraph-out" not in p.parts)
    ...
    monkeypatch.undo()
    shutil.rmtree(corpus_b / "omnigraph-out")
    _reset_stat_index()
...
def test_cached_ids_round_trip_under_the_same_root(tmp_path):
    ...
    from omnigraph.extract import _make_id
...
# are only valid for the omnigraph version that wrote them. Keying purely on
...
def test_ast_cache_invalidated_on_version_bump(tmp_path, monkeypatch):
    ...
    import omnigraph.cache as cache_mod
    ...
    assert load_cached(f, root=tmp_path, kind="ast") is None, (
        "AST cache entry from a previous omnigraph version must not be served"
    )
...
def test_ast_cache_schema_rejects_same_version_legacy_collision(
    ...
    old_dir = tmp_path / cache_mod._OMNIGRAPH_OUT / "cache" / "ast" / "v0.9.46"
...
def test_legacy_unversioned_ast_entries_not_served(tmp_path):
    """Entries written by pre-versioning omnigraph (flat cache/ or unversioned
    ...
    import json
    from omnigraph.cache import file_hash, _OMNIGRAPH_OUT
    ...
    unversioned = tmp_path / _OMNIGRAPH_OUT / "cache" / "ast"
...
def test_file_hash_distinguishes_walked_symlink_paths_portably(
    ...
    from omnigraph import cache as cache_mod
...
def test_semantic_cache_self_heals_legacy_symlink_collision(
    ...
    from omnigraph.cache import check_semantic_cache, save_semantic_cache
...
def test_semantic_symlink_policy_uses_walked_identity(
    ...
    from omnigraph.cache import load_cached, save_semantic_cache
...
def test_semantic_prune_removes_orphan_entries(tmp_path):
    ...
    current one."""
    from omnigraph.cache import prune_semantic_cache
...
# --- mode-namespaced semantic cache -----------------------------------
...
def test_semantic_cache_mode_none_layout_unchanged(tmp_path):
    ...
    h = file_hash(f, tmp_path)
    assert (tmp_path / "omnigraph-out" / "cache" / "semantic" / f"{h}.json").exists()
    assert not (tmp_path / "omnigraph-out" / "cache" / "semantic-deep").exists(), (
...
def test_clear_cache_removes_deep_namespace(tmp_path):
    ...
    save_semantic_cache([{"id": "d", "source_file": "doc.md"}], [],
                        root=tmp_path, mode="deep")
    base = tmp_path / "omnigraph-out" / "cache"
...
def test_semantic_prune_sweeps_both_namespaces_against_same_live_set(tmp_path):
    ...
    from omnigraph.cache import prune_semantic_cache, save_semantic_cache
...
# --- extraction-prompt fingerprinting -------------------------------
...
def test_prompt_fingerprint_stable_and_prompt_sensitive(tmp_path):
    ...
    from omnigraph.cache import prompt_fingerprint
...
def test_semantic_cache_prompt_namespaced_layout(tmp_path):
    ...
    from omnigraph.cache import prompt_fingerprint, save_semantic_cache
    ...
    sem = tmp_path / "omnigraph-out" / "cache" / "semantic"
    h = file_hash(f, tmp_path)
...
def test_semantic_cache_prompt_and_mode_compose(tmp_path):
    ...
    deep = tmp_path / "omnigraph-out" / "cache" / "semantic-deep"
...
def test_semantic_prune_and_clear_reach_fingerprint_subdirs(tmp_path):
    ...
    save_semantic_cache([{"id": "n", "source_file": "doc.md"}], [],
                        root=tmp_path, prompt="PROMPT V1")
    clear_cache(tmp_path)
    assert not list((tmp_path / "omnigraph-out" / "cache" / "semantic").glob("**/*.json"))
...
# --- zero-node semantic cache rejection and healing -------------------
...
def test_edge_only_semantic_result_not_cached(tmp_path):
    ...
    from omnigraph.cache import check_semantic_cache, load_cached, save_semantic_cache
...
def test_poisoned_edge_only_cache_entry_treated_as_miss(tmp_path):
    ...
    import json
    from omnigraph.cache import cache_dir, file_hash, load_cached, prompt_fingerprint
...
def test_existing_hyperedge_only_cache_entry_remains_hit(tmp_path):
    ...
    assert loaded is not None
    assert len(loaded["hyperedges"]) == 1
# --- graph-side scope filter -------------------------------------------
# The guard protects the cache write, but the unfiltered fresh result
...
def test_scope_semantic_result_drops_out_of_scope_groups(tmp_path):
    ...
    from omnigraph.cache import scope_semantic_result
