"""Tests for omnigraph/dedup.py entity deduplication pipeline."""
from __future__ import annotations
import pytest
from omnigraph.dedup import (
...
def test_build_calls_dedup():
    ...
    from omnigraph.build import build
    chunk1 = {
...
# --- fuzzy dedup false merges on short/variant labels ---
...
def test_dedup_still_merges_real_typos():
    ...
    from omnigraph.dedup import _is_variant_pair, _short_label_blocked
...
def test_variant_pair_helper():
    ...
    from omnigraph.dedup import _is_variant_pair
...
def test_pass2_winner_union_does_not_pull_in_uncompared_same_label_nodes():
    ...
    # source files is exactly what the guards keep separate.
    assert "sm" in ids, (
...
def test_prefix_guard_does_not_block_same_length_typos():
    ...
    from omnigraph.dedup import _norm
...
# ── JaroWinkler prefix-bonus over-merge (cross-file) ──────────────────
...
# ── cross-chunk node ID collision warning ──────────────────────────────
...
def test_defines_id_helper():
    ...
    # Pre- IDs keyed off the bare filename stem.
...
# ── lifecycle-aware, environment-stable collision ranking ──────────────
# Active-vs-archived path markers adapted from @michaelxer's PR, judged on
...
# ── review: attribute-merge correctness (fixes A-D) ─────────────────────
...
# ── cross-file exact-duplicate concepts must merge ─────────────────────
...
@pytest.mark.parametrize("a,b", [
    # narrowed the document/rationale rows here to their boundary case: an
...
# ── same-file labels differing by a content-word swap ──────────────────
# Guard adopted from @wilyan09007's PR, hardened: any same-position
...
def test_content_token_swap_helper():
    ...
    from omnigraph.dedup import _content_token_swap
    ...
    # DELTA over two differing content-word positions block too (the PR
...
def test_same_word_variant_helper():
    ...
    from omnigraph.dedup import _same_word_variant
...
# -- cross-file merge for entity nodes typed by their file's extension --
#
...
def test_reads_as_file_entity_helper():
    ...
    from omnigraph.dedup import _reads_as_file_entity
    ...
    assert not _reads_as_file_entity(
        {"id": "2024_03_01", "source_file": "journal/2024-03-01.md"})  # pre- bare stem
    assert not _reads_as_file_entity(
