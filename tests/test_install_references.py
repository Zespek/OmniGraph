...
The real claude bundle now ships in the package (omnigraph/skills/claude/), so
...
PKG_DIR = Path(omnigraph.__file__).parent
...
def test_install_stages_references_sidecar(tmp_path, fake_bundle):
    ...
    skill_dir = tmp_path / ".claude" / "skills" / "omnigraph"
...
def test_single_version_stamp_covers_skill_and_references(tmp_path, fake_bundle):
    """One .omnigraph_version stamp versions SKILL.md + references/ together."""
    platform = fake_bundle
    _install(tmp_path, platform)
    skill_dir = tmp_path / ".claude" / "skills" / "omnigraph"
    stamps = list(skill_dir.rglob(".omnigraph_version"))
    assert len(stamps) == 1
    assert stamps[0] == skill_dir / ".omnigraph_version"
...
def test_check_skill_version_ignores_permission_error(tmp_path, fake_bundle, monkeypatch, capsys):
    ...
    def guarded_exists(self):
        if self.name == ".omnigraph_version":
...
def test_claude_install_ships_lean_core_and_references(tmp_path):
    ...
    assert (skill_dir / ".omnigraph_version").read_text() == mainmod.__version__
...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    skill = tmp_path / ".gemini" / "skills" / "omnigraph" / "SKILL.md"
    assert skill.exists()
...
def test_pyproject_declares_references_globs():
    ...
    The references sidecar ships under omnigraph/skills/<host>/references/ and the
    always-on injection blocks under omnigraph/always_on/. The earlier
    skills/*/SKILL.md glob matched nothing (no omnigraph/skills/<host>/SKILL.md
    exists; the skill bodies are omnigraph/skill*.md, listed explicitly), so it was
    ...
    pkg_data = data["tool"]["setuptools"]["package-data"]["omnigraph"]
...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    assert "omnigraph/skills/claude/references/extraction-spec.md" in names
    assert "omnigraph/skills/trae/references/hooks.md" in names
    # amp is now a split host too; its bundle must ship like every other.
    assert "omnigraph/skills/amp/references/hooks.md" in names
...
def test_monolith_install_clears_orphan_references(tmp_path, fake_bundle):
    ...
    skill_dst = tmp_path / ".aider" / "omnigraph" / "SKILL.md"
...
def test_amp_user_install_carries_references(tmp_path, monkeypatch):
    ...
    amp's real bundle now ships in the package (omnigraph/skills/amp/), so the
    ...
    with patch("omnigraph.__main__.Path.home", return_value=home):
        ...
        main()
        skill_dir = home / ".config" / "agents" / "skills" / "omnigraph"
        assert (skill_dir / "SKILL.md").exists()
