"""A omnigraph skip must not end the whole hook (#2986).
...
entire hook process — so anything after omnigraph's end marker was silently
dropped on every root commit (HEAD~1 does not exist), every rebase/merge,
every linked worktree and every OMNIGRAPH_SKIP_HOOK=1.
"""
...
from omnigraph.hooks import _CHECKOUT_MARKER_END, _HOOK_MARKER_END, _MERGE_MARKER_END, install
...
def test_a_skipped_post_commit_still_runs_what_follows(tmp_path):
    ...
    r = _run(hook, repo, OMNIGRAPH_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "AFTER_OMNIGRAPH" in r.stdout
...
def test_a_skipped_checkout_hook_still_runs_what_follows(tmp_path):
    repo = _repo(tmp_path)
    hook = _install_with_trailer(repo, "post-checkout", _CHECKOUT_MARKER_END)
    r = _run(hook, repo, "abc", "def", "1", OMNIGRAPH_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "AFTER_OMNIGRAPH" in r.stdout
...
def test_a_preexisting_hook_before_the_block_still_runs_first(tmp_path):
    ...
    hook.write_text("#!/bin/sh\necho BEFORE_OMNIGRAPH\n", encoding="utf-8", newline="\n")
    install(repo)
    r = _run(hook, repo, OMNIGRAPH_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "BEFORE_OMNIGRAPH" in r.stdout


def test_a_skipped_post_merge_still_runs_what_follows(tmp_path):
    repo = _repo(tmp_path)
    hook = _install_with_trailer(repo, "post-merge", _MERGE_MARKER_END)
    r = _run(hook, repo, OMNIGRAPH_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "AFTER_OMNIGRAPH" in r.stdout
    assert "launching background rebuild" not in r.stdout  # the skip itself still holds
...
def test_a_rebase_in_progress_still_runs_what_follows_for_post_merge(tmp_path):
    repo = _repo(tmp_path)
    (repo / ".git" / "rebase-merge").mkdir()
    hook = _install_with_trailer(repo, "post-merge", _MERGE_MARKER_END)
    r = _run(hook, repo)
...
def test_an_empty_diff_still_runs_what_follows_for_post_merge(tmp_path):
    """No ORIG_HEAD (e.g. the hook invoked outside of a real merge): both the
    ORIG_HEAD diff and its HEAD-only fallback yield nothing, so the block
    exits on the empty CHANGED without ever reaching the rebuild."""
    repo = _repo(tmp_path)
    hook = _install_with_trailer(repo, "post-merge", _MERGE_MARKER_END)
    r = _run(hook, repo)
    assert r.returncode == 0, r.stderr
