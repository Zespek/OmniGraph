"""Tests for the global graph infrastructure (omnigraph/global_graph.py),
prefix/prune helpers in omnigraph/build.py, and the cross-repo guard in
omnigraph/dedup.py."""
...
def test_prefix_graph_preserves_label():
    from omnigraph.build import prefix_graph_for_global
...
def test_prune_repo_removes_correct_nodes():
    from omnigraph.build import prune_repo_from_graph
    G = nx.Graph()
...
def test_global_add_creates_global_graph(tmp_path):
    ...
    global_dir = tmp_path / ".omnigraph"
    with patch("omnigraph.global_graph._GLOBAL_DIR", global_dir), \
         patch("omnigraph.global_graph._GLOBAL_GRAPH", global_dir / "global-graph.json"), \
         patch("omnigraph.global_graph._GLOBAL_MANIFEST", global_dir / "global-manifest.json"):
        from omnigraph.global_graph import global_add
...
def test_global_add_two_repos_no_collision(tmp_path):
    ...
    with patch("omnigraph.global_graph._GLOBAL_DIR", global_dir), \
         patch("omnigraph.global_graph._GLOBAL_GRAPH", global_graph_path), \
         patch("omnigraph.global_graph._GLOBAL_MANIFEST", global_manifest_path):
        from omnigraph.global_graph import global_add, _load_global_graph
        global_add(g1, "repoA")
        global_add(g2, "repoB")
...
def test_global_remove(tmp_path):
    ...
    with patch("omnigraph.global_graph._GLOBAL_DIR", global_dir), \
         patch("omnigraph.global_graph._GLOBAL_GRAPH", global_dir / "global-graph.json"), \
         patch("omnigraph.global_graph._GLOBAL_MANIFEST", global_dir / "global-manifest.json"):
        from omnigraph.global_graph import global_add, global_remove
        global_add(src_graph, "repoA")
    ...
    with patch("omnigraph.global_graph._GLOBAL_DIR", global_dir2), \
         patch("omnigraph.global_graph._GLOBAL_GRAPH", global_dir2 / "global-graph.json"), \
         patch("omnigraph.global_graph._GLOBAL_MANIFEST", global_dir2 / "global-manifest.json"):
        from omnigraph.global_graph import global_list
        repos = global_list()
...
def test_global_remove_unknown_tag_raises(tmp_path):
    ...
    with patch("omnigraph.global_graph._GLOBAL_DIR", global_dir), \
         patch("omnigraph.global_graph._GLOBAL_GRAPH", global_dir / "global-graph.json"), \
         patch("omnigraph.global_graph._GLOBAL_MANIFEST", global_dir / "global-manifest.json"):
        from omnigraph.global_graph import global_remove
...
def test_merge_graphs_prefixes_ids(tmp_path):
    ...
    repo1 = tmp_path / "repo1" / "omnigraph-out"
    repo2 = tmp_path / "repo2" / "omnigraph-out"
