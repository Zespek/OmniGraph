...
Pre-fix, the installers wrote a "## omnigraph" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed omnigraph and then upgraded to the fixed package still had the
...
# wrote a variant of "ALWAYS read omnigraph-out/GRAPH_REPORT.md before ...".
_OLD_CLAUDE_SECTION = """\
## omnigraph
...
Rules:
- ALWAYS read omnigraph-out/GRAPH_REPORT.md before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase.
- IF omnigraph-out/wiki/index.md EXISTS, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `omnigraph query "<question>"`, `omnigraph path "<A>" "<B>"`, or `omnigraph explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
...
code, your **first tool call must be** to read `omnigraph-out/GRAPH_REPORT.md` (if it exists).
...
_OLD_CURSOR_RULE = """\
---
description: omnigraph knowledge graph context
alwaysApply: true
---

This project has a omnigraph knowledge graph at omnigraph-out/.

- Before answering architecture or codebase questions, read omnigraph-out/GRAPH_REPORT.md for god nodes and community structure
- If omnigraph-out/wiki/index.md exists, navigate it instead of reading raw files
"""
...
omnigraph: A knowledge graph of this project lives in `omnigraph-out/`. \
If `omnigraph-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
...
_OLD_HOOK_PAYLOAD_SNIPPET = "Read omnigraph-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files"


def _assert_no_report_first(text: str, ctx: str) -> None:
    assert "ALWAYS read omnigraph-out/GRAPH_REPORT.md" not in text, (
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "omnigraph query" in text, (
        f"{ctx}: new 'omnigraph query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `omnigraph claude install` again after upgrading to a fixed package."""
    ...
    # Pre-existing non-omnigraph content must be preserved
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    # Since the nudge text lives in the `omnigraph hook-guard` subcommand, not
    ...
    assert "hook-guard" in new_settings_text, (
        "new hook payload should route to the `omnigraph hook-guard` subcommand"
    )
...
def test_cursor_install_upgrades_stale_rule(tmp_path, monkeypatch):
    """Same upgrade behavior for .cursor/rules/omnigraph.mdc.
    The Cursor rule file is wholly omnigraph-owned; overwrite on upgrade."""
    monkeypatch.chdir(tmp_path)
    rule_path = tmp_path / ".cursor" / "rules" / "omnigraph.mdc"
    ...
    assert "read omnigraph-out/GRAPH_REPORT.md for god nodes and community structure" not in after
    _assert_query_first(after, ".cursor/rules/omnigraph.mdc")
...
def test_kiro_install_upgrades_stale_steering(tmp_path, monkeypatch):
    """Same upgrade behavior for .kiro/steering/omnigraph.md (wholly owned)."""
    monkeypatch.chdir(tmp_path)
    steering = tmp_path / ".kiro" / "steering" / "omnigraph.md"
    ...
    _assert_query_first(after, ".kiro/steering/omnigraph.md")
...
def test_kiro_install_ships_references_sidecar_and_version_stamp(tmp_path, monkeypatch):
    ...
    and .omnigraph_version stamp are written alongside SKILL.md (#1142).
    ...
    skill_dir = tmp_path / ".kiro" / "skills" / "omnigraph"
    ...
    # .omnigraph_version stamp written
    version_file = skill_dir / ".omnigraph_version"
    assert version_file.exists(), ".omnigraph_version stamp must be written (#1142)"
    ...
    assert (tmp_path / ".kiro" / "steering" / "omnigraph.md").exists()
    ...
    assert not (tmp_path / ".kiro" / "steering" / "omnigraph.md").exists()
