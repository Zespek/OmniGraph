...
from omnigraph.cluster import cluster
from omnigraph.export import to_json, to_cypher, to_graphml, to_html, to_canvas, to_obsidian
...
def test_to_json_commit_fallback_uses_output_repo_not_cwd(tmp_path, monkeypatch):
    ...
    # be in — running `omnigraph extract <target>` from another repo's root
    ...
    (target / "omnigraph-out").mkdir(parents=True)
    git(target, "init")
    ...
    out = target / "omnigraph-out" / "graph.json"
...
def test_to_html_title_uses_portable_path_not_host_absolute():
    ...
    communities = cluster(G)
    with tempfile.TemporaryDirectory() as tmp:
        userish = Path(tmp) / "Users" / "mike" / "proj" / "omnigraph-out" / "graph.html"
    ...
    title = m.group(1)
    assert title.startswith("omnigraph - ")
    label = title[len("omnigraph - "):]
    assert "mike" not in label
    assert "Users" not in label
    assert not label.startswith("/")
    assert "omnigraph-out/graph.html" in label or label == "graph.html"


def test_html_document_title_helper_windows_and_relative():
    from omnigraph.exporters.html import _html_document_title

    assert _html_document_title(r"C:\Users\mike\proj\omnigraph-out\graph.html") == "omnigraph-out/graph.html"
    assert _html_document_title("/home/u/proj/omnigraph-out/graph.html") == "omnigraph-out/graph.html"
    assert _html_document_title("omnigraph-out/graph.html") == "omnigraph-out/graph.html"
...
def test_to_html_neighbor_links_have_no_inline_onclick_xss():
    ...
    (from a document or a scraped `omnigraph add` URL) inject a live event handler
...
# ── Issue punctuation-only Obsidian/Canvas filenames ───────────────────
...
def test_obsidian_safe_stem_all_dots_label_falls_back_to_unnamed():
    ...
    from omnigraph.export import _obsidian_safe_stem
    assert _obsidian_safe_stem(".env") == "dot-env"        # fix unchanged
    assert _obsidian_safe_stem("...") == "unnamed"         # not "dot-"
    assert _obsidian_safe_stem("Database") == "Database"   # normal labels untouched


# ── Existing-vault safety: omnigraph must not clobber user notes /.obsidian ──
...
def test_to_obsidian_preserves_existing_user_notes_and_obsidian_config():
    ...
    collides with a omnigraph node name, nor their .obsidian/ graph settings."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # non-colliding omnigraph note still written
...
def test_to_obsidian_rerun_updates_own_notes_but_not_user_files():
    """A re-run overwrites omnigraph's own prior notes (via the manifest) but leaves a
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (out / "Database.md").exists()  # omnigraph re-wrote its own
...
def test_to_obsidian_rerun_prunes_removed_nodes():
    """#1896: re-exporting into the same vault must delete omnigraph's own notes for
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # surviving omnigraph notes and the user's own note remain
        assert (out / "Database.md").exists() and (out / "Server.md").exists()
...
# ── Issue backup_if_protected ──────────────────────────────────────────

def test_backup_no_graph_json(tmp_path):
    """No graph.json → no backup."""
    from omnigraph.export import backup_if_protected
...
def test_backup_semantic_marker(tmp_path):
    """graph.json + .omnigraph_semantic_marker → backup taken."""
    from omnigraph.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / "GRAPH_REPORT.md").write_text("# Report")
    (tmp_path / ".omnigraph_semantic_marker").write_text('{"output_tokens": 1234}')
    result = backup_if_protected(tmp_path)
    assert result is not None
    assert result.is_dir()
    assert (result / "graph.json").exists()
    assert (result / "GRAPH_REPORT.md").exists()
    assert (result / ".omnigraph_semantic_marker").exists()


def test_backup_curated_labels(tmp_path):
    """graph.json + non-default label in .omnigraph_labels.json → backup taken."""
    import json
    from omnigraph.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".omnigraph_labels.json").write_text(json.dumps({"0": "Auth Pipeline", "1": "Community 1"}))
    result = backup_if_protected(tmp_path)
    assert result is not None
...
def test_backup_default_labels_only(tmp_path):
    ...
    from omnigraph.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".omnigraph_labels.json").write_text(json.dumps({"0": "Community 0", "1": "Community 1"}))
    assert backup_if_protected(tmp_path) is None
...
def test_backup_same_day_no_accumulation(tmp_path):
    ...
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".omnigraph_semantic_marker").write_text("{}")
...
def test_backup_env_disable(tmp_path, monkeypatch):
    """OMNIGRAPH_NO_BACKUP=1 disables backup entirely."""
    from omnigraph.export import backup_if_protected
    monkeypatch.setenv("OMNIGRAPH_NO_BACKUP", "1")
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".omnigraph_semantic_marker").write_text("{}")
    assert backup_if_protected(tmp_path) is None
...
def test_existing_graph_node_count(tmp_path):
    from omnigraph.export import existing_graph_node_count, MALFORMED_GRAPH
    p = tmp_path / "graph.json"
    ...
    # guard — a corrupt/mid-write file could be hiding a complete graph.
...
def test_hyperedge_perimeter_uses_convex_hull_not_member_order():
    ...
    from omnigraph.exporters.html import _hyperedge_script
