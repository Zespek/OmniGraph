...
from omnigraph.paths import OMNIGRAPH_OUT, out_path
...
def count_words(path: Path) -> int:
    try:
        ...
        # cannot help, and `omnigraph update` hangs with no output. os.stat
...
_SKIP_DIRS = {
    ...
    "dist", "build", "target",  # bare "out" is gated on build-output evidence below
    "site-packages", "lib64",
    ".pytest_cache", ".mypy_cache", ".ruff_cache",
    ".tox", ".nox", ".eggs", "*.egg-info",  # nox é o sucessor de tox, mesmo formato.nox/venv
    "omnigraph-out",  # never treat the default output as source input
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[omnigraph] WARNING: {path} is not valid UTF-8; read it as "
...
# `omnigraph watch` process spanning many repos could still accumulate keys over
...
def _is_scan_ignored(
    ...
    *,
    ...
    contains only .omnigraphignore/--exclude rules, which remain authoritative
...
def ignored_predicate(
    ...
    *,
    ...
    .omnigraphignore/.gitignore chain (_load_omnigraphignore), CLI/persisted
    ...
    """
    root = root.resolve()
    patterns = _load_omnigraphignore(root, gitignore=gitignore)
    explicit_patterns = _load_omnigraphignore(root, gitignore=False)
    # Only shell out to git when .gitignore actually contributes patterns beyond
    # the explicit (.omnigraphignore/--exclude) set: with no .gitignore in play,
    ...
    cache: dict[Path, bool] = {}
    explicit_cache: dict[Path, bool] = {}
    # root's own ignore file is the last entry of _load_omnigraphignore's chain.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    root = root.resolve()
    configured_out_dir = root / OMNIGRAPH_OUT
    ...
    explicit_ignore_patterns = _load_omnigraphignore(root, gitignore=False)
    ...
    converted_dir = out_base / OMNIGRAPH_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                ...
                try:
                    ...
                    continue
                if md_path:
                    # the sidecar lands under converted_dir, which the
                    # documented .gitignore advice puts inside a gitignored
                    # omnigraph-out/ -- an ignore check here would reject the
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    # see the matching comment in the Google Workspace
...
def save_manifest(
    ...
    *,
    ...
    # Avoid rewriting manifest.json when the serialized payload is identical.
