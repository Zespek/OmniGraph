#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from omnigraph knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .omnigraph_labels.json, and sections JSON,
...
Usage:
  python3 -m omnigraph export callflow-html
  python3 -m omnigraph export callflow-html /path/to/project/omnigraph-out/graph.json
  python3 -m omnigraph export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a omnigraph node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize omnigraph edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current omnigraph graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .omnigraph_labels.json, tolerating wrapper keys."""
...
def safe_file_path(path: str) -> str:
    ...
    # Truncar caminhos longos para exibição
    parts = path.split("/")
    if len(parts) > 3:
        return "/".join(parts[-3:])
    return path
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == OMNIGRAPH_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_omnigraph_paths(args) -> dict:
    """Resolve project root, omnigraph output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.omnigraph_out:
        omnigraph_out = Path(args.omnigraph_out).expanduser()
    elif args.graph:
        omnigraph_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        omnigraph_out = base
    else:
        omnigraph_out = base / OMNIGRAPH_OUT

    project_root = omnigraph_out.parent if omnigraph_out.name == OMNIGRAPH_OUT_NAME else base
    graph = Path(args.graph).expanduser() if args.graph else omnigraph_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else omnigraph_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else omnigraph_out / ".omnigraph_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "omnigraph_out": omnigraph_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def should_include_edge(edge: dict) -> bool:
    ...
    if conf == "INFERRED" and score >= 0.85:
        return True
    # INFERIDO ou AMBIGUOSO de baixa confiança: comente para revisão do LLM
    return False
...
def node_importance(node: dict) -> float:
    """Use omnigraph centrality fields when available."""
...
def select_diagram_nodes(nodes: list, edges: list, max_nodes: int) -> list:
    ...
    # Comece com prováveis ​​pontos de entrada: nós que chamam mais do que são chamados.
    ...
    # Em seguida, extraia os vizinhos mais úteis das arestas mais fortes.
...
def generate_call_table_rows(nodes: list, section_edges: list, lang: str) -> str:
    ...
    for i, n in enumerate(nodes[:30], 1):  # limite em 30 linhas
        ...
        # Sugira um tipo de tag com base na heurística file_type e label
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 omnigraph 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from omnigraph knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 omnigraph 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from omnigraph community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        omnigraph_out: str | Path | None = None,
        ...
        self.omnigraph_out = str(omnigraph_out) if omnigraph_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from omnigraph output files."""
    ...
    paths = resolve_omnigraph_paths(args)
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"omnigraph output not found: {paths['graph']}. "
    ...
    if args.output:
        ...
        output_path = paths["omnigraph_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from omnigraph knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or omnigraph output directory")
    parser.add_argument("--omnigraph-out", default=None, help="Path to omnigraph output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .omnigraph_labels.json")
    ...
    try:
        write_callflow_html(
            args.project,
            omnigraph_out=args.omnigraph_out,
            graph=args.graph,
            report=args.report,
            labels=args.labels,
