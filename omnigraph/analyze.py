...
# Nomes integrados/simulados que podem aparecer como nós derivados de anotações em pré-existentes
# grafos. Excluídos da classificação do god node para que não substituam abstrações reais
# mesmo que não tenham sido filtrados no momento da extração.
_BUILTIN_NOISE_LABELS = frozenset({
    ...
    # Tipos stdlib do Python comumente confundidos com símbolos de projeto
...
# Famílias de idiomas — extensões que compartilham um tempo de execução podem chamar umas às outras legitimamente
...
def _is_file_node(G: nx.Graph, node_id: str) -> bool:
    ...
    if source_file:
        ...
        if _is_file_node_label(label, source_file):
            return True
    # Esboço de método: o extrator AST rotula os métodos como '.method_name()'
    if label.startswith(".") and label.endswith("()"):
        return True
    # Esboço de função em nível de módulo: rotulado como 'function_name()' - possui apenas uma aresta contém
    # Estas são funções reais, mas estruturalmente isoladas por definição; não é uma lacuna que vale a pena sinalizar
...
def _is_concept_node(G: nx.Graph, node_id: str) -> bool:
    ...
    if not source:
        return True
    # Não tem extensão de arquivo → provavelmente um rótulo conceitual, não um arquivo real
    if "." not in source.split("/")[-1]:
        return True
    return False


from omnigraph.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS
...
def _surprise_score(
    ...
    # 1. Peso da confiança – conexões incertas são mais dignas de nota
    ...
    # Suprimir todos os bônus estruturais para chamadas/usos INFERIDOS que cruzam idiomas
    # limites ou conecte o código a um arquivo doc.  Ambos os casos são poluição do resolvedor:
    # correspondência de rótulos dispara em famílias de idiomas em monorepos e código → doc
    # As “chamadas” arestas são artefatos de extração, não arquitetura real.
    # Exclui `semantically_similar_to` (insight transfronteiriço genuíno) e todos
    # Arestas AMBIGUAS/EXTRAÍDAS (não do caminho do resolvedor).
    ...
    # 2. Bônus de tipo de arquivo cruzado - código↔papel ou código↔imagem não é óbvio
    ...
    # 4. Bônus entre comunidades – Leiden diz que estes são estruturalmente distantes
    ...
    # 5. Periférico → hub: um nó de baixo grau conectado a um nó de alto grau
...
def _cross_community_surprises(
    ...
    """
    if not communities:
        # Sem informações da comunidade - use a centralidade de intermediação de aresta
    ...
    for u, v, data in G.edges(data=True):
        ...
        if cid_u is None or cid_v is None or cid_u == cid_v:
            continue
        # Ignorar nós de hub de arquivos e arestas estruturais simples
        if _is_file_node(G, u) or _is_file_node(G, v):
            continue
        relation = data.get("relation", "")
        if relation in ("imports", "imports_from", "contains", "method"):
    ...
    # Classificar: AMBIGUOSO primeiro, depois INFERIDO e depois EXTRAÍDO
...
def suggest_questions(
    ...
    if G.number_of_edges() > 0:
        ...
        # Nós de ponte superiores que NÃO são hubs em nível de arquivo
        bridges = sorted(
    ...
    # 3. Nós de Deus com muitas arestas INFERIDAS → questões de verificação
    degree = dict(G.degree())
    top_nodes = sorted(
    ...
    for node_id, _ in top_nodes:
        ...
        if len(inferred) >= 2:
            label = G.nodes[node_id].get("label", node_id)
            # Use _src/_tgt para obter a direção correta; volte para v (o outro nó)
            others = []
    ...
    # 4. Nós isolados ou fracamente conectados → questões de exploração
    isolated = [
        n for n in G.nodes()
        if G.degree(n) <= 1
...
def find_import_cycles(
    ...
    # Etapa 1: construir um grafo direcionado em nível de arquivo a partir de arestas de importação/reexportação.
    # IMPORTANTE: resolva endpoints usando apenas source_file; nunca deduza do rótulo/id.
    ...
    for u, v, data in G.edges(data=True):
        ...
        # As arestas `import(...)` diferidas são dependências reais, mas não formam um
