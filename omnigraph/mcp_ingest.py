...
and turns the `mcpServers` map into OmniGraph nodes and edges.

Symmetry with `serve.py`: OmniGraph exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
omnigraph with `--mcp` can now query its own configured MCP layer.
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into OmniGraph nodes and edges.
    ...
    for server_name, spec in servers.items():
        ...
        if not isinstance(spec, dict):
            # Ignore entradas de servidor que não sejam de objeto silenciosamente — a entrada quebrada é
            # do usuário, não nosso.
            continue
...
def _emit_server(
    *,
    ...
    _add_node(
        ...
        line=1,  # JSON não expõe números de linha sem uma passagem de analisador
    )
    _add_edge(
    ...
    if isinstance(env, dict):
        # SOMENTE CHAVES. Os valores podem conter segredos e nunca são lidos aqui.
...
# Receita canônica importada diretamente (sem ciclo de importação: importações extractors.base
# apenas omnigraph.ids), então isso não pode mais derivar de extract._file_stem.
from omnigraph.extractors.base import _file_stem  # noqa: E402
