"""omnigraph install/uninstall subsystem.

The per-platform skill/hook installers and uninstallers, extracted verbatim from
omnigraph/__main__.py so the CLI dispatcher (`main`) stays readable. Import path is
unchanged: `omnigraph.__main__` re-exports every name defined here, so
`from omnigraph.__main__ import claude_install` (etc.) keeps working.

Lives at omnigraph/install.py (same package dir as __main__) on purpose: the
installers resolve packaged assets via `Path(__file__).parent / "always_on"` and
`/ "skills"`, which only works from within the omnigraph/ directory.
"""
...
def _write_version_stamp(skill_dst: Path, version: str) -> None:
    """Atomically write ``.omnigraph_version`` beside ``skill_dst``.
...
def _always_on(basename: str) -> str:
    """Read a packaged always-on instruction block from omnigraph/always_on/.
    ...
    try:
        ...
        # Adie o tempo de uso para que um bloco empacotado ausente/corrompido não possa travar o módulo
        # import (que bloquearia todos os comandos CLI, não apenas instalar). Alcançado
        # apenas por um caminho de instalação/integração que realmente precisa deste bloco.
        raise RuntimeError(
...
def _platform_skill_destination(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if platform_name == "gemini":
        if project:
            return (project_dir or Path(".")) / ".gemini" / "skills" / "omnigraph" / "SKILL.md"
        if platform.system() == "Windows":
            return Path.home() / ".agents" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".gemini" / "skills" / "omnigraph" / "SKILL.md"

    if platform_name == "opencode":
        if project:
            return (project_dir or Path(".")) / ".opencode" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".config" / "opencode" / "skills" / "omnigraph" / "SKILL.md"

    if platform_name == "hermes":
        if project:
            return (project_dir or Path(".")) / ".hermes" / "skills" / "omnigraph" / "SKILL.md"
        # No Windows, o Hermes verifica %LOCALAPPDATA%\hermes\skills, não ~/.hermes.
        if platform.system() == "Windows":
            local_appdata = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local"))
            return local_appdata / "hermes" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".hermes" / "skills" / "omnigraph" / "SKILL.md"

    if platform_name == "devin":
        if project:
            return (project_dir or Path(".")) / ".devin" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".config" / "devin" / "skills" / "omnigraph" / "SKILL.md"

    if platform_name == "amp":
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".config" / "agents" / "skills" / "omnigraph" / "SKILL.md"

    if platform_name == "agents":
        # O destino genérico Agent-Skills: project ./.agents/skills, global the
        # especificação global do usuário ~/.agents/skills (lida por `npx skills` e compatível
        # frameworks), NÃO ~/.config/agents/skills do amplificador.
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "omnigraph" / "SKILL.md"
        return Path.home() / ".agents" / "skills" / "omnigraph" / "SKILL.md"
    ...
    if platform_name in ("antigravity", "antigravity-windows"):
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "omnigraph" / "SKILL.md"
        # Diretório global de habilidades antigravidade (todos os espaços de trabalho): ~/.gemini/config/skills/
        return Path.home() / ".gemini" / "config" / "skills" / "omnigraph" / "SKILL.md"
    ...
    if platform_name in ("claude", "windows") and os.environ.get("CLAUDE_CONFIG_DIR"):
        return Path(os.environ["CLAUDE_CONFIG_DIR"]) / "skills" / "omnigraph" / "SKILL.md"
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``omnigraph/skills/<bundle>/references/``. Reuse keys (e.g. trae-cn) point at
    their twin's bundle.
    ...
    ``omnigraph/skills/<bundle>/`` is not in this build has not gone progressive
...
def _copy_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    ``.omnigraph_version`` stamp covers both. For monolith platforms (no
    ...
    if not skill_src.exists():
        print(f"error: {skill_file} not found in package - reinstall omnigraph", file=sys.stderr)
        sys.exit(1)
    ...
    # Instale as referências/sidecar (ou limpe um órfão) ANTES de escrever
    # SKILL.md, então SKILL.md é o último artefato estabelecido. Uma instalação que é
    # interrompido parcialmente, então não deixa nenhum SKILL.md em vez de um SKILL.md que
    # aponta para uma referência/diretório ausente.
    if refs_src is not None:
        _install_skill_references(skill_dst, refs_src)
        print(f"  references       ->  {skill_dst.parent / 'references'}")
    else:
        # Monolith (ou progressivo sem referências): limpe quaisquer referências órfãs/.
    ...
    # while the edits were gone. Keep one.bak beside it and say so.
    ...
    # SKILL.md por último (segurança contra colisão), por meio de temperatura atômica + renomeação.
...
def _remove_claude_skill_registration(project_dir: Path) -> None:
    ...
    if cleaned:
        ...
        print(f"  CLAUDE.md        ->  omnigraph skill registration removed from {claude_md}")
    else:
        claude_md.unlink()
...
def _claude_pretooluse_hooks(strict: bool = False, project: bool = False) -> "list[dict]":
    ...
    """
    exe = _resolve_omnigraph_exe(project=project)
...
def _skill_registration(skill_path: str = "~/.claude/skills/omnigraph/SKILL.md") -> str:
    return (
        "\n# omnigraph\n"
        f"- **omnigraph** (`{skill_path}`) "
        "- any input to knowledge graph. Trigger: `/omnigraph`\n"
...
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".claude") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": True,
        "skill_refs": "claude",
    },
    "codex": {
        "skill_file": "skill-codex.md",
        "skill_dst": Path(".codex") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "codex",
    },
    "opencode": {
        "skill_file": "skill-opencode.md",
        "skill_dst": Path(".config") / "opencode" / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "opencode",
    },
    "kilo": {
        "skill_file": "skill-kilo.md",
        "skill_dst": Path(".config") / "kilo" / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kilo",
    },
    "aider": {
        # Monolith: Aider envia o SKILL.md completo inline, sem referências/sidecar.
        "skill_file": "skill-aider.md",
        "skill_dst": Path(".aider") / "omnigraph" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
        "skill_file": "skill-copilot.md",
        "skill_dst": Path(".copilot") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "copilot",
    },
    "claw": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".openclaw") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "droid",
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "trae-cn": {
    ...
    "kiro": {
        ...
        "skill_dst": Path(".kiro") / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kiro",
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "omnigraph" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "pi",
    },
    "codebuddy": {
    ...
    "amp": {
        # Amp pesquisa .agents/skills (projeto) e ~/.config/agents/skills (usuário),
        # não .amp/skills. O caminho do escopo do usuário é definido em _platform_skill_destination.
...
def _replace_or_append_section(content: str, marker: str, new_section: str) -> str:
    """Idempotently update or append a omnigraph-owned section in shared files.
    ...
    omnigraph`` inside a bullet or an inline reference used to anchor the replace
    ...
    headings exist, the last one is used, since omnigraph's section is appended.
    """
    lines = content.split("\n")
...
def _remove_marker_section(content: str, marker: str, boundary_prefix: str = "## ") -> "str | None":
    ...
    regex ``## omnigraph`` was unanchored, so it matched inside a user's
    ``### omnigraph`` heading and deleted hand-written content (#2062) — the same
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "cursor":
        ...
        return
    # No Windows, a antigravidade precisa da habilidade do PowerShell, não da do bash
    ...
    if platform == "kilo":
        # O Kilo Code também suporta um arquivo de comando nativo /omnigraph.
        command_src = Path(__file__).parent / "command-kilo.md"
        if not command_src.exists():
            print(
                f"error: command-kilo.md not found in package - reinstall omnigraph",
                file=sys.stderr,
            )
            sys.exit(1)
        command_dst = Path.home() / ".config" / "kilo" / "command" / "omnigraph.md"
    ...
    if cfg["claude_md"]:
        ...
        # does for the skill copy path -- this always-on registration
        # path was missed by that fix.
        if project:
            claude_md = project_dir / ".claude" / "CLAUDE.md"
...
def _print_install_usage() -> None:
    ...
    print("Usage: omnigraph install [--project] [--strict] [--platform P|P]")
    print(f"Platforms: {platforms}")
    print("  --strict  block the first raw file read per session until one "
          "`omnigraph query` runs (Claude Code project hook only; needs --project)")
_CLAUDE_MD_MARKER = "## omnigraph"
_CODEBUDDY_MD_MARKER = "## omnigraph"
_AGENTS_MD_MARKER = "## omnigraph"
_GEMINI_MD_MARKER = "## omnigraph"
def _gemini_hook(project: bool = False) -> dict:
    """Gemini CLI BeforeTool hook, resolved to a shell-agnostic `omnigraph` call.
...
def gemini_install(project_dir: Path | None = None, *, project: bool = False) -> None:
    ...
    if target.exists() and new_content == target.read_text(encoding="utf-8"):
        print(f"omnigraph already configured in {target.resolve()} (no change)")
    else:
        target.write_text(new_content, encoding="utf-8")
        print(f"omnigraph section written to {target.resolve()}")

    # Sempre reinstale o gancho Gemini para que uma carga útil mais antiga (por exemplo, pré-edição-
    # texto) é substituído na atualização.
...
def _refuse_to_modify(settings_path: Path) -> "NoReturn":
    ...
    print(
        f"[omnigraph] refusing to modify {settings_path}: not valid JSON "
...
def _write_settings_with_backup(settings_path: Path, settings: dict) -> None:
    ...
    the existing file to ``<name>.omnigraph-bak`` (single rolling backup) before
...
def _install_gemini_hook(project_dir: Path, project: bool = False) -> None:
    ...
    if not isinstance(before_tool, list):
        _refuse_to_modify(settings_path)
    hooks["BeforeTool"] = [
        h for h in before_tool if "omnigraph" not in str(h)
    ]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "omnigraph" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the omnigraph section from GEMINI.md, uninstall hook, and remove skill file.
    ...
    if cleaned is None:
        print("omnigraph section not found in GEMINI.md - nothing to do")
        return
    if cleaned:
        target.write_text(cleaned + "\n", encoding="utf-8")
        print(f"omnigraph section removed from {target.resolve()}")
    else:
        target.unlink()
        print(f"GEMINI.md was empty after removal - deleted {target.resolve()}")
    _uninstall_gemini_hook(project_dir)
_VSCODE_INSTRUCTIONS_MARKER = "## omnigraph"
def vscode_install(project_dir: Path | None = None) -> None:
    """Install omnigraph skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    skill_src = Path(__file__).parent / "skill-vscode.md"
    refs_bundle = "vscode"
    if not skill_src.exists():
        skill_src = Path(__file__).parent / "skill-copilot.md"
        refs_bundle = "copilot"
    skill_dst = Path.home() / ".copilot" / "skills" / "omnigraph" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    tmp_dst = skill_dst.with_suffix(skill_dst.suffix + ".tmp")
    try:
        shutil.copy(skill_src, tmp_dst)
        ...
        try:
            ...
            pass
        raise
    # Capacidade progressiva: instale as referências/sidecar empacotadas quando presentes.
    ...
    if instructions.exists():
        ...
        if new_content == content:
            ...
            print(f"  {instructions}  ->  omnigraph section {'updated' if _VSCODE_INSTRUCTIONS_MARKER in content else 'added'}")
    else:
    ...
    print()
    print(
        "VS Code Copilot Chat configured. Type /omnigraph in the chat panel to build the graph."
    )
    print("Note: for GitHub Copilot CLI (terminal), use: omnigraph copilot install")
def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove omnigraph VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "omnigraph" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  omnigraph section removed from {instructions}")
    else:
        instructions.unlink()
...
Follow the omnigraph skill to run the full pipeline.

If no path argument is given, use `.` (current directory).
"""
def _kiro_install(project_dir: Path) -> None:
    """Write omnigraph skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")
    ...
    steering_dst = steering_dir / "omnigraph.md"
    if steering_dst.exists() and steering_dst.read_text(encoding="utf-8") == _always_on("kiro-steering"):
        print(f"  .kiro/steering/omnigraph.md  ->  already configured (no change)")
    else:
        # O arquivo é de propriedade integral do zspekfy. Substituir na atualização tão antiga
        # a redação do primeiro relatório não permanece silenciosamente (edição nº 580).
        action = "updated" if steering_dst.exists() else "written"
        steering_dst.write_text(_always_on("kiro-steering"), encoding="utf-8")
        print(f"  .kiro/steering/omnigraph.md  ->  always-on steering {action}")

    print()
    print("Kiro will now read the knowledge graph before every conversation.")
    print("Use /omnigraph to build or update the graph.")
def _kiro_uninstall(project_dir: Path) -> None:
    """Remove omnigraph skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []
    ...
    steering_dst = project_dir / ".kiro" / "steering" / "omnigraph.md"
...
def _antigravity_finalize(skill_dst: Path, project_dir: Path) -> None:
    ...
    writes ``.agents/rules/omnigraph.md`` and ``.agents/workflows/omnigraph.md``
    ...
    if rules_path.exists():
        ...
        if _always_on("antigravity-rules").strip() != existing.strip():
            ...
            print(f"omnigraph rule updated at {rules_path.resolve()}")
        else:
            print(f"omnigraph rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_always_on("antigravity-rules"), encoding="utf-8")
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"omnigraph workflow updated at {wf_path.resolve()}")
        else:
            print(f"omnigraph workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
...
def _antigravity_install(project_dir: Path) -> None:
    ...
    print("codebase questions. Run /omnigraph first to build the graph.")
    print()
    print(
    ...
    print('    "command": "uv",')
    print(
        '    "args": ["run", "--with", "omnigraph", "--with", "mcp", "-m", "omnigraph.serve", "${workspace.path}/omnigraph-out/graph.json"]'
    )
    print("  }")
def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    ...
    if rules_path.exists():
        rules_path.unlink()
        print(f"omnigraph rule removed from {rules_path.resolve()}")
    else:
    ...
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"omnigraph skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".omnigraph_version"
    if version_file.exists():
        version_file.unlink()
    refs_dir = skill_dst.parent / "references"
    ...
    for d in (
        ...
        try:
            ...
            break
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "omnigraph.mdc"
_CURSOR_RULE = """\
---
...
**MANDATORY: Before using Read, Grep, Glob, or Bash to explore the codebase, you MUST run omnigraph first:**
- `omnigraph query "<question>"` — scoped subgraph for any codebase or architecture question
- `omnigraph path "<A>" "<B>"` — dependency path between two symbols
- `omnigraph explain "<concept>"` — all nodes related to a concept

This applies to YOU and to every subagent you spawn. Include this rule explicitly in every subagent prompt that involves code exploration. Do not skip omnigraph because files are "already known" or because you are executing a plan — the graph surfaces cross-file dependencies and INFERRED edges that grep and Read cannot find.

Only use Read/Grep/Glob directly when:
1. omnigraph has already oriented you and you need to modify or debug specific lines
2. `omnigraph-out/graph.json` does not exist yet

- If `omnigraph-out/wiki/index.md` exists, navigate it instead of reading raw files
- Read `omnigraph-out/GRAPH_REPORT.md` only for broad architecture review when query/path/explain do not surface enough context
- After modifying code files, run `omnigraph update .` to keep the graph current (AST-only, no API cost)
"""
def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/omnigraph.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"omnigraph rule at {rule_path} already configured (no change)")
        return
    # O arquivo é de propriedade integral do zspekfy. Substituir na atualização tão antiga
    # a redação do primeiro relatório não permanece silenciosamente (edição nº 580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"omnigraph rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /omnigraph . first to build the graph if you haven't already.")
def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/omnigraph.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No omnigraph Cursor rule found - nothing to do")
        return
    rule_path.unlink()
...
Rules:
- For codebase or architecture questions, when `omnigraph-out/graph.json` exists, first run `omnigraph query "<question>"` (or `omnigraph path "<A>" "<B>"` / `omnigraph explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If omnigraph-out/wiki/index.md exists, navigate it instead of reading raw files
...
def _devin_rules_install(project_dir: Path) -> None:
    """Write .windsurf/rules/omnigraph.md for always-on Devin context."""
...
def _devin_rules_uninstall(project_dir: Path) -> None:
    """Remove .windsurf/rules/omnigraph.md."""
    rules_path = (project_dir or Path(".")) / _DEVIN_RULES_PATH
...
_KILO_PLUGIN_JS = """\
// omnigraph Kilo plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
import { existsSync } from "fs";
import { join } from "path";

export const OmniGraphPlugin = async ({ directory }) => {
  let reminded = false;

  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "omnigraph-out", "graph.json"))) return;
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[omnigraph] Knowledge graph available. Read omnigraph-out/GRAPH_REPORT.md for god nodes and architecture context before searching files." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_KILO_PLUGIN_PATH = Path(".kilo") / "plugins" / "omnigraph.js"
...
def _install_kilo_plugin(project_dir: Path) -> None:
    """Write omnigraph.js plugin and register it without rewriting user JSONC."""
...
def _uninstall_kilo_plugin(project_dir: Path) -> None:
    """Remove omnigraph.js plugin and deregister it without rewriting user JSONC."""
    plugin_file = project_dir / _KILO_PLUGIN_PATH
...
# Plug-in OpenCode tool.execute.before - é acionado antes de cada chamada de ferramenta.
# Injeta um lembrete de grafo na saída do comando bash quando graph.json existe.
_OPENCODE_PLUGIN_JS = """\
// omnigraph OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
//
// IMPORTANT: keep the reminder string free of backticks and $(...) constructs.
// The hook prepends `echo "<reminder>" && <cmd>` to the user's bash command;
// backticks inside the double-quoted echo trigger bash command substitution,
// which both corrupts tool output and silently executes the very omnigraph
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[omnigraph] knowledge graph at omnigraph-out/. For focused questions, run omnigraph query with your question (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_OPENCODE_PLUGIN_PATH = Path(".opencode") / "plugins" / "omnigraph.js"
_OPENCODE_CONFIG_PATH = Path(".opencode") / "opencode.json"
def _install_opencode_plugin(project_dir: Path) -> None:
    """Write omnigraph.js plugin and register it in opencode.json."""
...
def _uninstall_opencode_plugin(project_dir: Path) -> None:
    """Remove omnigraph.js plugin and deregister from opencode.json."""
    plugin_file = project_dir / _OPENCODE_PLUGIN_PATH
    if plugin_file.exists():
        plugin_file.unlink()
...
def _resolve_omnigraph_exe(project: bool = False) -> str:
    """Return the absolute path to the omnigraph executable, with forward slashes.

    With *project* set, return the bare ``omnigraph`` command instead. A
    ...
    checkouts. A committed hook refers to ``omnigraph`` the way it would refer to
    ``git`` or ``node``, and PATH resolves it per machine (#3129).

    Falls back to bare 'omnigraph' if resolution fails. Using an absolute path
    ...
    character: a raw ``C:\\Users\\me\\omnigraph.EXE`` collapses to
    ``C:Usersmeomnigraph.EXE: command not found`` and the guard silently fails.
    ...
    if not found:
        # Derive de sys.executable: mesmo diretório Scripts/ (Windows) ou bin/ (Unix)
        scripts_dir = Path(sys.executable).parent
        for name in ("omnigraph.exe", "omnigraph"):
            candidate = scripts_dir / name
            if candidate.exists():
                found = str(candidate)
                break
    return (found or "omnigraph").replace("\\", "/")
def _install_codex_hook(project_dir: Path, project: bool = False) -> None:
    """Add omnigraph PreToolUse hook to .codex/hooks.json.
    ...
    omnigraph_exe = _resolve_omnigraph_exe(project=project)
    hook_entry = {
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [{"type": "command", "command": f"{omnigraph_exe} hook-check"}],
                }
            ]
        }
    }
    ...
    if not isinstance(pre_tool, list):
        _refuse_to_modify(hooks_path)
    hooks["PreToolUse"] = [h for h in pre_tool if "omnigraph" not in str(h)]
    hooks["PreToolUse"].extend(hook_entry["hooks"]["PreToolUse"])
    _write_settings_with_backup(hooks_path, existing)
    print(
        f"  .codex/hooks.json  ->  PreToolUse hook registered ({omnigraph_exe} hook-check"
...
def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove omnigraph PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
    ...
    filtered = [h for h in pre_tool if "omnigraph" not in str(h)]
    existing["hooks"]["PreToolUse"] = filtered
    hooks_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
    print(f"  .codex/hooks.json  ->  PreToolUse hook removed")
def _agents_install(project_dir: Path, platform: str, project: bool = False) -> None:
    """Write the omnigraph section to the local AGENTS.md for always-on platforms."""
...
def _amp_legacy_cleanup() -> None:
    """Best-effort removal of the pre-fix ~/.amp/skills/omnigraph install dir.

    Older omnigraph versions wrote the Amp skill to ~/.amp/skills, which Amp does
    ...
    """
    legacy = Path.home() / ".amp" / "skills" / "omnigraph"
    if legacy.exists():
...
def _agents_platform_install(project_dir: Path | None = None) -> None:
    """`omnigraph agents install`: skill into ~/.agents/skills + AGENTS.md.
    ...
    rendered hooks reference, which points at `omnigraph agents install`. The bare
    `omnigraph install --platform agents` path stays skill-only (via install()),
...
def _agents_platform_uninstall(project_dir: Path | None = None) -> None:
    """`omnigraph agents uninstall`: remove the skill and the AGENTS.md section."""
...
def _project_uninstall_all(project_dir: Path | None = None) -> None:
    ...
    project_dir = project_dir or Path(".")
    print("Uninstalling project-scoped omnigraph files...\n")
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the omnigraph section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if cleaned is None:
        print("omnigraph section not found in AGENTS.md - nothing to do")
        if platform == "opencode":
            _uninstall_opencode_plugin(project_dir or Path("."))
        elif platform == "kilo":
            _uninstall_kilo_plugin(project_dir or Path("."))
...
def claude_install(project_dir: Path | None = None, strict: bool = False, project: bool = False) -> None:
    """Write the omnigraph section to the local CLAUDE.md."""
    ...
    if strict:
        ...
        print("one `omnigraph query` runs (toggle with OMNIGRAPH_HOOK_STRICT=0).")
def _install_claude_hook(project_dir: Path, strict: bool = False, project: bool = False) -> None:
    """Add omnigraph PreToolUse hook to .claude/settings.json.
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    ...
    for name in ("settings.json", "settings.local.json"):
        _strip_omnigraph_hook(claude_dir / name)
def _strip_omnigraph_hook(settings_path: Path) -> None:
    """Drop omnigraph PreToolUse hooks from a single Claude settings file, if present."""
    if not settings_path.exists():
        return
    try:
        settings = json.loads(settings_path.read_text(encoding="utf-8"))
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove omnigraph from every platform detected in the current project."""
    ...
    # A habilidade de escopo do usuário da plataforma genérica de agentes reside em ~/.agents/skills,
    # que nem a limpeza do AGENTS.md nem a remoção do amplificador alcançam.
    ...
    if purge:
        ...
        out = pd / _OMNIGRAPH_OUT
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  {_OMNIGRAPH_OUT}/  ->  deleted (--purge)")
        else:
            print(f"\n  {_OMNIGRAPH_OUT}/  ->  not found (nothing to purge)")

    print("\nDone. Run 'pip uninstall omnigraph' to remove the package itself.")
def claude_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the omnigraph skill tree (SKILL.md + references/) and the omnigraph
    section from CLAUDE.md and its local-only variants, plus the PreToolUse hook.

    Mirrors gemini_uninstall: the bare `omnigraph uninstall` and `omnigraph claude
    ...
    for target in existing:
        # Não em curto-circuito: todos os arquivos presentes devem ser limpos, não apenas o primeiro.
        if _strip_omnigraph_md_section(target):
            removed_any = True

    if not existing:
        print("No CLAUDE.md found in current directory - nothing to do")
    elif not removed_any:
        print("omnigraph section not found in CLAUDE.md - nothing to do")

    _uninstall_claude_hook(project_dir)
def _strip_omnigraph_md_section(target: Path) -> bool:
    """Strip the ## omnigraph section from one CLAUDE.md-style file.
...
def codebuddy_install(project_dir: Path | None = None) -> None:
    """Install the omnigraph skill and CODEBUDDY.md section for CodeBuddy."""
    ...
    # Escreva também o gancho CodeBuddy PreToolUse em .codebuddy/settings.json
...
def _install_codebuddy_hook(project_dir: Path) -> None:
    """Add omnigraph PreToolUse hook to .codebuddy/settings.json."""
...
def _uninstall_codebuddy_hook(project_dir: Path) -> None:
    """Remove omnigraph PreToolUse hook from .codebuddy/settings.json."""
    settings_path = project_dir / ".codebuddy" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def codebuddy_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the omnigraph skill tree (SKILL.md + references/) and the CODEBUDDY.md section.
    ...
    if cleaned is None:
        print("omnigraph section not found in CODEBUDDY.md - nothing to do")
        return
...
def dispatch_install_cli(cmd: str) -> bool:
    """Handle `omnigraph <install-command>` dispatch (install/uninstall + every
    ...
    if cmd not in _CLI_INSTALL_COMMANDS:
        return False
    if cmd == "install":
        # Padrão para plataforma Windows no Windows, claude em outro lugar
        ...
        if project_scope:
            ...
            if strict:
                print(
                    ...
                    "`omnigraph install --project --strict` or `omnigraph claude install --strict`.",
                    file=sys.stderr,
                )
        ...
        if subcmd == "install":
            ...
            print("Usage: omnigraph claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "codebuddy":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph codebuddy [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kilo":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph kilo [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "devin":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph devin [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "amp":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: omnigraph amp [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: omnigraph {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: omnigraph antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    return True
