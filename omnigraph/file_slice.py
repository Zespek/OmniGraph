...
# Tipos de documentos de texto simples onde o fatiamento baseado em limites é significativo e onde
# `_file_to_text` é um ``read_text`` direto (portanto, um intervalo de caracteres corresponde aos bytes
# o modelo é mostrado). Exclui deliberadamente código (.py, .ts, ...) e binário
# docs (.pdf) — nunca são fatiados.
#
...
# as its first 20k with no warning and no partial marker.
...
# Preferências de limite, mais fortes primeiro. Um título Markdown (``\n#``) mantém um
# seção com seu título; uma linha em branco mantém um parágrafo intacto; uma nova linha vazia
# evita cortar a linha média. Se nada for encontrado na janela, nós cortamos.
...
# Uma unidade de trabalho de extração: um arquivo inteiro (``Path``) ou uma fatia de um.
Unit = "Path | FileSlice"
...
def expand_oversized_files(
    ...
    for f in files:
        ...
        try:
            ...
            # carry — not the container's bytes.
...
def bisect_slice(fs: FileSlice) -> tuple[FileSlice, FileSlice] | None:
    ...
    # slice could cut mid-line or past the text end. Any converter/read
