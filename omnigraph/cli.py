...
def _zero_node_stamped_code_sources(
    ...
    documented recovery was deleting omnigraph-out/. A stamped file that the
    ...
    if cmd == "provider":
        ...
        if labels_path.exists() and not force_relabel:
            ...
            if _ignored_label_flags:
                print(
                    f"[omnigraph] warning: {'/'.join(_ignored_label_flags)} ignored: "
                    f"reusing saved labels at {labels_path}. "
                    f"Run `omnigraph label` to relabel, or delete the labels file.",
                    file=sys.stderr,
                )
        ...
        if not has_path:
            ...
            if _healed_sources:
                print(
                    f"[omnigraph extract] re-queuing {len(_healed_sources)} "
        ...
        if code_files:
            ...
            if incremental_mode and existing_graph_path.exists():
                ...
                try:
                    from omnigraph.build import _is_ast_tier as _ctx_is_ast_tier
