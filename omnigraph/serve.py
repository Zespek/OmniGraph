# Servidor stdio MCP - expõe ferramentas de consulta ao grafo para Claude e outros agentes
...
from omnigraph.security import sanitize_label, check_graph_file_size_cap
from omnigraph.build import edge_data, edge_datas
...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        try:
            ...
            if _legacy(data.get("nodes", [])):
                print(
                    ...
                    "rebuild with `omnigraph extract --force` for path-qualified IDs.",
                    file=sys.stderr,
                )
        except Exception:
            pass
        ...
        # Anexe a sobreposição de memória de trabalho (sidecar derivado próximo a graph.json) para
        # a superfície de leitura de consulta/MCP pode anotar linhas NODE somente para exibição. Vazio
        # quando não existe nenhum sidecar, deixando o byte de saída não anotado idêntico.
        try:
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /omnigraph to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def _max_server_contexts() -> int:
    ...
    ``OMNIGRAPH_MAX_CONTEXTS`` overrides the default. Invalid or blank values
    ...
    """
    raw = os.environ.get("OMNIGRAPH_MAX_CONTEXTS", "").strip()
    if not raw:
        return 8
    try:
...
# consulta viável), mas deliberadamente OMITIR "guerra"/"careca" (alemão era/em breve) então
# Perguntas em inglês sobre “guerra” ou “careca” não são ignoradas. Do lado românico
...
def _query_terms(question: str) -> list[str]:
    ...
    for raw in question.split():
        if _has_chinese(raw):
            ...
            # Remova a pontuação sem tocar nos caracteres Unicode (evite que o NFKD altere scripts não latinos)
...
def _trigram_candidates(G: nx.Graph, needles: list[str], *, guard_frac: float = 0.10) -> list[str] | None:
    ...
    for s in needles:
        ...
        if min(present) > thresh:
            return None  # trigrama mais raro ainda muito comum -> não vale o índice
    cand: set[int] = set()
    for s in needles:
        sets: list[set] | None = []
        for g in _trigrams(s):
            bucket = postings.get(g)
            if bucket is None:
                sets = None  # um trigrama ausente em todos os lugares -> a agulha não corresponde a nada
                break
...
def _score_query(
    ...
    # Desduplicar tokens, preservar pedidos (como _pick_seeds já faz): uma repetição
    # a palavra de consulta não deve contar duas vezes em cada camada e com escalonamento de cobertura
    # abaixo, também inflacionaria o índice de prazo combinado.
    ...
    # String de consulta inteira para correspondência de rótulo completo (espelha o `term` de _find_node).
    joined = " ".join(norm_terms)
    # Pondere o bônus de consulta completa pelo termo constituinte mais raro, de modo que um específico
    # o rótulo com várias palavras ainda supera o ruído do token comum; andar em 1,0.
    joined_w = max((idf.get(t, 1.0) for t in norm_terms), default=1.0)
    # Pré-filtro trigrama: pontua apenas nós cujo texto pode corresponder a um termo, caindo
    # de volta ao grafo inteiro quando o índice não é seletivo. O resultado é
    # idêntico de qualquer maneira - a pontuação por nó abaixo permanece inalterada e um
    # o nó não candidato sempre pontua 0. (O IDF acima permanece uma estatística de grafo inteiro.)
    ...
    for nid, data in node_iter:
        ...
        # Forma tokenizada do rótulo (sem pontuação, mesma transformação do
        ...
        # isso preserva o desempenho do tempo de consulta única, onde nid_lower era preguiçoso.
        nid_lower = nid.lower() if (joined or collect_per_term_seeds) else ""
        score = 0.0
        # Camada de consulta completa: uma consulta com várias palavras que é igual (ou prefixa) o todo
        # O rótulo deve dominar as somas de palavras por token abaixo, então `caminho`/
        ...
        # Cobertura de prazo: dimensione os níveis exatos/prefixos por prazo de acordo com o
        # fração quadrada dos termos de consulta aos quais o LABEL do nó corresponde, portanto, um único
        # palavra genérica que é igual a um rótulo curto (termo de consulta "casa"
        # vs. uma folha home()) não pode enterrar nós que correspondam a vários dos
        # termos da consulta. A quadratura é importante porque o nível exato é 10x o
        # nível de prefixo: na cobertura linear, uma correspondência exata de 1 de 10 termos ainda
        # supera uma correspondência de prefixo + substring de 3 de 10. Prazo único e
        # consultas de cobertura total permanecem inalteradas (cobertura == 1), então o identificador
        # as pesquisas mantêm o domínio da correspondência exata. O arquivo de origem atinge a pontuação, mas o faz
        # não conta como cobertura: uma folha em colisão cujo diretório compartilha
        # tokens com a consulta (comuns perto do alvo pretendido) não devem
        # reconquistar seu nível exato por meio de fragmentos de caminho. A substring/fonte
        # os bônus e o nível de consulta completa acima permanecem sem escala.
        matched = 0
        tiered = 0.0
        for t in norm_terms:
            w = idf.get(t, 1.0)
            # Contribuições por nível para este token, mantidas separadas para que o
    ...
    # Classifique por pontuação; quebrar os laços em direção ao rótulo mais curto, de modo que seja conciso e exato
    # a partida vence um superconjunto mais longo que compartilha a mesma pontuação.
...
def _pick_seeds(
    ...
    *,
    ...
    # desmoronar juntos. Quando G está ausente, não podemos ler os rótulos, então volte para
    # o ID do nó (exclusivo), que é autônomo - preservando o comportamento antigo.
    ...
    if G is not None and best_seed_by_term:
        # Garantir uma semente por termo de consulta distinto que tenha alguma correspondência,
        # portanto, uma correspondência exata incidental em um termo não pode privar as correspondências de
        ...
        for term in sorted(best_seed_by_term):
            ...
            # reintroduzir uma segunda cópia de um rótulo genérico já divulgado.
...
def _bfs(G: nx.Graph, start_nodes: list[str], depth: int) -> tuple[set[str], list[tuple]]:
    # Limite do hub de computação: os nós acima desse grau não são expandidos como trânsito.
    # p99 da distribuição de graus, com piso em 50 para evitar o bloqueio excessivo de pequenos grafos.
    ...
    for _ in range(depth):
        ...
        for n in frontier:
            # Não expanda através de centros de alto nível (exceto sementes - um centro que
            # é o nó inicial ainda deve ser explorado).
...
def _subgraph_to_text(G: nx.Graph, nodes: set[str], edges: list[tuple], token_budget: int = 2000, *, seeds: list[str] | None = None) -> str:
    ...
    lines = []
    # Sobreposição de memória de trabalho (sidecar derivado) armazenada no grafo no momento do carregamento.
    # Vazio quando não existe nenhum arquivo secundário, portanto a saída não anotada permanece idêntica em bytes.
    ...
    if len(output) > char_budget:
        ...
        # never cut. When every node is shown the answer is complete:
...
def _find_node_tiers(
    ...
    if not term:
        return []
    # Consulta normalizada com preservação de pontuação. `term` é tokenizado em \w+ (então
    # "blockStream.ts" -> "blockstream ts", espaço onde está o '.' era), mas um nó
    # armazenado `norm_label` mantém a pontuação ("blockstream.ts"). Correspondência somente via
    ...
    prefix: list[str] = []
    substring: list[str] = []
    # Pré-filtro de trigrama (ordem de iteração do grafo preservada de forma exata/prefixo/substring
    # ordenação - e, portanto, correspondências[0] - é idêntica em bytes à varredura completa).
...
def _shortest_path_text(G: nx.Graph, arguments: dict) -> str:
    ...
    for name, scored, nid in (
        ...
        # Só faz sentido quando a pontuação bruta é o que foi escolhido - um
        # a substituição completa do token foi escolhida na cobertura do token, não na pontuação.
...
def _community_header(cid: int, community_name) -> str:
    # Cabeçalho para get_community: "Comunidade N — Nome", correspondente a get_node/query
    # saída que lê o atributo community_name to_json e grava nos nós.
...
def _build_server(graph_path: str):
    ...
    try:
        ...
        raise ImportError('mcp not installed. Run: pip install "omnigraph[mcp]"') from e
    try:
    ...
    def _resolve_graph_path(project_path) -> str:
        ...
        resolves to ``<project_path>/<OMNIGRAPH_OUT>/graph.json``, honouring the
        OMNIGRAPH_OUT override so worktree/shared-output setups keep working."""
        if not project_path:
            return _default_graph_path
        return str(Path(project_path) / _paths.OMNIGRAPH_OUT / "graph.json")

    # Contexto ativo por solicitação, recuperado por _select_graph() e lido pela ferramenta
    ...
    try:
        ...
        # Nenhum grafo padrão na inicialização → executado como um servidor multiprojeto puro. Ferramentas
        # então exija project_path; uma chamada sem uma recebe um erro claro em vez
        # do que o processo se recusando a iniciar (que é o que _load_graph faria).
        G, communities = None, {}
        ...
        for _t in _tools:
            ...
            _schema.setdefault("properties", {})["project_path"] = {
                ...
                "description": (
                    ...
                    "omnigraph-out/graph.json. Optional — defaults to the graph "
    ...
    def _tool_get_community(arguments: dict) -> str:
        ...
        for n in nodes:
            d = G.nodes[n]
            # Limpe o rótulo e o source_file (F-010).
            lines.append(
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from omnigraph.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from omnigraph.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
        number = int(arguments["pr_number"])
        repo = arguments.get("repo") or None
        # Use gh pr view diretamente – funciona para qualquer branch base, não apenas para o padrão
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from omnigraph.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
        ...
        # Busque diferenças simultaneamente e calcule o impacto do grafo usando G na memória
    ...
    def _load_community_labels() -> dict[int, str]:
        labels_path = Path(active_graph_path).parent / ".omnigraph_labels.json"
    ...
    async def list_resources() -> list[types.Resource]:
        ...
        return [
            ...
            types.Resource(uri="omnigraph://god-nodes", name="God Nodes", description="Top 10 most-connected nodes", mimeType="text/plain"),
            types.Resource(uri="omnigraph://surprises", name="Surprising Connections", description="Cross-community surprising connections", mimeType="text/plain"),
            types.Resource(uri="omnigraph://audit", name="Confidence Audit", description="EXTRACTED/INFERRED/AMBIGUOUS edge breakdown", mimeType="text/plain"),
            types.Resource(uri="omnigraph://questions", name="Suggested Questions", description="Suggested questions for this codebase", mimeType="text/plain"),
        ]

    async def read_resource(uri: AnyUrl) -> str:
        _select_graph(None)  # recursos leem o grafo padrão do servidor
        uri_str = str(uri)
        if uri_str == "omnigraph://report":
            report_path = Path(active_graph_path).parent / "GRAPH_REPORT.md"
            if report_path.exists():
                return report_path.read_text(encoding="utf-8")
            return "GRAPH_REPORT.md not found. Run omnigraph extract first."
        if uri_str == "omnigraph://stats":
            return _tool_graph_stats({})
        if uri_str == "omnigraph://god-nodes":
            return _tool_god_nodes({"top_n": 10})
        if uri_str == "omnigraph://surprises":
            try:
                from omnigraph.analyze import surprising_connections
        ...
        if uri_str == "omnigraph://questions":
            try:
                from omnigraph.analyze import suggest_questions
    ...
    if hasattr(Server, "list_tools"):
        ...
        async def _on_read_resource(ctx, params) -> types.ReadResourceResult:
            ...
            mime = "text/markdown" if str(params.uri).startswith("omnigraph://report") else "text/plain"
        ...
        try:
            ...
            _version = _pkg_version("omnigraph")
        except Exception:
            _version = "0"
        server = Server(
            "omnigraph",
...
class _ApiKeyMiddleware:
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        if provided is None:
            # RFC 6750: o token do esquema de autenticação não diferencia maiúsculas de minúsculas.
            scheme, _, token = headers.get(b"authorization", b"").partition(b" ")
            if scheme.lower() == b"bearer" and token:
                provided = token.strip()
        # Comparação em tempo constante; rejeitar quando nenhuma chave foi fornecida.
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            ...
            'Run: pip install "omnigraph[mcp]"'
        ) from e

    # Uma chave em branco (por exemplo, --api-key "" ou uma OMNIGRAPH_API_KEY vazia) não deve ser
    # confundido com "auth on" - normalize-o para None para que o portão seja inequívoco.
    ...
    # O SDK rejeita um tempo limite não positivo e proíbe um no modo sem estado.
    ...
    async def lifespan(_app):
        # O gerenciador de sessão possui um grupo de tarefas anyio que deve agrupar todo o
        # vida útil do servidor, então insira-o aqui em vez de por solicitação.
...
def _main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m omnigraph.serve",
        description="Serve a omnigraph knowledge graph over MCP (stdio or Streamable HTTP).",
    )
    parser.add_argument(
        "graph_path",
        nargs="?",
        default=None,
        help="Path to graph.json (default: omnigraph-out/graph.json)",
    )
    parser.add_argument(
        "--graph",
        dest="graph_flag",
    ...
    parser.add_argument(
        "--api-key",
        default=os.environ.get("OMNIGRAPH_API_KEY"),
        help="Require this key on the HTTP transport (env: OMNIGRAPH_API_KEY)",
    )
