...
# (cache/ast/v{versão}/), com entradas de outras versões removidas primeiro
...
# Riscos de correção: `touch` causa um re-hash extra inofensivo; edições do mesmo tamanho
# dentro do NFS, o mtime de segunda resolução tem uma janela de 1 segundo (igual ao make).
...
def cached_word_count(path: Path, root: Path, compute, cache_root: "Path | None" = None) -> int:
    ...
    if st is not None:
        ...
        if (entry
                and entry.get("size") == st.st_size
                and entry.get("mtime_ns") == st.st_mtime_ns):
            entry["word_count"] = wc  # aumentar a entrada de hash existente no lugar
        else:
            _stat_index[abs_key] = {
...
def cache_dir(root: Path = Path("."), kind: str = "ast",
    ...
    AST entries live in omnigraph-out/cache/ast/v{version}/ — namespaced by
    omnigraph version because they depend on extractor code, not just file
