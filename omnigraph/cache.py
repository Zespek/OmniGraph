# cache de extração por arquivo - ignora arquivos inalterados na nova execução
...
# conteúdo significa que as correções do extrator enviadas em uma nova versão continuam obsoletas
# resultados pré-fixados. O cache AST é, portanto, nomeado por versão do pacote
# and cache-key schema (cache/ast/v{version}-s{schema}/), with entries from
# other versions or schemas removed on first
# usar. O cache semântico NÃO é versionado deliberadamente — suas entradas são
# produzido pelo LLM a partir do conteúdo do arquivo e invalidando-os a cada
# release cobraria novamente a extração de arquivos inalterados.
try:
    from importlib.metadata import version as _pkg_version

    _EXTRACTOR_VERSION = _pkg_version("omnigraph")
except Exception:
...
# Os diretórios de versão já varreram esse processo – a limpeza é executada uma vez por (base, versão).
_cleaned_ast_dirs: set[str] = set()


def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other omnigraph versions.
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`omnigraph.llm._extraction_system`) or a Path to the
...
def _body_content(content: bytes) -> bytes:
    ...
    if closer is None:
        return content
    # Fatie logo após o fechamento `---` (não após sua linha) para que a saída
    # permanece idêntico em bytes à implementação histórica para bem formado
    # frontmatter - os hashes de cache semântico existentes não devem ser alterados.
...
def _mtime_granularity_ns() -> int:
    ...
    """
    raw = os.environ.get("OMNIGRAPH_MTIME_GRANULARITY_MS", "").strip()
    if raw:
        try:
            ms = float(raw)
        except ValueError:
...
def _stat_sig_fresh(entry: object, st: "os.stat_result") -> bool:
    ...
    Entries written by an older omnigraph carry no ``indexed_at_ns``; they are
...
def _stat_key_to_relative(key: str, anchor: Path) -> str:
    ...
    Local duplicate of :func:`omnigraph.detect._to_relative_for_storage` —
...
def _stat_key_to_absolute(key: str, anchor: Path) -> str:
    ...
    written by an older omnigraph remains readable.
    """
    p = Path(key)
    if p.is_absolute():
        return str(p)
...
def _flush_stat_index() -> None:
    ...
    for k, v in _stat_index.items():
        ...
        on_disk[dk] = v
    # Never resurrect a corpus that was deleted while omnigraph was running
    #: a hook-launched `omnigraph update. &` in a short-lived worktree
    ...
    # written. Creating omnigraph-out/cache/ under a root that still exists is
...
def file_hash(path: Path, root: Path = Path("."), cache_root: "Path | None" = None) -> str:
    ...
    # O índice estatístico é um artefato de cache, portanto deve seguir a localização do cache
    # (cache_root), não a raiz da âncora-chave - caso contrário, deixa um erro perdido
    # omnigraph-out/cache/stat-index.json dentro da árvore de origem analisada, mesmo quando
    # o próprio cache AST é redirecionado para CWD (conclusão).
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`omnigraph.watch._relativize_source_files` so cached
    ...
    :func:`omnigraph.detect._to_relative_for_storage`.
    """
    try:
    ...
    # definition_file is a path into the scanned tree exactly like
    # source_file; a cache entry keeping it absolute replayed the build host's
    # layout on every warm hit.
...
def _normalize_source_file_value(src: "str | Path", root_resolved: Path) -> str:
    ...
    ``omnigraph update`` runs with a machine-specific ``source_file`` (#2197).
...
def _absolutize_ids_in(payload: dict, path: "str | Path", root: Path) -> None:
    ...
    stored form (``$omnigraph-root$_pkg_mod_base``) is a storage encoding, not a valid id,
...
def _absolutize_source_files_in(payload: dict, root: Path) -> None:
    ...
    for bucket in ("nodes", "edges", "hyperedges", "raw_calls"):
        for item in payload.get(bucket, []):
            if not isinstance(item, dict):
                continue
            # Mirror of the relativize side: definition_file re-anchors too.
            for key in ("source_file", "definition_file"):
                source = item.get(key)
                if not source:
                    continue
...
def cache_dir(root: Path = Path("."), kind: str = "ast",
    ...
    AST entries live in omnigraph-out/cache/ast/v{version}-s{schema}/, namespaced
    by omnigraph version and cache-key schema because they depend on extractor
    code and key semantics, not just file contents. Semantic entries are still
    NOT version-namespaced (re-extraction
    costs LLM calls, #1252): they live in omnigraph-out/cache/semantic/, with
    deep-mode entries beside them in omnigraph-out/cache/semantic-deep/.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast",
    ...
    Cache value: stored as omnigraph-out/cache/{kind}/{hash}.json (AST entries
    ...
    AST entries written by other omnigraph versions — including the legacy
    ...
    if entry.exists():
        ...
        if not allow_partial and isinstance(result, dict) and result.get("partial"):
            return None
        # A semantic entry with zero nodes and zero hyperedges is invalid:
        ...
        # ensures the file is re-dispatched and retried.
        if (
            not allow_partial
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast",
    ...
    Stores as omnigraph-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def prune_semantic_cache(root: Path, live_hashes: set[str]) -> int:
    ...
    extraction-spec) can share one omnigraph-out/, and a wholesale sweep would
...
def save_semantic_cache(
    ...
    def resolved_source_path(value: str | Path) -> Path:
        ...
        try:
            ...
            # Mantenha o melhor esforço de gravação do cache para caminhos inacessíveis ou um
            # loop de link simbólico emitido por um resultado semântico não confiável.
    ...
    # Poda de referência pendente. Um grupo de nós é ignorado pela gravação
    # loop abaixo quando seu source_file não é um arquivo real (caminho fantasma) ou é
    # fora do escopo de acordo com a proteção # 1757 - mas uma aresta/hiperedge em um PERMITIDO
    # grupo que faz referência a um ID de nó de um grupo ignorado costumava ser gravado
        ...
        for fpath, result in by_file.items():
            ...
            for n in result["nodes"]:
                ...
                try:
                    ...
                    continue
                target.add(nid)
        # Um nó de atribuição duplicada (definido em um nó ignorado E escrito
        # group) ainda atinge o cache — não limpe demais as referências a ele.
        ...
        if skipped_ids:
            ...
            def edge_dangles(e: dict) -> bool:
                try:
                    ...
                    # Endpoint não hashável de um resultado não confiável; deixe
                    # para validação em tempo de construção, em vez de falhar no salvamento.
                    return False
    ...
    for fpath, result in by_file.items():
        ...
        if p.is_file():
            ...
            # standalone extraction: edge-only or empty results must not be
            # cached, so that subsequent runs can re-dispatch and retry the file.
