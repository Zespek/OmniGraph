# buscar URLs (tweet/arxiv/pdf/web) e salvar como marcação anotada
...
from omnigraph.security import safe_fetch, safe_fetch_text, validate_url
...
def _html_to_markdown(html: str, url: str) -> str:
    ...
    # Sempre pré-retira o script/estilo para que o conteúdo do texto nunca vaze na saída
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a omnigraph-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from omnigraph.transcribe import download_audio
    ...
    out_path = target_dir / filename
    # Evite sobrescrever - anexe o contador, se necessário
    counter = 1
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically omnigraph-out/memory/) with YAML frontmatter
    that omnigraph's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `omnigraph reflect` can
    ...
    # later write_text silently replaces the earlier one. The short uuid
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a omnigraph /raw folder")
    ...
    print(f"Ready for omnigraph: {out}")
