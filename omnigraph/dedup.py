"""Entity deduplication pipeline for omnigraph knowledge graphs.
...
from omnigraph._minhash import MinHash, MinHashLSH
...
def _make_minhash(text: str, num_perm: int = 128) -> MinHash:
    # Remova os espaços para que "extrator de grafo" e "extrator de grafo" compartilhem telhas
...
# Corresponde aos rótulos cujo token final é um sufixo de versão/variante:
# dígitos opcionalmente seguidos por letras (SKUs de chip: ASR1603, M1, Cortex-A55)
# ou mais de 2 letras (revisões de codinome: Cranelr vs Cranel).
# Requer que o radical termine em uma letra para que palavras simples não correspondam acidentalmente.
...
def _short_label_blocked(a: str, b: str, jw_score: float) -> bool:
    ...
    # Permitir apenas substituições de caracteres únicos do mesmo comprimento (erros de digitação verdadeiros como "Extractor"/"Extractar").
    # Bloqueie pares com comprimentos diferentes, independentemente da pontuação.
...
# sense — two files' `## Decisions` sections are two sections.
#
    ...
    for loser in losers:
        ...
        if lose_file == keep_file:
            if _norm(lose_label) != _norm(keep_label):
                print(
                    f"[omnigraph] note: node '{nid}' was extracted twice from "
            ...
            continue  # o perdedor apenas faz referência à entidade que o sobrevivente define
        else:
            print(
                f"[omnigraph] WARNING: node '{nid}' is minted by two different files — "
                ...
                f"dropped node is lost. To keep them distinct, run 'omnigraph extract' "
                f"per subfolder and merge with 'omnigraph merge-graphs'.",
                file=sys.stderr,
            )
...
def deduplicate_entities(
    ...
    *,
    ...
    """
    # Guard: a desduplicação entre projetos não é suportada — nós de diferentes repositórios
    # compartilham nomes de rótulos por coincidência e nunca devem ser mesclados por similaridade de strings.
    # Se você precisar desduplicar um grafo global, execute primeiro deduplicate_entities por repositório.
    ...
    for node in unique_nodes:
        ...
        # símbolos distintos com o mesmo nome nunca são mesclados por similaridade de string.
        if _is_code(node):
            continue
    ...
    for key, group in norm_to_nodes.items():
        ...
        for sf, file_group in by_file.items():
            if not sf:
                # No source_file — não é possível provar o mesmo símbolo; pular para evitar
                # colapsando nós distintos que compartilham um rótulo.
                continue
            if len(file_group) > 1:
                if protected_set:
        ...
        # unify across files — code is keyed by ID and
        ...
        # reads as an entity inside its file: a file-anchored
        ...
        if len(mergeable) > 1:
            if protected_set:
                ...
                if not inc_members:
                    # All participants belong exclusively to untouched files:
    ...
    # ── passo 2: MinHash/LSH + Jaro-Winkler (somente nós de alta entropia) ─────────
    candidates: list[dict] = []
    seen_norms: set[str] = set()
    for node in unique_nodes:
        # Os símbolos de código também são excluídos da correspondência difusa: duas funções com
        # nomes longos semelhantes em arquivos diferentes (backends paralelos, irmãos
        # classes) não devem ser mescladas de forma difusa, e uma correspondência difusa de código↔conceito deve
        # não unir transitivamente dois símbolos de código distintos por meio de um conceito.
        if _is_code(node):
            continue
        key = _norm(node.get("label", node.get("id", "")))
    ...
    if len(candidates) >= 2:
        ...
        # Pré-construa estruturas de pesquisa O(1) para que o loop de consulta abaixo não seja verificado
        # os candidatos são listados linearmente para cada vizinho LSH (era O (n² × B)).
        ...
        for node in candidates:
            ...
            try:
                ...
                pass  # chave duplicada no LSH – já inserida
        ...
        for node in candidates:
            ...
            for neighbor_id in neighbors:
                ...
                if _hi.startswith(_lo) and _hi != _lo:
                    continue
                # Irmãos numerados/versionados e ancorados em arquivos cruzados
                # clichê (justificativa/documento) são decisivamente distintos
    ...
    # ── passo 3: desempate LLM para pares ambíguos (opt-in) ──────────────────
    ...
    # ── construir tabela de remapeamento a partir de componentes de localização de união ──────────────────────────
    ...
    for root, members in components.items():
        ...
        # same-source collision path already applies; loser
    ...
    total = len(remap)
    msg = f"[omnigraph] Deduplicated {total} node(s)"
    # Ambos os contadores são relatados quando diferentes de zero. O formulário anterior aninhou o difuso
    # ramificação dentro de `if exact_merges`, descartando silenciosamente a contagem difusa em
    # doc/semantic-heavy é executado onde o Passo 1 não encontra nada.
    parts: list[str] = []
    if exact_merges:
    ...
    for edge in edges:
        ...
        # Remova as chaves legadas para que não vazem para os atributos de aresta em graph.json.
        e.pop("from", None)
        e.pop("to", None)
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from omnigraph.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[omnigraph] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[omnigraph] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        ...
        print(
            f"[omnigraph] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[omnigraph] --dedup-llm batch failed: {exc}", flush=True)
