"""julia — moved verbatim from omnigraph/extract.py."""
from __future__ import annotations

from omnigraph.extractors.base import _file_stem, _make_id, _read_text
from omnigraph.extractors.engine import _semantic_reference_edge
...
def extract_julia(path: Path) -> dict:
    ...
    def walk(node, scope_nid: str) -> None:
        ...
        if t == "struct_definition":
            ...
            # Tipos de campo: cada `name::Type` é reduzido para um filho typed_expression de struct_definition
            for child in node.children:
        ...
        if t == "assignment":
            ...
            if lhs and lhs.type == "call_expression" and lhs.children:
                ...
                if callee.type == "identifier":
                    ...
                    add_node(func_nid, f"{func_name}()", line)
                    add_edge(scope_nid, func_nid, "defines", line)
                    # Ande apenas no RHS (índice 2 após lhs e operador) para evitar auto-loops
