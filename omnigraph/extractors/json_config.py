"""Json_config extractor. Moved verbatim from omnigraph/extract.py."""
...
def _is_config_json(path: Path, obj_node, source: bytes) -> bool:
    ...
    if name.endswith((".eslintrc.json", ".prettierrc.json", ".babelrc.json",
        ...
        return True
    # Teste de chave de nível superior: verifica as chaves imediatas do objeto raiz (sem caminhada profunda).
...
def extract_json(path: Path) -> dict:
    ...
    def walk_object(obj_node, parent_nid: str, parent_key: str | None,
        ...
        for child in obj_node.children:
            ...
            if not key:
                continue
            # Uma chave que normaliza para nada (uma chave de comentário JSONC `"//"`, digamos)
            # recolheria `_make_id(stem, key)` até o id do arquivo-stem simples,
            # que é derivado do caminho absoluto e vaza o caminho de varredura. Tal
            # as teclas não carregam sinal grafo, então descarte-as.
            ...
            if val.type == "object":
                ...
                # Para matrizes "estende" (tsconfig, eslint): cada elemento de string.
                ...
                if key == "extends" and val_text:
                    # Referências externas de namespace para evitar colisão de ID com nós de arquivo (J-4)
                    ...
                    # Valores $ref do namespace para evitar sequestro de aresta em nós de código (J-4)
                    ref_nid = _make_id("ref", val_text)
                    if ref_nid:
    ...
    if doc.type == "document" and doc.child_count > 0:
        doc = doc.children[0]
    if doc.type == "object":
        # Somente JSON de configuração/manifesto reconhecido pela extração AST. JSON de dados (luminárias,
        # conjuntos de dados, GeoJSON, despejos de API) é ignorado para não explodir em
        # nós-chave órfãos; resta a passagem semântica do LLM.
        ...
        # Matriz de nível superior ou escalar => dados JSON, nunca uma configuração/manifesto.
