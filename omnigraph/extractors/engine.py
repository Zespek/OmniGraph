    ...
    def walk_calls(
        ...
        if node.type in config.call_types:
            ...
            if config.ts_module == "tree_sitter_swift":
                ...
                if recv is not None:
                    ...
                    if recv.type in ("identifier", "constant"):
                        ...
                        # Receptor com namespace `Billing::Processor.call` — captura o
            ...
            if callee_name and callee_name not in _LANGUAGE_BUILTIN_GLOBALS:
                # Uma chamada de membro receptor em maiúscula (`ClassName.method()`) deve adiar
                # para resolução de arquivos cruzados baseada em receptor: o nome do método simples pode
                # colidir com um nó no arquivo - até mesmo o próprio método de chamada, quando um
                # a ação viewset delega para uma ação de serviço com o mesmo nome - o que seria
                # combine `tgt_nid == caller_nid` e desligue silenciosamente a chamada. O
                # o receptor capturado é resolvido posteriormente em _resolve_python_member_calls.
                ...
                if tgt_nid and tgt_nid != caller_nid:
                    ...
                    # table (, per-method since). `this.field.M()` is
