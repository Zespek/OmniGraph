...
`omnigraph/extract.py` is being split into this package, one language per PR
...
3. **Facade re-export is mandatory.** `extract.py` must keep exporting every
   moved name (`from omnigraph.extractors.<mod> import extract_<lang>  # noqa: F401`
...
4. **Never import from `omnigraph.extract` inside this package.** Import
...
- run `grep -c '_name' omnigraph/extract.py` AFTER your candidate move;
...
2. `grep -n 'def extract_<lang>' omnigraph/extract.py`; the span ends at the
...
3. Save the span to a temp file. Create `omnigraph/extractors/<lang>.py` with
   module docstring (`"""<Lang> extractor. Moved verbatim from omnigraph/extract.py."""`),
