"""Sln extractor. Moved verbatim from omnigraph/extract.py."""
...
def extract_sln(path: Path) -> dict:
    ...
    for m in _PROJECT_RE.finditer(src):
        ...
        # Uma pasta de solução é um agrupamento VIRTUAL, não um arquivo: o Visual Studio grava
        # seu nome como nome de exibição e "caminho" (proj_name == proj_path,
        # nenhum arquivo real). Resolvendo-o para um caminho absoluto e desativando o ID do nó
        # que vazou o caminho de verificação absoluto (incluindo o nome de usuário do sistema operacional) em graph.json,
        # porque a relativização de id da CLI apenas remapeia ids de arquivos reais no
        # conjunto de varredura — uma pasta virtual nunca corresponde, então seu ID absoluto sobreviveu
        #. Use o próprio nome da pasta (relativo, sem resolução de sistema de arquivos).
