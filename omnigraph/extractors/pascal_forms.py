"""Pascal_forms extractor. Moved verbatim from omnigraph/extract.py."""
...
def extract_lazarus_form(path: Path) -> dict:
    ...
    # Pilha de IDs de nós que representam o aninhamento de objetos... blocos finais
...
def extract_delphi_form(path: Path) -> dict:
    ...
    # Detectar DFM binário: fluxos de recursos binários Delphi começam com FF 0A
    if raw[:2] == b"\xff\x0a":
        return {
            "nodes": [], "edges": [],
    ...
    # Texto DFM — delegar ao analisador de formulário compartilhado (mesma sintaxe de .lfm)
    try:
