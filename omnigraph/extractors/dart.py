"""Dart extractor. Moved verbatim from omnigraph/extract.py."""
...
def extract_dart(path: Path) -> dict:
    ...
    # Remova comentários embutidos e multilinhas, deixando os literais de string intactos para evitar a remoção de URLs/caminhos dentro de strings
    ...
    def _comment_replace(match: re.Match) -> str:
        ...
        if token.startswith("/"):
            ...
            # line number up by the comment's own height.
            return "\n" * token.count("\n")
        return token
    ...
    # 1. Declarações de classes, mixins e enums (com herança, mixins, interfaces e genéricos)
    # Suporta vários modificadores combinados (por exemplo, classe base abstrata, classe mixin) sem capturar "classe" como nome
    ...
    for m in re.finditer(class_pattern, src_clean, re.MULTILINE):
        ...
        # that precede the declaration, reporting the line too early.
        ...
        # Analise manualmente extensões/ligações, com e implementações no cabeçalho para lidar com colchetes genéricos aninhados balanceados
        start_idx = m.end()
        ...
        # Extraia o corpo da classe para dependências precisas da estrutura e manipulação de eventos
        start_idx = m.start()
        ...
        if has_body:
            ...
            # Emissões do estado do bloco: emit(MyState) ou yield MyState
            ...
            # Adições de eventos de bloco: widget.add(MyEvent()) ou bloc.add(MyEvent())
            ...
            # Referências de widget para bloco: BlocBuilder<MyBloc, ...>
    ...
    for am in re.finditer(annotation_pattern, src_clean):
        ...
        if target_nid and target_name:
            ...
            if ";" not in actual_intervening and "}" not in actual_intervening and "{" not in actual_intervening:
                ...
                # Mapeamento de geração de provedor específico do Riverpod (suporta classe camelCase e provedores funcionais)
    ...
    # 3. Extensões (extensão MyExt em MyClass)
    ...
    # 4. Declarações de variáveis ​​de nível superior e de classe (variáveis ​​genéricas, registros, atrasos e desestruturação)
    # Restrinja o recuo a 0-2 espaços para evitar a correspondência de variáveis ​​locais dentro de funções ou expressões de troca
    ...
    # 5. Funções/métodos de nível superior e de membro (suporta tipos de retorno digitados/genéricos/de registro e referências Riverpod/Bloc)
    # Restrinja o recuo a 0-2 espaços para evitar a correspondência de funções ou métodos locais aninhados dentro de instruções switch multilinhas
    ...
    for m in re.finditer(method_pattern, src_clean, re.MULTILINE):
        ...
        # Obtenha o corpo da função usando chave correspondente para extrair padrões de referência do Riverpod
        start_idx = m.start()
        brace_pos = src_clean.find("{", start_idx)
        semi_pos = src_clean.find(";", start_idx)
        ...
        if has_body:
            ...
            # Extraia adições de eventos do bloco: widget.add(MyEvent()) ou bloc.add(MyEvent())
