"""Bash extractor. Moved verbatim from omnigraph/extract.py."""
...
def extract_bash(path: Path) -> dict:
    ...
    # file_nid é totalmente derivado do caminho e nunca produzido por _make_id(stem, func_name),
    # portanto, anexar "__entry" garante um ID distinto de qualquer nó de função.
    ...
    # Tipos de nós pai que significam que um comando contido faz parte de uma substituição
    # ou expansão, não uma chamada de função real. Erros de filtragem em nível de token
    # isso porque `$(build)` expõe `build` como um comando filho cujo nome
    # token não possui metacaracteres — apenas o pai possui.
    ...
    def _bash_func_name(node) -> str | None:
        ...
        # gramática bash: function_definition tem uma palavra filho (o nome)
        ...
        for child in body_node.children:
            ...
            if child.type == "command" and _bash_call_in_command_allowed(child):
                ...
                if cmd_name_node:
                    ...
                    # As funções definidas vencem. Listas de ignorados para comandos externos
                    # criaria falsos negativos quando um usuário definisse um
                    # função que sombreia um externo (`install`, `find`, etc.).
    ...
    def walk(node, parent_nid: str) -> None:
        ...
        if t == "function_definition":
            ...
            if name:
                ...
                # encontre o corpo da declaração composta
                body = None
                for child in node.children:
        ...
        if t == "command":
            ...
            if cmd_name_node:
                ...
                if cmd in _BASH_SOURCE_COMMANDS and cmd not in defined_functions:
                    # encontre o argumento do caminho (primeira palavra após o nome do comando)
                    if args:
                    ...
                    if raw is None:
                        ...
                        # "$DIR/lib/x.sh"`. `_bash_source_suffix`
        ...
        if t == "declaration_command":
            # exportar/declarar/somente leitura VAR=valor no nível do programa
    ...
    # Pré-passagem: coleta todos os nomes de funções definidos para que o manipulador de comando de origem
    # em walk() pode detectar funções definidas pelo usuário que ocultam 'source' / '.'
    # independentemente da ordem de definição no arquivo.
