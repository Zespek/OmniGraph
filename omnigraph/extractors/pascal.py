"""pascal — moved verbatim from omnigraph/extract.py."""
from __future__ import annotations

import re
from omnigraph.extractors.base import _file_stem, _make_id
from omnigraph.extractors.resolution import _pascal_resolve_class, _pascal_resolve_unit
...
def _extract_pascal_regex(path: Path) -> dict:
    ...
    # Declarações de tipo (classes/interfaces) na seção de interface
    ...
    while pos < len(search_text):
        ...
        # Encontre o corpo da classe (até o próximo final;)
        ...
        # Encaminhar declarações de método dentro do corpo da classe
    ...
    # Arestas de chamada intra-arquivo, com escopo definido pela própria classe do chamador, então seu
    # cadeia ancestral (via arestas `herdadas` já emitidas acima), então
    # funções livres em nível de arquivo; volte para uma correspondência global apenas por nome
    # quando é inequívoco (único proprietário no arquivo). Previne
    # métodos com o mesmo nome em classes não relacionadas (acessadores de propriedade, gerados
    # classes wrapper, como unidades de importação TLB, etc. - um Pascal/Delphi comum
    # padrão) de colapsar em uma aresta de classe cruzada arbitrária.
