"""Powershell extractor. Moved verbatim from omnigraph/extract.py."""
...
def extract_powershell(path: Path) -> dict:
    ...
    _PS_SKIP = frozenset({
        ...
        # Comandos de importação — tratados como arestas de importação, não como chamadas de função
        "import-module",
    })
    ...
    def walk(node, parent_class_nid: str | None = None) -> None:
        ...
        if t == "class_method_definition":
            ...
            if name_node:
                ...
                # Tipo de retorno: type_literal irmão de simple_name
                        ...
                        for el in child.children:
                            if el.type == "command_parameter":
                                ...
                                if module_name is None or expect_name:
                                    ...
                                    expect_name = False
                    if module_name:
                        # Extensão de tira; mantenha apenas o radical do ID do nó
...
def _psd1_collect_string_literals(node, source: bytes) -> list[str]:
    ...
    def _walk(n) -> None:
        if n.type == "string_literal":
            ...
            # Remova os caracteres de citação ao redor (' ou ")
...
def extract_powershell_manifest(path: Path) -> dict:
    ...
    def walk_manifest(node) -> None:
        ...
        if key_text == "RootModule":
            ...
            def find_modulename_entries(n) -> None:
                if n.type == "hash_entry":
                    ...
                    if sub_key is not None:
                        ...
                        # Colete strings dentro de *todas* as subchaves para que possamos excluí-las
                        for c in n.children:
                            if c.type == "pipeline":
                    ...
                    return  # não recorra mais a este hash_entry
                for child in n.children:
            ...
            # Agora reúna literais de string diretos que não estejam dentro de entradas de hash
