"""Sql extractor. Moved verbatim from omnigraph/extract.py."""
...
# the whole-file has_error fallback. They MUST share one pattern: when
...
# PL/pgSQL routine and every bracket-named T-SQL procedure. T-SQL's
...
def extract_sql(path: Path, content: str | bytes | None = None) -> dict:
    ...
    try:
        ...
        # different Python ABI) raises ImportError here too. Reporting
    ...
    table_nids: dict[str, str] = {}  # nome → nid para resolução de referência
    ...
    def walk(node) -> None:
        ...
        if t == "create_table":
            ...
            if name:
                ...
                _add_node(nid, name, line)
                table_nids[_norm_ident(name)] = nid
                # Referências de tabela FROM/JOIN dentro do corpo da view
            ...
            # ON <table> (...). Unlike CREATE POLICY the grammar
            # parses this statement fine; the walk simply never dispatched on
            # it, so every index was silently dropped. The name is the
    ...
    for stmt in root.children:
        if stmt.type == "statement":
            ...
            # are statement nodes, not direct create_table nodes.
            walk(stmt)
        elif stmt.type in ("fb_proc_or_trigger", "set_term", "declare_external_function", "ERROR"):
            walk(stmt)

    # Fallback de regex global: capture quaisquer REFERÊNCIAS perdidas devido a nós ERROR na árvore de análise
    # (por exemplo, as colunas COMPUTED BY do Firebird eliminam totalmente as restrições da árvore).
    # Instantâneo após caminhada na árvore para não reemitir arestas já capturadas acima.
