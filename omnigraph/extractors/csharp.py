...
still lives in ``omnigraph/extract.py``; per ``extractors/MIGRATION.md`` the
...
class CsharpNameResolver:
    ...
    (``_resolve_csharp_member_calls`` in ``omnigraph/extract.py``) can type a
    ...
    def resolve_qualified(
        ...
        # Resolução qualificada de som: um alias no escopo para Q sombreia o namespace Q. Para um qualificado
        # ref Q.label, procure (alias_target_namespace, rótulo). Se não houver nenhum alias no escopo, passe para um
        # namespace conhecido exato. Pendure na ambigüidade / sem acerto / qualificador desconhecido.
