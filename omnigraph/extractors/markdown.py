...
# not a declared dependency of omnigraph (see manifest_ingest._parse_apm for the
...
# same lost-edge failure fixed for hub docs, reintroduced for every
...
def _active_scan_root() -> "Path | None":
    ...
    fallback stays off.
    """
    import omnigraph.extract as _extract
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from omnigraph.detect import _SKIP_DIRS
