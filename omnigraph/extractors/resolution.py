...
# source actually lives behind is skipped. This is the order
...
# is attributed to code that never imports it.
...
def _package_entry_candidates(
    ...
    if subpath:
        ...
        # the whole import down with it. Targets that escape the
...
def _find_js_project_anchor(start_dir: Path) -> Path:
    ...
    """
    from omnigraph.detect import _find_vcs_root
...
def _resolve_js_module_path(raw: str | Path, start_dir: Path | None = None) -> Path | None:
    ...
    # after tsconfig `paths` so an explicit alias keeps precedence.
    if raw.startswith("#"):
...
def _apply_symbol_resolution_facts(
    ...
    # name to a class/interface member. Track those keys separately
...
def _js_top_level_function_bodies(path: Path, root_node, source: bytes) -> list[tuple[str, object]]:
    ...
    #. Unwrap a non-re-export export_statement to its inner declaration
...
def _resolve_python_namespace_dir(module_name: str, current_path: Path, root: Path, level: int) -> "Path | None":
    ...
    for anc in current_path.parents:
        ...
        if anc == root or (anc / "__init__.py").is_file():
            continue  # root already probed; a package dir is not a sys.path root
