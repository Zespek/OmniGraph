"""resolution — moved verbatim from omnigraph/extract.py."""
...
def _resolve_js_import_path(candidate: Path) -> Path:
    ...
    # Convenção TS ESM: as importações geralmente soletram .js/.jsx enquanto a origem é .ts/.tsx.
    ...
    # Somente retorne aos índices de diretório depois que os candidatos ao arquivo perderem.
    if candidate.is_dir():
...
def _strip_jsonc(text: str) -> str:
    ...
    # Remova as vírgulas finais antes de } ou ] (permitindo espaços em branco entre eles).
...
def _read_tsconfig_aliases(tsconfig: Path, base_dir: Path, seen: set) -> dict[str, list[str]]:
    ...
    for ext in extends_list:
        # Ignore as configurações do pacote npm com escopo (por exemplo, @tsconfig/svelte) - não no disco.
    ...
    # tsconfig `paths` são resolvidos em relação a `baseUrl` (ele próprio em relação a
    ...
    for alias, targets in paths.items():
        if not targets:
            continue
        # Mantenha TODOS os alvos na ordem declarada – tsc tenta cada um até que um resolva
        # no disco. Descartando os substitutos importações mal resolvidas/descartadas
        # cujo arquivo residia em um alvo não-primeiro. Preservar tokens curinga em
        # ambos os lados até que o resolvedor substitua o segmento capturado, então
        # normaliza o caminho concreto. Entradas vazias/sem string são ignoradas.
        target_patterns = [
            str(paths_base / t)
            for t in targets
...
def _match_tsconfig_alias(raw: str, pattern: str) -> "tuple[tuple[int, int], str, bool] | None":
    ...
    rule. The final branch preserves OmniGraph's existing support for treating a
...
def _resolve_tsconfig_alias(raw: str, aliases: dict[str, list[str]],
    ...
    for target in targets:
        if is_wildcard:
            # O TypeScript substitui apenas quando a estrela correspondente não está vazia.
...
def _resolve_js_import_target(raw: str, str_path: str) -> "tuple[str, Path | None] | None":
    ...
    # produzindo um fantasma multilíngue confiável (EXTRAÍDO) imports_from edge
    #. O alvo ref-namespaced não tem nó, então build o descarta como um
    # referência externa — o resultado correto para uma importação de terceiros.
...
def _resolve_lua_import_target(raw_module: str, str_path: str) -> str:
    ...
    if start_dir is not None:
        probe = start_dir
        # Suba alguns níveis para que os requisitos dos arquivos aninhados ainda sejam resolvidos quando
        # a raiz do pacote está acima do arquivo de importação.
        for _ in range(6):
...
def _disambiguate_colliding_node_ids(
    ...
    for old_id, group in by_id.items():
        ...
        # Salgue o ID de colisão com o *caminho* de onde ele veio. O sal ingênuo é
        ...
        # source_keys que, após a normalização, não são únicos entre si.
        seen: dict[str, int] = {}
    ...
    # Um C/ObjC/C++ `#include "foo.h"` / `#import "foo.h"` resolve para o cabeçalho
    ...
    for edge in edges:
        ...
        # imports/imports_from sempre tem como alvo um arquivo de cabeçalho, então eles devem resolver para
        # a variante do cabeçalho ANTES do salt do mesmo arquivo de origem é considerada. Codificação
        # o destino de importação pelo próprio arquivo de origem do importador aponta erroneamente um `.m`
        # importando seu próprio `.h` de volta para si mesmo (self-loop), e é errado para qualquer
        # importação de arquivos cruzados cujo importador compartilha o ID de colisão.
...
def _apply_symbol_resolution_facts(
    ...
    # emite arestas imports_from arquivo para arquivo para importações de submódulos em formato de pacote.
...
def _ts_walk_class_members(class_node, source: bytes, path: Path, class_nid: str,
    ...
    for child in class_node.children:
        if child.type == "class_heritage":
            ...
            # são os mesmos tipos de nó que extends_clause contém, então o auxiliar é
            # reutilizável. Sem esta ramificação, a herança da interface é eliminada.
...
def _collect_js_symbol_resolution_facts(paths: list[Path], facts: _SymbolResolutionFacts) -> None:
    ...
    for path in js_paths:
        ...
        for node in _walk_js_tree(root_node):
            ...
            # `export default class Foo {}` / `export default foo` expõe o
            # símbolo sob o nome "padrão"; registre isso para uma importação padrão
            # (imported_name="default") resolve isso. `exportar {X como padrão}`
            # já é tratado através do caminho export_clause acima.
...
def _resolve_cross_file_imports(
    ...
    # o nome é conhecido – o primeiro escritor vence quando os nomes colidem (inerentemente ambíguos).
    ...
    # Passo 2: para cada arquivo, encontre `from .X import A, B, C` e resolva
    new_edges: list[dict] = []
    ...
    for file_result, path in zip(per_file, paths):
        ...
        # Encontre todas as classes definidas neste arquivo (os importadores).
        # Exclui nós de lógica cujos rótulos não terminam em ")" ou ".py"
        # mas que nunca devem ser tratadas como entidades importadoras.
        local_classes = [
        ...
        # Analisar importações deste arquivo
        try:
            source = path.read_bytes()
        ...
        def walk_imports(node) -> None:
            if node.type == "import_from_statement":
                ...
                # índice secundário de radical simples (o primeiro escritor vence quando os nomes colidem).
                    ...
                    if child.type == "relative_import":
                        for sub in child.children:
                            if sub.type == "dotted_name":
                                ...
                                bare = raw.split(".")[-1]
                                # Resolva a importação relativa para o radical qualificado exato.
                ...
                # Colete nomes importados: dotted_name filhos de import_from_statement
                # que vêm DEPOIS do token de palavra-chave 'importar'.
                ...
                for child in node.children:
                    ...
                    if child.type == "dotted_name":
                        ...
                        # `importar X como Y` - pegue o nome original
                        name_node = child.child_by_field_name("name")
                        if name_node:
                            imported_names.append(
...
def _merge_decl_def_classes(
    ...
    """
    # Agrupe cada nó de código por ID, registrando os arquivos de origem distintos envolvidos.
    by_id: dict[str, list[dict]] = {}
    for n in all_nodes:
    ...
    # Identifique, por ID sobrevivente, qual nó manter (preferencialmente o cabeçalho). Nós não podemos
    # altere all_nodes no meio da varredura, então colete um conjunto de IDs de objetos de nó para descartar.
    ...
    for nid, group in by_id.items():
        ...
        if not ok:
            continue
        # Tudo de uma família irmã (dir, base_stem), com um cabeçalho UNIQUE.
...
def _resolve_cross_file_java_imports(
    ...
    # Passo 1: nome da classe → índice node_id (somente nomes internos iniciados em letras maiúsculas)
    ...
    # Passo 2: resolver importações para IDs de nós reais
    new_edges: list[dict] = []
...
def _resolve_java_type_references(
    ...
    # pacote + nome simples-> importações FQN, codificadas pela string source_file o
    # os próprios nós do arquivo usam (para que corresponda exatamente ao edge/node source_file).
    ...
    # FQN (package.Class) -> ID do nó de definição, para definições do tipo tipo com uma fonte.
    ...
    # Stubs de sombra nua: sem source_file, rótulo tipo tipo.
    ...
    # `imports` é incluído para que a aresta de importação em nível de arquivo que também chega ao
    # O stub de sombra também é apontado novamente, deixando o stub sem referência (e descartado).
    # As importações externas/stdlib nunca são resolvidas (sem correspondência interna def/mesmo pacote),
    # para que suas arestas permaneçam corretamente no topo. `referências` (campo/parâmetro/
    # usos de tipo de retorno) é incluído para que uma referência de módulo cruzado para um mesmo nome
    ...
    for edge in all_edges:
        ...
        if resolved is None:  # referência do mesmo pacote (sem importação explícita)
    ...
    # Drop shadow stubs que não fazem mais referência de aresta.
...
def _resolve_php_type_references(
    ...
    for path, result in zip(paths, per_file):
        ...
        def _record_use_clause(clause, prefix: str) -> None:
            ...
            for c in clause.children:
                if c.type in ("function", "const"):
                    return  # não é uma importação de classe
                if c.type == "as":
                    saw_as = True
        ...
        if len(set(namespaces)) > 1:
            continue  # arquivo com vários namespaces (violação de PSR-1): mantém o comportamento legado
    ...
    # FQN inferior -> ID do nó de definição (nomes de classes PHP não diferenciam maiúsculas de minúsculas).
    fqn_to_id: dict[str, str] = {}
    for node in all_nodes:
        label = node.get("label", "")
        src = node.get("source_file", "")
    ...
    for edge in all_edges:
        ...
        if raw and "\\" in raw:
            ...
            continue  # sem fatos de namespace: a religação de rótulo exclusivo herdado se aplica
        ...
        if resolved and resolved != tgt:
            ...
            repointed_from.add(tgt)
        # erro não explícito: deixe o stub vazio para a religação do legado
...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the omnigraph node ID of its source file.
