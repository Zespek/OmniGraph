...
Run once per project to make omnigraph always-on in Amp sessions:

```bash
omnigraph amp install
```

This writes a `## omnigraph` section to the local `AGENTS.md` that instructs Amp to check the graph before answering codebase questions and rebuild it after code changes. No manual `/omnigraph` needed in future sessions.

```bash
omnigraph amp uninstall  # remove the section
```
