# omnigraph reference: query, path, explain

Load this when the user asks a question against an existing graph, or runs `/omnigraph path` or `/omnigraph explain`. The core's query stub points here for the full traversal flow. These flows use the `omnigraph query` CLI when it is available and fall back to an inline NetworkX traversal otherwise.
...
import json, re
from pathlib import Path
data = json.loads(Path('omnigraph-out/graph.json').read_text(encoding='utf-8'))
vocab = set()
for n in data['nodes']:
    for c in re.findall(r'[^\W\d_]+', n.get('label','') or '', re.UNICODE):
        ...
        for p in parts:
            ...
            if 3 <= len(t) <= 30:
                vocab.add(t)
Path('omnigraph-out/.vocab.txt').write_text('\n'.join(sorted(vocab)), encoding='utf-8')
print(f'vocab: {len(vocab)} tokens')
"
```

2. Read `omnigraph-out/.vocab.txt`. Then for the user's question, select **up to 12 tokens from this exact list** that semantically match the query intent. Hard constraints:
...
```bash
omnigraph query "QUESTION"
# or: omnigraph query "QUESTION" --dfs --budget 3000
```

If the CLI is unavailable, load `omnigraph-out/graph.json` and run the traversal inline:
...
```bash
$(cat omnigraph-out/.omnigraph_python) -m omnigraph save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat omnigraph-out/.omnigraph_python) -m omnigraph save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```

---

## For /omnigraph explain

Give a plain-language explanation of a single node - everything connected to it. Prefer the CLI when installed:

```bash
omnigraph explain "NODE_NAME"
```
...
```bash
$(cat omnigraph-out/.omnigraph_python) -m omnigraph save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
