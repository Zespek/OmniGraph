...
from omnigraph.paths import out_path as _out_path
...
def _model_name() -> str:
    return os.environ.get("OMNIGRAPH_WHISPER_MODEL", _DEFAULT_MODEL)
...
def _get_whisper():
    try:
        ...
        raise ImportError(
            ...
            "Run: pip install 'omnigraph[video]'"
        ) from exc
...
def download_audio(url: str, output_dir: Path) -> Path:
    ...
    """
    from omnigraph.security import validate_url
    validate_url(url)  # bloqueia IPs privados e esquemas ruins antes da execução do yt-dlp
    yt_dlp = _get_yt_dlp()
    output_dir.mkdir(parents=True, exist_ok=True)

    # yt-dlp usa %(title)s que pode ser longo/estranho - use um nome estável baseado em hash de URL
    import hashlib
    url_hash = hashlib.sha1(url.encode(), usedforsecurity=False).hexdigest()[:12]
    out_template = str(output_dir / f"yt_{url_hash}.%(ext)s")

    # Verifique se há arquivo já baixado
    ...
    ydl_opts = {
        ...
        'no_warnings': True,
        'noplaylist': True,
        'postprocessors': [],  # não é necessário ffmpeg - use áudio nativo
    }
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if not downloaded.exists():
            # yt-dlp pode ter escolhido uma extensão diferente
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via OMNIGRAPH_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("OMNIGRAPH_WHISPER_PROMPT")
    if override:
        return override
