...
``omnigraph.resolver_registry``), with the full merged node/edge corpus
...
def resolve_pascal_inherited_calls(
    ...
    # method_nid -> sua classe proprietária nid, então o caller_nid de uma chamada bruta (um método
    ...
    for e in all_edges:
        if e.get("relation") == "inherits":
            ...
            # Conte métodos DISTINCT, não multiplicidade de arestas: o assistente da árvore
