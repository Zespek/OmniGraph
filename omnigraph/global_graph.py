...
_GLOBAL_DIR = Path.home() / ".omnigraph"
...
def _load_manifest() -> dict:
    if _GLOBAL_MANIFEST.exists():
        try:
            ...
            # Não limpe silenciosamente o manifesto do usuário em caso de erro de análise: isso
            # exclui todos os repositórios rastreados. Faça backup do arquivo incorreto e exiba o
            # erro para que o usuário possa recuperá-lo ou relatá-lo.
            ...
            try:
                ...
                print(
                    f"[omnigraph global] manifest at {_GLOBAL_MANIFEST} failed to parse ({exc}); "
...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    """
    from omnigraph.build import prefix_graph_for_global, prune_repo_from_graph
    ...
    if existing_path and existing_path != str(source_path.resolve()):
        print(
            f"[omnigraph global] warning: repo tag '{repo_tag}' previously pointed to "
    ...
    # Mesclar nós da biblioteca externa (sem source_file) por rótulo para evitar duplicação
    external_labels = {
        d.get("label", ""): n
    ...
    # A member call parked on a caller node may be answered by a repo
    ...
    from omnigraph.cross_repo_calls import link_cross_repo_member_calls
