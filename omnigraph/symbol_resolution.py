...
from omnigraph.ids import make_id as _shared_make_id
...
def _module_stem(module_name: str | None) -> str:
    """Return the final module component used to match OmniGraph source stems."""
...
def parse_python_import_aliases(path: Path) -> dict[str, ImportedSymbol]:
    ...
    # Somente instruções `from ... import ...` de nível superior contam como todo o arquivo
...
def find_unique_python_symbol(
    ...
    """Resolve one imported symbol to exactly one OmniGraph node id."""
...
def resolve_cross_file_raw_calls(
    ...
    This intentionally preserves OmniGraph's existing behavior:
    ...
    known_pairs = existing_edge_pairs(all_edges)
    # nid -> source_file, para os desempates do god node compartilhado (# 1553), então um
    # A simulação de teste com o mesmo nome não apaga mais uma chamada real entre arquivos.
    nid_to_source_file = {
    ...
    for raw_call in iter_raw_calls(per_file):
        ...
        if len(candidates) == 1:
            ...
            # Nome nu ambíguo. Aplicar os critérios de desempate compartilhados (não teste
            # preferência e depois proximidade do caminho); resolver apenas se exatamente um
            # o candidato sobrevive; caso contrário, preserve a guarda de god node e pule.
        ...
        resolved.append(
            {
                ...
                # are a discrete set and 0.8 is not one of them.
...
def _bash_make_id(*parts: str) -> str:
    ...
    Previously an inline copy to dodge an import cycle; ``omnigraph.ids`` is
    dependency-free, so it can be imported directly.
    """
    return _shared_make_id(*parts)


from omnigraph.extractors.base import _file_stem as _bash_file_stem  # receita canônica (sem ciclo de importação: a base importa apenas zspekfy.ids)
...
def resolve_bash_source_edges(
    ...
    ``bash_sources[].target_path`` contract (OmniGraph static-analysis policy):
        ...
        - Relative paths: resolved against the *source file's* directory
          ...
          OmniGraph, NOT bash runtime semantics. At runtime, ``source ./X``
    ...
    for result, path in zip(per_file, path_by_index):
        ...
        for source in bash_sources:
            ...
            if not isinstance(raw_target, (str, Path)) or not str(raw_target).strip():
                continue
            # Caminhos relativos são resolvidos no diretório do arquivo de origem —
            # Política de análise estática OmniGraph (NÃO bash semântica de tempo de execução;
            # em tempo de execução `source ./X` é relativo ao CWD, mas análise estática
            # não podemos saber o futuro CWD, então resolvemos em relação ao
            # arquivo sendo analisado para arestas determinísticas e reproduzíveis).
    ...
    for result, path in zip(per_file, path_by_index):
        ...
        for raw_call in raw_calls:
            ...
            # callee deve ser uma string não vazia - qualquer outra coisa (lista, dicionário,
            # int, None, …) é ignorado silenciosamente para evitar TypeError no
            # `in functions_by_file[...]` verificação de associação abaixo.
