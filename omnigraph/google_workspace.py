...
googleworkspace CLI (`gws`) so OmniGraph can extract their actual contents.
"""
...
def google_workspace_enabled(value: str | None = None) -> bool:
    ...
    raw = value if value is not None else os.environ.get("OMNIGRAPH_GOOGLE_WORKSPACE", "")
...
def _run_gws_export(file_id: str, mime_type: str, output: Path, resource_key: str | None = None) -> None:
    ...
    # As chaves de recursos do Drive são enviadas via X-Goog-Drive-Resource-Keys. O actual
    # O comando gws export não tem sinalizador de cabeçalho personalizado, portanto, não passe resourceKey
    # como um parâmetro de consulta não suportado.
    _ = resource_key
    output = output.resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    timeout = int(os.environ.get("OMNIGRAPH_GOOGLE_WORKSPACE_TIMEOUT", "120"))
    ...
    *,
    ...
    if ext == ".gsheet":
        if xlsx_to_markdown is None:
            raise RuntimeError("Google Sheets export requires the office extra: pip install omnigraph[office,google]")
