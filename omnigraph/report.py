# gerar GRAPH_REPORT.md - a trilha de auditoria legível por humanos
...
def load_learning_for_report(graph_path) -> dict | None:
    ...
    Reads the ``.omnigraph_learning.json`` overlay (preferred sources) next to
    ...
    try:
        gp = _Path(graph_path)
        from omnigraph.reflect import load_learning_overlay, load_memory_docs, aggregate_lessons
...
def _learning_section(lines: list, learning: dict | None, top_n: int = 10) -> None:
    ...
    # Mais corroborado primeiro (usa desc), depois por pontuação e depois por id para estabilidade.
...
def generate(
    ...
    # A desserialização JSON produz chaves de string; normalize para int então .get(cid) funciona.
    if community_labels:
    ...
    if detection_result.get("warning"):
        ...
        # files detect() saw but could not classify (no supported
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run `omnigraph update .` after code changes (no API cost).",
        ]

    # Navegação no hub comunitário. O `_COMMUNITY_*.md` indica que esses wikilinks são direcionados
    ...
    for cid, nodes in communities.items():
        ...
        # Filtrar stubs de método/função da exibição - eles são ruído estrutural
    ...
    # byte idêntico ao relatório de pré-recurso.
