"""graphdb — moved verbatim from omnigraph/export.py."""
...
def push_to_falkordb(
    ...
    identical to push_to_neo4j. Differences from the Neo4j path:
      ...
      - a named graph is selected via db.select_graph(graph_name) (default
        "omnigraph"); FalkorDB keys each graph by name in the same instance.
    ...
    # A autenticação FalkorDB é opcional. Envie credenciais apenas quando uma senha for
    # oferecido; caso contrário, conecte-se anonimamente e ignore qualquer padrão de estilo bolt
    # nome de usuário (por exemplo, "neo4j" do Neo4j), que o FalkorDB rejeita como uma ACL desconhecida
    # usuário. As credenciais incorporadas no URI têm precedência sobre os argumentos.
