...
OmniGraph nodes and edges. NOT a full SCIP protobuf implementation —
...
Entry point:
  ...
  Returns {"nodes": [...], "edges": [...]} compatible with OmniGraph's
...
def ingest_scip_json(
    ...
    """Convert a SCIP-style JSON document into OmniGraph nodes and edges.
    ...
    # Metadados por símbolo mantidos para emissão do nó de passagem 2 (evita andar novamente
    ...
    for document in documents:
        ...
        for symbol in symbols:
            ...
            # Desduplicar node_ids no índice global – registros de símbolos duplicados
            # dentro do MESMO documento produzem node_ids idênticos, e nós
            # não quero que pareçam ambiguidades entre documentos.
...
def _emit_symbol_node(
    ...
    seen_node_ids.add(node_id)  # rótulo usa display_name ou sufixo (nunca vazio para símbolos válidos)
    nodes.append(
        {
            "id": node_id,
            "label": label,
...
def _scip_relation_for(rel: dict[str, Any]) -> str:
    """Pick the OmniGraph relation tag for a SCIP relationship dict.
...
def _make_scip_node_id(symbol: str, source_file: str) -> str:
    """Derive a stable OmniGraph node ID from a SCIP symbol identifier.
...
def _scip_kind_to_file_type(kind: str) -> str:
    """Map SCIP symbol kind to a OmniGraph file_type."""
    # Todos os símbolos SCIP são entidades de código (funções, métodos, classes,…);
    # o `tipo` é preservado em metadados para consumidores a jusante.
    _ = kind  # reconhecido, mas não usado atualmente para roteamento file_type
    return "code"
