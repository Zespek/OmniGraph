...
Rules:
- For codebase or architecture questions, when `omnigraph-out/graph.json` exists, first run `omnigraph query "<question>"` (CLI) or `query_graph` (MCP). Use `omnigraph path "<A>" "<B>"` / `shortest_path` for relationships and `omnigraph explain "<concept>"` / `get_node` for focused concepts. These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
