"""omnigraph prs — graph-aware PR dashboard.
...
Usage:
  omnigraph prs                   # dashboard of all open PRs
  omnigraph prs <number>          # deep dive on one PR
  omnigraph prs --triage          # Opus ranks your review queue
  omnigraph prs --worktrees       # show worktree → branch → PR mapping
  omnigraph prs --conflicts       # PRs sharing graph communities (merge-order risk)
  omnigraph prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def attach_graph_impact(
    ...
    # Buscar diferenças simultaneamente - gh pr diff é o gargalo (E/S de rede)
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  omnigraph prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using OMNIGRAPH_TRIAGE_BACKEND or first available key."""
    from omnigraph.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("OMNIGRAPH_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("OMNIGRAPH_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from omnigraph.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  omnigraph.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
...
def cmd_prs(argv: list[str]) -> None:
    ...
    # O impacto do grafo é caro (chamadas gh pr diff simultâneas) - busque apenas quando
    # o usuário realmente precisa disso: aprofundamento, triagem e detecção de conflitos.
