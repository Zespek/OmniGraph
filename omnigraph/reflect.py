"""Deterministic "work memory" reflection over omnigraph-out/memory/.

`omnigraph reflect` reads the Q&A memory docs that `omnigraph save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.omnigraph_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``omnigraph-out/reflections/LESSONS.md`` rather than inside the wiki
because ``omnigraph export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from omnigraph.ingest import OUTCOMES
...
# entre plataformas (C pow pode diferir no último ULP).
_SCORE_NDIGITS = 9
...
# don't reconheça é ignorado, portanto, os arquivos .md estrangeiros na memória/ são ignorados de forma limpa.
...
def load_memory_docs(memory_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(memory_dir.glob("*.md")):
        ...
        docs.append(parsed)
    # Ordem estável: por (data, nome do arquivo) para que a saída seja determinística entre execuções.
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `omnigraph export wiki` reads graph.json + .omnigraph_analysis.json +
    .omnigraph_labels.json. Community membership in the analysis sidecar is keyed by
    ...
    if labels_path.exists():
        try:
            ...
            labels = {}
    # id -> rótulo do grafo, portanto, uma citação em formato de rótulo também é resolvida para uma comunidade.
...
def _doc_community(nodes: list[str],
    ...
    # A contagem mais alta vence; em caso de empate, o rótulo menor (contagem mais negativa primeiro,
    # em seguida, rótulo ascendente) - um min() simples sobre (-count, rótulo).
...
def _empty_bucket() -> dict[str, Any]:
    return {
        ...
        "node_pos": Counter(),
        "node_neg": Counter(),
        # nó -> data do evento mais recente visto (para a linha de veredicto contestada)
        "node_last": {},
        # nó -> lista de (data, pergunta, resultado) para citações úteis/corrigidas.
        # Alimenta a proveniência por nó da sobreposição do sidecar; nunca lido por LESSONS.md,
        # portanto, não afeta a forma pública do agregado.
        "node_provenance": {},
        "dead_ends": [],
        "corrections": [],
    }
...
def _record_node(bucket: dict[str, Any], node: str, sign: int,
    ...
    # Proveniência: apenas eventos úteis/corrigidos são registrados (a experiência
    # trilha com a qual um agente se preocupa - o que citou esse nó e como ele acabou).
...
def aggregate_lessons(docs: list[dict[str, Any]],
    ...
    for doc in docs:
        ...
        date = doc.get("date", "")
        # Um evento por nó por documento; descartar nós que o grafo não conhece mais.
    ...
    return {
        ...
        **_finalize_sources(overall, min_corroboration),
        ...
        # Privado: trilha por nó (data, pergunta, resultado) para o sidecar
        # proveniência da sobreposição. Prefixado por sublinhado e não renderizado por
        # render_lessons_md, portanto a forma agregada pública permanece inalterada.
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `omnigraph reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in omnigraph-out/memory/. "
    ...
    # Nova linha à direita, sem linhas de espaço em branco à direita.
    return "\n".join(out).rstrip("\n") + "\n"
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``omnigraph reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".omnigraph_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".omnigraph_labels.json")
    ...
    # Projete também um sidecar experiencial derivado próximo a graph.json quando um grafo
    # está em mãos. Melhor esforço: uma falha no sidecar nunca deve quebrar LESSONS.md.
    if graph_path is not None:
        try:
...
# uma curta trilha de proveniência. graph.json (verdade estrutural durável) nunca é
# tocado – as superfícies de leitura mesclam essa sobreposição somente no momento da exibição.
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/omnigraph-out/`` (so its own dir is not the root) or directly
    ...
    Order: the committed ``.omnigraph_root`` marker (#686/#1423 — authoritative for
    an absolute/elsewhere ``OMNIGRAPH_OUT`` override); then the layout-appropriate
    root *first* — graph.json's parent's parent for the ``omnigraph-out`` layout,
    ...
    try:
        recorded = (out_dir / ".omnigraph_root").read_text(encoding="utf-8-sig").strip()
        if recorded:
...
def _provenance_for(node: str, prov_map: dict[str, list],
    ...
    # Classifique o recente primeiro; (data desc, depois questione para um desempate estável).
...
def build_learning_overlay(agg: dict[str, Any], graph_path: Path,
    ...
    # id -> entrada; um id canônico pode ser citado tanto na forma de id quanto no rótulo,
    # mas as desduplicações agregadas por string de nó, então as colisões aqui são benignas e
    # resolvido deterministicamente por ordem de iteração (preferencial, provisório, contestado).
...
def write_learning_sidecar(agg: dict[str, Any], graph_path: Path,
    ...
    """Write ``.omnigraph_learning.json`` next to ``graph_path`` deterministically.
