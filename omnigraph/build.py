...
def _infer_merge_root(graph_path: Path) -> str | None:
    ...
    ``<root>/omnigraph-out/graph.json`` -> ``<root>``). The grandparent heuristic is
    applied only when ``graph.json``'s own directory actually looks like a omnigraph
    ...
    try:
        ...
        pass
    from .paths import OMNIGRAPH_OUT
    try:
        if (
            parent.name == Path(OMNIGRAPH_OUT).name
            or (parent / ".omnigraph_root").exists()
...
def _derive_prune_root(prune_sources: "list[str]", stored_sfs: "set[str]") -> "str | None":
    ...
    layouts (``graph.json`` not under ``<root>/omnigraph-out/``), so every prune
...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    # — after the links remap so aliased edges are covered too.
    ...
    _ast_file_nodes: list[tuple[str, str]] = []  # (node_id, source_file) for AST file-self nodes
    ...
    for nid in sorted(node_set):
        ...
        if attrs.get("source_location") or is_ast:
            ...
            if is_ast:
                ...
                _loc_nodes[key] = nid
                # a self-referential semantic pass (e.g. re-extracting a
                # saved omnigraph-out/memory/*.md query answer) mints a NEW
                ...
                # hit the (sf, label) key above — same underlying bug as,
    ...
    # Pass 2b: catch ghosts the (source_file, label) key above cannot,
    # because their source_file is simply wrong — a semantic pass over a
    # document that only *mentions* a file (a saved omnigraph-out/memory/*.md
    ...
    for edge in sorted(
        ...
        if attrs.get("definition_file"):
            # Same portability rule as source_file; heals a graph
        ...
        # `calls` and `uses` after everything. On omnigraph's own corpus that
    ...
    if hyperedges:
        ...
        if kept_hyperedges:
            ...
            G.graph["hyperedges"] = []
            print(
                f"[omnigraph] WARNING: all {len(hyperedges)} hyperedge(s) were "
    ...
    if dedup and combined["nodes"]:
        ...
        for n in combined["nodes"]:
            if isinstance(n, dict):
                ...
                # deduplication, so exact-ID collision checks and same-file
...
def merge_raw_extraction(
    ...
    *,
    ...
    # Check for unverified semantic shrink on re-extracted sources.
...
def build_merge(
    ...
    *,
    ...
    #
    # AST replacement ownership is derived from explicit extraction
    # provenance (ast_sources or chunk-level "extracted_sources"), so cross-file
    # stub nodes emitted by extractors (e.g. .sln project stubs or ProjectReference
    # stubs) do not pollute the replacement set and wipe the target project's
    # nodes/edges. Falls back to AST node source_file only when no provenance is
    ...
    if new_sources:
        ...
        if replaced_n:
            print(
                f"[omnigraph] Replaced {replaced_n} node(s) from re-extracted "
                f"source file(s).",
                file=sys.stderr,
            )
    ...
    # attached to G afterwards: entity dedup rewires every edge endpoint
    # and every hyperedge member it sees onto the survivor, but a
    ...
    # Untouched existing nodes must not be collapsed with each other during dedup.
    _protected_ids = {
    ...
    if prune_sources:
        ...
        if not n_nodes and not edges_to_remove:
            if (prune_set or prune_abs) and not _matched_prune_entries:
                ...
                print(
                    f"[omnigraph] WARNING: {len(prune_sources)} prune source(s) "
                ...
                print(
                    f"[omnigraph] {len(prune_sources)} source file(s) deleted or "
                    f"excluded since last run — no matching nodes or edges in "
                    f"graph, already clean.",
                    file=sys.stderr,
                )

    # Safety check: refuse to SILENTLY drop nodes (, reworked in).
    ...
    if had_graph and not dedup:
        ...
        if unexplained:
            raise ValueError(
                f"omnigraph: build_merge would drop {len(unexplained)} node(s) "
...
def prefix_graph_for_global(
    ...
    # Out-of-band hyperedges must be relabeled with the nodes (, after
    # @oleksii-tumanov's diagnosis in PR): relabel_nodes copies graph
