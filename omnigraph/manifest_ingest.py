...
per package -- keyed by NAME via :func:`omnigraph.ids.make_id` -- plus
...
def extract_package_manifest(path: Path) -> dict[str, Any]:
    ...
    for dep in info.get("deps", []):
        ...
        if dep_nid == pkg_nid or dep_nid in seen:
            continue
        seen.add(dep_nid)
        # A aresta tem como alvo o ID canônico do pacote da dependência. Se esse pacote for
        # próprio manifesto está no corpus, a aresta resolve para seu (único) nó; se
        # a dependência é externa, build_from_json elimina a aresta pendente. Nós
        # deliberadamente NÃO emita um nó stub - um stub com um source_file vazio
        # correria o risco de destruir o source_file do nó real sob id-dedup.
        edges.append({
            "source": pkg_nid,
            "target": dep_nid,
            "relation": "depends_on",
...
def _parse_pom(text: str) -> dict | None:
    # Elimine o namespace padrão para que findtext/findall não precise do prefixo {uri}.
