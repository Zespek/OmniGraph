...
_MAX_FETCH_BYTES = 52_428_800   # Limite máximo de 50 MB para downloads binários
_MAX_TEXT_BYTES  = 10_485_760   # Limite rígido de 10 MB para HTML/texto
...
def _max_graph_file_bytes() -> int:
    ...
    Honors the ``OMNIGRAPH_MAX_GRAPH_BYTES`` environment variable so users with
    ...
    take effect.
    """
    raw = os.environ.get("OMNIGRAPH_MAX_GRAPH_BYTES", "").strip()
    if not raw:
...
def _ip_is_blocked(ip: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    ...
    """
    # Para endereços NAT64, verifique o IPv4 incorporado em vez do wrapper
...
def _build_opener() -> urllib.request.OpenerDirector:
    # build_opener substitui os manipuladores HTTP(S) padrão por nossos protegidos por SSRF
    # subclasses, então cada conexão resolve + valida o DNS uma vez e se conecta
    # para esse IP exato. Thread-safe: nenhum estado global do processo sofre mutação.
...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 omnigraph/1.0"})

    with opener.open(req, timeout=timeout) as resp:
        # urllib gera HTTPError para não-2xx ao usar urlopen diretamente;
        # com um abridor personalizado, verificamos manualmente a segurança.
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `omnigraph-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick omnigraph into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == OMNIGRAPH_OUT_NAME:
                base = candidate
                break
        if base is None:
            base = Path(OMNIGRAPH_OUT).resolve()

    base = base.resolve()
    if not base.exists():
        raise ValueError(
            f"Graph base directory does not exist: {base}. "
            "Run /omnigraph first to build the graph."
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside omnigraph-out/ are permitted."
        )
...
def check_graph_file_size_cap(path: Path) -> None:
    ...
    ``OMNIGRAPH_MAX_GRAPH_BYTES`` env var can be set before import and still
    apply.
    ...
    if size > cap:
        raise ValueError(
            ...
            f"(set OMNIGRAPH_MAX_GRAPH_BYTES=<bytes> or "
            f"OMNIGRAPH_MAX_GRAPH_BYTES=<N>GB to raise the limit)"
        )
...
def _sanitize_metadata_string(value: object) -> str:
    ...
    return text  # html é importado no nível do módulo (linha 5)
...
def _sanitize_metadata_value(value: object) -> object:
    ...
    if isinstance(value, bool):
        # bool é uma subclasse de int – deve ser verificado primeiro para evitar coerção.
        return value
