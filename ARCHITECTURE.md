...
omnigraph is a Claude Code skill backed by a Python library. The skill orchestrates the library; the library can be used standalone.
...
Each stage is a single function in its own module. They communicate through plain Python dicts and NetworkX graphs - no shared state, no side effects outside `omnigraph-out/`.
...
| `callflow_html.py` | `write_callflow_html(...)` | omnigraph-out files → Mermaid architecture/call-flow HTML |
...
All external input passes through `omnigraph/security.py` before use:
...
- Graph file paths → `validate_graph_path()` (must resolve inside `omnigraph-out/`)
