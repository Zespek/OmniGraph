...
omnigraph processes your files in three passes:
...
Markdown sidecars under `omnigraph-out/converted/`. Office files (`.docx`,
`.xlsx`) use the `[office]` extra. Google Workspace shortcuts (`.gdoc`,
`.gsheet`, `.gslides`) are opt-in with `--google-workspace` or
`OMNIGRAPH_GOOGLE_WORKSPACE=1` and require an authenticated `gws` CLI.
...
| omnigraph source + Transformer paper | 4 | **5.4x** |
...
Every extracted file is fingerprinted by content hash. Re-runs skip unchanged files entirely — only new or modified files go through extraction again. The cache lives in `omnigraph-out/cache/`.
