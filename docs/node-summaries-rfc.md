...
This RFC proposes an optional way for omnigraph to expose compact file-level
...
`omnigraph query`, `omnigraph explain`, MCP node lookup, and graph navigation.

## Goals

- Help agents choose relevant files with less context.
- Preserve omnigraph's offline, deterministic behavior by default.
...
- Replacing `omnigraph explain`; summaries should make `explain` more useful.
...
- Display a summary in `omnigraph explain <node>` when one exists.
- Include summaries in `omnigraph serve` / MCP node lookup when one exists.
...
{
  "label": "extract.py",
  "source_file": "omnigraph/extract.py",
...
be fetched through `omnigraph explain`, `omnigraph query`, or direct file reads
when needed.
...
```bash
omnigraph . --summarize-nodes
omnigraph explain "extract.py"
```
...
3. Surface summaries in `omnigraph explain`.
4. Surface summaries in `omnigraph serve` / MCP node lookup.
...
- Extend summaries to class or module-level nodes if file-level summaries prove
  useful.
- Allow `omnigraph query` to include summaries for returned nodes under a budget.
- Add cache/freshness metadata if summaries are generated separately from the
  main graph.

## Questions for maintainers and users

1. Should omnigraph prefer one artifact (`graph.json`) or keep generated text in a
   sidecar?
2. Should deterministic file-level summaries be generated during graph creation,
   or only through an explicit command such as `omnigraph summarize`?
